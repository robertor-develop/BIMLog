...
$requiredBuildRoot = 'H:\BIMLogPlugin2021'
if (-not ($sourceRoot.StartsWith($requiredBuildRoot + '\',[StringComparison]::OrdinalIgnoreCase))) { throw "STOP: Navisworks 2021 source/build/package root must be under $requiredBuildRoot." }
...
foreach($name in @('INSTALL-BIMLOG-LENS-NEXT-2021.bat','UNINSTALL-BIMLOG-LENS-NEXT-2021.bat','Install-BIMLogLensNext.ps1','Install-BIMLogLensNext2021.ps1','Uninstall-BIMLogLensNext2021.ps1','README-ROBERTO-RUBEN.txt','FIELD-ACCEPTANCE-CHECKLIST.txt','COLLECT-LENS-NEXT-FAILURE-DETAILS.bat','Collect-LensNextFailureDetails.ps1')){Copy-Item -LiteralPath (Join-Path $sourceRoot $name) -Destination $packageRoot -Force}
