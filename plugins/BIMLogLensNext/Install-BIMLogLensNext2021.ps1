...
if($manifest.productYear -ne 2021 -or $manifest.release -ne 'v1.05.N16-P10' -or $manifest.binaryVersion -ne '1.5.16.10'){throw 'STOP: wrong product year or release.'}
...
if(Get-Process -Name 'roamer' -ErrorAction SilentlyContinue){throw 'STOP: close Navisworks Manage 2021 before installation.'}
...
$source=Join-Path $packageRoot 'BIMLogLensNext2021.bundle'
$installRoot='C:\ProgramData\Autodesk\ApplicationPlugins'
$target=Join-Path $installRoot 'BIMLogLensNext2021.bundle'
...
try{
  ...
  $installedNative=Join-Path $target 'Contents\BIMLogLensNext.Native2021.dll'
  ...
  if($installedComponent.ModuleName -ne './Contents/BIMLogLensNext.Native2021.dll'){throw 'STOP: installed package points to the wrong native module.'}
  if($installedRequirements.Platform -ne 'NAVMAN' -or $installedRequirements.SeriesMin -ne 'Nw18' -or $installedRequirements.SeriesMax -ne 'Nw18'){throw 'STOP: installed package does not target Navisworks Manage 2021 exactly.'}
