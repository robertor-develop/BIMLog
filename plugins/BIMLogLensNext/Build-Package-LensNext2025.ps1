...
if($component.ModuleName -ne './Contents/BIMLogLensNext.Native2025.dll'){throw 'STOP: PackageContents.xml has the wrong native module.'}
if($requirements.Platform -ne 'NAVMAN' -or $requirements.SeriesMin -ne 'Nw22' -or $requirements.SeriesMax -ne 'Nw22'){throw 'STOP: PackageContents.xml does not target Navisworks Manage 2025 exactly.'}
...
$pulseSource=Join-Path $sourceRoot "pulse\$year\$pulseBundleName"
if(-not(Test-Path -LiteralPath $pulseSource -PathType Container)){throw "STOP: Pulse-only source bundle is missing: $pulseSource"}
Copy-Item -LiteralPath $pulseSource -Destination (Join-Path $packageRoot $pulseBundleName) -Recurse -Force
...
$manifest=[ordered]@{schemaVersion='bimlog-lens-next-package-v1';product='BIMLog Lens Next';productYear=$year;release=$Version;binaryVersion=$binaryVersion;installBundle=$bundleName;pulseBundle=$pulseBundleName;installTarget="C:\ProgramData\Autodesk\ApplicationPlugins\$bundleName";originalLensPreserved=$false;pulsePreserved=$true;legacyLensRetirementPolicy='replace-shared-legacy-bundle-with-verified-pulse-only-bundle';files=$manifestFiles}
...
& (Join-Path $packageRoot 'Install-BIMLogLensNext2025.ps1') -PackageOnly
