...
| 340 | `1e588fcf` | Consolidate positive/negative candidate, real-schema SQL, UI and still-guarded mutation regressions in an executable block gate. |
...
Block gate: `pnpm run test:edt-engine-block13` with the isolated PostgreSQL URL, API and frontend typechecks passed. After the route-graph repair, the complete exact-head `gate:pre-push` passed on clean `c137fe608c7685c79205196f9beedce6f29c5339`, including production build/runtime closure and artifact proof. GitHub master was fast-forwarded from published `bd3be0742d702ba7d186d6a47b87830b526a183b` to that exact commit and independently read back. No Block 13 publication was attempted; the accepted live site remains the corrective Block 12 release.

The first full gate found a stale tracked route-interconnection graph after the new candidate endpoint. Regenerating that deterministic graph brought the API count to 660; `pnpm run test:post120-block37` then passed. No runtime route or permission was changed by this inventory repair.
