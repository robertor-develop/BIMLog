...
Block gate: `pnpm run test:edt-engine-block13` with the isolated PostgreSQL URL, API and frontend typechecks passed. The full exact-head pre-push gate, clean source commit and exact GitHub push remain pending. Pending gates must be updated with observed evidence, not assumed.
