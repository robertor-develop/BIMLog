...
dotnet build (Join-Path $root "Build18OrthographicProbe.csproj") -c Release
...
try {
    ...
    $application.AddPluginAssembly((Join-Path $root "bin\Release\net48\Build18OrthographicProbe.dll"))
    $status = $application.ExecuteAddInPlugin("Build18OrthographicProbe.BIMLog", @([IO.Path]::GetFullPath($OutputDirectory)))
