...
Physical 2021 inventory exposed an additional historical direct-load DLL at `C:\Program Files\Autodesk\Navisworks Manage 2021\Plugins\BIMLogNavisPlugin`. It contained the retired Original Lens panel and sat outside the ApplicationPlugins inventory previously checked by Build 119. The verified installer treats that exact year-bound path as a rollback-preserved retirement candidate, installs the complete Pulse-only bundle and Lens Next bundle, removes the direct loader, and fails if it survives. Package staging includes the Pulse DLL/PDB for both years. The physical 2021 cutover passed with exactly Pulse plus Lens Next active, the retired direct loader absent, WebView2 complete, and licensing unchanged. Dual-year deterministic package, core/native, and repeated upgrade/rollback simulation pass; only physical 2025 confirmation remains a field action assigned to Ruben.
...
## Lens Next responsive dock — Block 2 Platform-only candidate — 2026-09-23

The next five source builds use the same mounted issue component and state at every width. Narrow and medium Lens containers present one side workspace at a time. Filters keep the canonical query controls and return to the issue list; Viewpoints moves between the list and selected details with Back preserving the selected issue and scroll position; Create retains the existing capture/save handler and offers Cancel; Settings retains project binding, model tools and My View preferences. At measured wide width, Filters/List/Details remain side by side. Local Chrome exercised the actual component at narrow/medium/wide viewport sizes with fixture records, including nonzero list-scroll restoration and resize continuity. This does not prove connected Navisworks or authenticated production workflows. No Native source, installer, manifest, API, database schema, or customer data changed, so focused Navisworks smoke is not triggered by this block. Publication and live verification are due after its complete exact-head release gate.
