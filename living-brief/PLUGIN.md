...
- Build 10 **open**: no interactive web 3D proof exists yet, because the real-model geometry and matrix semantics are not verified. A fixture-only preview would not satisfy the one-genuine-clash go/no-go gate. Field proof must first validate exact model binding, triangle/transform correctness, clipping, rights, size, responsive orbit controls and performance in Navisworks 2021 and 2025. Existing Working View/XML and digest behavior remains unchanged.
