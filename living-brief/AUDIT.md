...
- Build 13 identified the remaining 1.526 MB `ProjectDetail` chunk as an eager-import aggregation of 18 independently selected project workspaces. The correction adds one lazy boundary per existing workspace and a local accessible loading state inside the unchanged project shell.
