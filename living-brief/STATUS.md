...
- Local Builds 21–25 connect the existing published Governance Policy to Intake's Delivery Workflow options: published scope and fingerprint are checked, an applicable policy version is named beside the selected workflow, and known phase-review/final-approval/document incompatibilities are visibly blocked before activation. Focused tests cover no-policy, template-scoped, approved-only, fingerprint mismatch and overlapping-policy cases. Activation still uses its own canonical snapshot validation; no parallel policy store or Native/installer change was introduced. This block is a push-only candidate until its exact-head gate and GitHub push pass; no live Chrome or six-role acceptance is claimed.
