...
Build 005 valid-startup correction: the next complete gate proved the invalid-authority fix but exposed that a valid cold runtime could still miss the 6-second `/api` probe because the 5.7 MB application graph was parsed before the listener bound. Packaging now emits a small `dist/index.cjs` listener bootstrap and a deferred native-ESM `dist/app.mjs` application bundle. The bootstrap serves only liveness while starting, keeps canonical readiness at 503, and delegates only after the application barrier completes. An intermediate CommonJS deferred bundle initialized correctly but took 12547 ms on the exact cold proof, above the unchanged 8000 ms budget; native ESM removes that wrapper path rather than weakening the gate. Focused type/secret/encoding checks pass; the exact clean full gate remains required before push.
