...
- Original Lens v1.60.36 adds a separate, explicit one-time historical migration. It pulls the configured project's authoritative BIMLog rows, enumerates only BIMLog-managed Saved Viewpoints, matches by exact project/server identity, exact non-empty Navisworks GUID, or a unique exact BIMLog identifier, captures the complete visual state, and uploads it to that exact BIMLog row. Ambiguous and unmatched records are preserved and reported; the migration never scans model objects, reorganizes Saved Viewpoints, or writes the model file, and restores the pre-migration working view.
- The H-only v1.60.36 source compiles for net48 and its focused migration contract passes 12/12. Package `H:\BIMLogPlugin2021\BIMLog-Lens-Navisworks2021-v1.60.36.zip` has SHA-256 `D2C47CEA6DABA7E149597658C01517DF7C6C6EC69A8903A3268266CDC21E2782`. Installation, migration execution on the real model, push, Replit synchronization/publication, and connected Navisworks acceptance have not occurred.
