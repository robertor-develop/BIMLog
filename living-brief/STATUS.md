...
- The exact local operational-intake product candidate is `a42b23e2db920ddb9cd417464ef7ff9a1bb6de83`, release identity `v1.60.35.09-F`. It preserves the governed package/register/notification and accessible markup scope, valid `/feedback` and `/admin/feedback` deep-link handling, and stable-ID targeting of the Feedback administration tab. The predecessor controlled Chromium evidence remains accepted for its exact predecessor inputs only and does not prove this operational-intake delta.
...
- The reviewer authorization mismatch is closed in source at `a42b23e2`: the Feedback navigation tab and reviewer panel render only when the authenticated Zustand store user is a super-admin; a non-super-admin deep link resolves to the ordinary administration overview and never mounts the reviewer API surface. The controlled browser evidence writer now covers super-admin queue/deep-link/detail, optimistic claim, customer response, package and follow-up-register downloads, plus non-super-admin tab/panel/API denial. This is expanded harness source, not a new executed or retained exact-head browser receipt.
...
- Production ClamAV scanner and quarantined-evidence worker source is implemented at `f13c38f4`. Activation requires `clamav-cli` plus an exact absolute executable, SHA-256, and version binding; startup verifies executable identity and version, and scan receipts bind verdict, bytes, media type, hash, scanner version, and executable identity. No approved external executable/hash/version is configured or live in this candidate, so production evidence remains quarantined and the worker remains inactive.
