...
- The exact local operational-intake product candidate is `f13c38f4e7090ed0e57e708f86a4f104fb42e1fc`, release identity `v1.60.35.09-F`. It preserves the governed package/register/notification and accessible markup scope while adding valid `/feedback` and `/admin/feedback` deep-link handling, including stable-ID targeting of the Feedback administration tab. The predecessor controlled Chromium evidence remains accepted for its exact predecessor inputs only and does not prove this operational-intake delta.
