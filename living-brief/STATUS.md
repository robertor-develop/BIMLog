...
# Lens Next v1.0.08-Pro / M8 - Governed first-use legacy visual-state migration

- Legacy rows that predate BIMLog visual-state custody expose **Migrate & open working view** only while an authenticated Navisworks bridge is bound to the same project. The bridge resolves the exact immutable issue identity, captures the complete native state once, BIMLog persists the JSON plus SHA-256 digest, and only then is that persisted package applied as the temporary Working View. Subsequent opens load only the BIMLog package. The flow never creates, edits, or deletes a Saved Viewpoint or writes the model file.
- Dedicated platform/migration behavior passes 8/8, whole-workspace Lens Next behavior passes 28/28, and BIMLog typechecking passes locally. Full workspace build, push, publication, and real Navisworks acceptance remain required gates.
- Reconciliation baseline is implementation commit `52c027dbd080cd1a6ff62116a37e99fee0f8ba28`; no production migration or field acceptance is claimed by this record.

# Lens Next v1.0.18 - projection-safe legacy capture correction

- Connected Navisworks field evidence reached exact legacy identity resolution but failed while reading an unset projection-specific `FocalDistance`. The canonical Navisworks 2021 adapter now treats focal distance and focal extents as optional camera values: positive finite values are retained, while Navisworks `InvalidOperationException` for an unset property is represented as null instead of aborting the governed capture.
- The native toolbar reports `v1.0.18` beside its small `LIVE`/`READY` status. The redundant web-workspace `Lens Next v1.0.08-Pro` heading is removed. The H-only v1.0.18 package builds with core tests 30/30 and native 2021 tests 19/19 and is installed in Autodesk's required Navisworks 2021 load path; live load and connected migration acceptance remain pending.
