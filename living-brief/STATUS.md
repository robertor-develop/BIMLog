...
- The exact frozen local source-and-harness candidate is `3736433a7a040461a979e92e5621a3728c658d7f` with package product correction `298585c62d6f8521449e0467286d33ace1cd67ca`, browser harness `c7a945acb3bb26b263be36cd8107a8d78ed0cfcf`, delivery correction `9440b0e21290d2639c737e674b044dfbfbae5504`, and release identity `v1.60.35.09-F`. It extends the existing authenticated Feedback widget and route rather than creating another feedback surface. Implemented source generates a bounded ZIP containing a canonical JSON manifest, a human-readable PDF, and only scanner-clean hash-verified evidence; exposes a reason-audited PostgreSQL follow-up CSV register; records immutable submission acknowledgments, reviewer notices, staff responses/decisions/fixes/answers, in-app delivery/read state, and explicitly opted-in provider-backed email copies; and preserves original/marked screenshot linkage from the browser through the server package contract. The final correction admits all six bounded multipart metadata fields, appends durable email `sent`/`failed`/`skipped` delivery outcomes, allows idempotent retry until a `sent` receipt exists, and adds a fail-closed deterministic PDF proof-output helper for artifact inspection.
