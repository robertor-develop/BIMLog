...
- Optional voice and screen capture require a durable `feedback-capture-v1` consent receipt with purpose, actor, timestamp, kind, and revocation state before a later capture gesture. Screenshot crop provenance retains source hash/linkage, bounds, dimensions, origin, and transformation timestamp.
...
- Remediation commit `1eb3b73ec78479ece9332378856d23af4df0ee2d` closes the six source-level P0 findings with transactional record/audit writes, hash-bound concurrent idempotency, active tenant/project authority checks, compensated evidence custody, safe media shutdown, and customer backlog/history/reopen/evidence review.
- The canonical additive migration runs transactionally and idempotently in the isolated loopback PostgreSQL proof. The combined DB/HTTP proof passes 30/30, including migration rollback, stale reopen, concurrent triage, triage-audit rollback, transcription default-deny/replay, visibility denial, CSV neutralization, retention hold, clean-byte download, and the prior tenancy/idempotency/residue cases.
- Actual production-component evidence passes 16/16 across English desktop and Spanish exact-390 flows, including visible versioned consent receipts before the separate microphone and screen-capture gestures.
- The exact canonical migration proof also enforces company, lifecycle-state, optimistic-version, stable-ID, idempotency, asset-deduplication, and transcription-request constraints and indexes.
- `ensureFeedbackSchema` is the sole runtime Feedback migration authority; the superseded inline startup DDL has been removed.
- Feedback evidence no longer defaults to a process-local production directory. Production startup now requires an explicitly selected durable-filesystem backend and an operator authority manifest held outside the data root; workspace, repository, artifact, build, cache, and temporary roots are rejected. The external binding pins the sanitized backend identity, exact root, backup requirement, and only the capabilities it authorizes. Stored database values remain opaque keys rather than host paths. Local disk fixtures remain development/test-only, and scanner/transcription fixtures cannot activate in production.
- The Feedback schema migration takes a transaction-scoped PostgreSQL advisory lock before canonical additive DDL, serializing concurrent application startup owners. The focused storage contract passes 20/20, including cross-process visibility, reopen persistence, exact bytes, multi-writer opaque identity, full existing-component link/reparse refusal, zero-residue forced write/fsync failures, held-delete refusal, backup inventory/exact-byte restore, and production default denial.
- The manifest-bound disposable PostgreSQL 18.4 harness passes the canonical DB/HTTP contract 30/30 at source commit `c8218d58db0daa2f737862f22324a495f5f1e5cc`, including actual advisory-lock blocking/serialization. The binding was consumed value-blind and no database URL was persisted.
- Capture evidence upload now fails closed if the corresponding versioned consent receipt is absent or expired; an undefined consent identifier can no longer enter multipart custody.
