...
- Local Builds 6–10 correct the Intake presentation and activation boundary: Quick Start exposes project Discipline only; generic activation tasks cannot inherit legacy project-wide Service/Phase; older drafts retain their values visibly in both modes until the user explicitly clears them; Client/floor guidance separates project/contract authority from package/task locations. Focused tests and typechecks pass, but the full gate, push, publish and authenticated live verification remain pending at this candidate state. This does not complete the template/governance/EDT acceptance path or validate the rejected guide.
