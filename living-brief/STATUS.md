...
- The second five-build source candidate adds versioned mapping profiles, an exact path resolver, a company/project routing API and bilingual editor with server-calculated preview. The tenth build changes the legacy Coordination Hub to refuse `queued_sync` before writing an event or consuming its temporary bytes, while explaining the unavailable action in the UI. This prevents a false success but does **not** implement durable SharePoint delivery. Publication, real role/browser verification and tenant-site Graph round-trip are still pending.
