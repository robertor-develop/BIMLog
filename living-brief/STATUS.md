...
Build 005 valid-startup correction: the next complete gate proved the invalid-authority fix but exposed that a valid cold runtime could still miss the 6-second `/api` probe because the 5.7 MB application graph was parsed before the listener bound. Packaging now emits a small `dist/index.cjs` listener bootstrap and a deferred `dist/app.cjs` application bundle. The bootstrap serves only liveness while starting, keeps canonical readiness at 503, and delegates only after the application barrier completes. The first split-package proof also caught Node's CommonJS namespace wrapper; the loader now unwraps that single generated boundary while retaining bootstrap type validation. Focused bootstrap/type/secret/encoding checks pass; the exact clean full gate remains required before push.
