...
- Builds 31–35 add compatibility validation against already published workflows before a Governance Policy can publish, recheck the applicable policy at workflow approval, and block workflow retirement when the published policy forbids it. The existing publication-time check remains the final authority. The focused binding behavior and API typecheck pass; isolated HTTP execution, complete pre-push gate and push are separate gates for this block. This is not enforcement of approval thresholds, company-role grants, other change rules, or positive six-role EDT acceptance. This five-build block is push-only; Replit publication and authenticated Chrome smoke are due at Build 40.
