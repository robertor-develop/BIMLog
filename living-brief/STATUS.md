...
- Replit Nix source now includes ClamAV, but production activation remains fail-closed until Replit supplies and the application verifies the absolute `clamscan` executable, exact SHA-256, and exact version. Existing ticket metadata remains reviewable while attachment bytes stay quarantined. The scan worker alone may promote exact bytes to clean and trigger regenerated packages; manual database promotion is prohibited.
