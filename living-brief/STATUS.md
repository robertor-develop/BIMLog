...
- Feedback entry-point correction `7fbcb3ec0978b3529ec67c27fa8063b62d108caa` separates creation from follow-up: the floating Feedback button and plain `/feedback` route always open **New feedback**, while in-app and email notification links use `/feedback?view=mine` to open **My feedback**. Both tabs remain available inside the same dialog. Focused package, route-authority, widget, API/UI TypeScript, and diff checks pass locally; this hotfix is not yet pushed or republished.
