...
- Local Builds 11–15 narrow the Delivery Workflow lifecycle gap without changing schema: a creator or last editor cannot approve their own company template even when no economic allocation exists; an economic allocation additionally requires the separate checker to hold company Finance cost-approver authority. The editor explains the distinction. Retiring a published or superseded version requires a bounded reason retained with the actor in append-only audit history and visible to company PMO users. Focused tests and typechecks are local candidate evidence only; GitHub push, publication, six-role field proof and full Governance policy runtime enforcement remain separate.
