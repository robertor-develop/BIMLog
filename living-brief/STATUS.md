...
- Replit Nix source includes ClamAV and launcher successor `d0d8d7bb9ba5939ede74edd2e5f84c1ea8104aae` binds a committed direct launcher at `/home/runner/workspace/artifacts/api-server/scripts/feedback-clamav-replit.sh`, launcher SHA-256 `2e98606a41feb0d24de65dacadf443f270db775e526ab1b9a671bf23baeb00bc`, exact content-addressed scanner path `/nix/store/j01wsla7rfrgjv3605l561mni4b4ka05-clamav-1.4.3/bin/clamscan`, and version `ClamAV 1.4.3`. The launcher is required because Replit's Nix isolation hides the package path from direct Node filesystem inspection; it accepts only the fixed version or stdin-scan operation, resolves the declared scanner by name through the governed Replit runtime PATH, and verifies the exact store identity plus version before forwarding. The production startup adapter still hashes the direct launcher before and after every operation. Exact Replit Shell startup verification passes with `SCANNER_LAUNCHER_PASS`; deployment promotion and automatic evidence backfill remain pending. Existing ticket metadata remains reviewable while attachment bytes stay quarantined. The scan worker alone may promote exact bytes to clean and trigger regenerated packages; manual database promotion is prohibited.
