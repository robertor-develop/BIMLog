...
- Feedback evidence no longer defaults to a process-local production directory. Production startup now requires an explicitly selected durable-filesystem backend, sanitized backend identity, and a matching root custody manifest; startup health checks exact-byte/atomic/shared-root/restart capabilities without exposing the root. Stored database values are opaque keys rather than host paths. Local disk fixtures remain development/test-only, and scanner/transcription fixtures cannot activate in production.
- The Feedback schema migration now takes a transaction-scoped PostgreSQL advisory lock before canonical additive DDL, serializing concurrent application startup owners. The focused storage contract passes 12/12, including reopen persistence, exact bytes, multi-writer opaque identity, held-delete refusal, backup/restore metadata, and production default denial.
