...
- The exact frozen local source-and-harness candidate is `c7a945acb3bb26b263be36cd8107a8d78ed0cfcf` with product correction parent `298585c62d6f8521449e0467286d33ace1cd67ca` and release identity `v1.60.35.09-F`. It extends the existing authenticated Feedback widget and route rather than creating another feedback surface. Implemented source generates a bounded ZIP containing a canonical JSON manifest, a human-readable PDF, and only scanner-clean hash-verified evidence; exposes a reason-audited PostgreSQL follow-up CSV register; records immutable submission acknowledgments, reviewer notices, staff responses/decisions/fixes/answers, in-app delivery/read state, and explicitly opted-in provider-backed email copies; and preserves original/marked screenshot linkage from the browser through the server package contract. The final commit changes only the production-component browser evidence harness for that linked package workflow.
- Focused behavior proof passes for package manifest/PDF/ZIP integrity and redaction, bounded evidence inclusion, atomic original/marked screenshot roles, follow-up register routing, submission/reviewer/customer notification separation, idempotent response delivery, opt-in email, provider default-deny, and HTML escaping. These are local source and deterministic behavior tests, not production receiver, provider, email-delivery, publish, or customer evidence.
