...
- Commit this reconciliation successor, rerun the complete pre-push production gate, and atomically advance GitHub `main`, `master`, and the release branch. Align the preserved Replit publish-wrapper descendant to that exact pushed successor through Replit Shell, require the complete Replit build, and publish only after the build is green.
