...
- Publication requires an operator-provisioned persistent Feedback storage root and `.bimlog-feedback-storage.json` identity manifest whose backend ID matches `BIMLOG_FEEDBACK_STORAGE_BACKEND_ID`; no provider/root has been activated by this candidate. Backup, restore, and multi-instance durability remain an external environment acceptance gate even though the provider-neutral adapter contract is locally proven.
...
- The new concurrent-startup advisory-lock assertion must be rerun in the isolated loopback PostgreSQL harness before this superseding candidate is accepted; this session had no `BIMLOG_FEEDBACK_TEST_DATABASE_URL` binding and did not invent one.
