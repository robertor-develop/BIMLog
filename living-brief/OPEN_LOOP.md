...
- Complete exact-head workspace build and controlled Chromium proof for product commit `854d9c16f0e2f8d3177e84ecffcd1dd2ebad5960` after Living Brief reconciliation. Browser proof must cover legal status options and visible save result, claim ownership, synchronized top scrolling and sticky header/actions, complete review drawer, PDF/Word/Excel/JSON/ZIP/master-register actions, and notification-panel resize/collapse/restore on desktop plus mobile containment.
...
- In Replit, bind production ClamAV only from the installed absolute executable, exact SHA-256, and exact version reported by the visible Shell. Require startup health, an exact clean/infected/failure scan receipt, and automatic backfill of existing quarantined evidence before claiming attachments are available in PDF/Word/ZIP. Never mark attachment rows clean manually.
