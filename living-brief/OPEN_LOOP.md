...
- [x] Preserve truthful liveness/readiness separation on valid cold starts. The package now binds through a small `dist/index.cjs` bootstrap, defers the heavy graph to `dist/app.cjs`, answers `/api` as starting, and keeps `/api/v1/healthz` at 503 until the application barrier completes. Do not raise the existing timing budgets; repeat the complete clean gate before push.
