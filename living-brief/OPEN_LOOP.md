...
- [ ] Repeat the clean exact-head full pre-push gate after the Windows artifact-test cold-start correction. The prior committed full run passed secret, schema, fixture, brief, typechecks and builds but its negative child timed out before any output/TCP. The test-only guard-entry marker now starts the original eight-second denial budget at JS entry; focused proof passed with natural invalid-authority exit and no TCP/readiness. Independent review remains open.
