...
- [ ] Complete the Build 010 publication correction: rerun the exact full gate after moving the pnpm 11.17.0 pin to `engines.pnpm`, push the corrective exact head to both remote branches, repeat Replit publication without Agents, and require authenticated visible-Chrome smoke before starting Build 011. The failed provider bootstrap changed neither production nor the database.
