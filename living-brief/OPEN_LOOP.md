...
- A BIMLog record that contains issue metadata but no visual-state package must show the control as unavailable with a plain explanation. It must not issue the former native lookup request or surface the former `409 identity_not_found` error.
- Production inspection on 2026-08-24 showed the current ELARA EAST list contains 20 BIMLog issue records; the visible records report no thumbnail, and inspected legacy records do not advertise a BIMLog visual-state package. Those records cannot reconstruct camera, selection, visibility, appearance, sectioning, or redline state from BIMLog because that data is absent. Do not fabricate it from metadata.
- New viewpoint synchronization must persist the complete visual-state package in BIMLog so later Working View opens remain BIMLog-source and Navisworks apply-only. Existing metadata-only records require a separately authorized data-correction decision; user-click migration is retired.
- Push, synchronize, publish, then field-verify one BIMLog record that has a complete package reconstructs its temporary Working View and one metadata-only record remains visibly unavailable without a 409 or local lookup. Publication and field verification remain separate gates.
