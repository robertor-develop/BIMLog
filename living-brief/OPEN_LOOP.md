...
- Replace the predecessor local-Saved-Viewpoint-only acceptance with BIMLog-source reconstruction acceptance: run the reconciled full build, push/synchronize/publish, migrate incomplete legacy records into governed BIMLog visual-state custody outside the user click path, and prove **Open Working View** reads the authoritative BIMLog package and reconstructs the temporary Navisworks view with matching camera, selection, visibility, appearance, sectioning/redline completeness, immutable identity, digest, and model fingerprint. Prove the user action performs no local Saved Viewpoint lookup, capture, backfill, Saved Viewpoint write, or model-file write.
- Do not accept or reintroduce a click-time Navisworks-to-BIMLog capture path. Any legacy migration must be separately governed, auditable, and complete before the Working View control becomes available for that record.
