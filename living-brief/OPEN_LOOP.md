...
- Publication requires an operator-provisioned persistent Feedback storage root plus `BIMLOG_FEEDBACK_STORAGE_AUTHORITY_MANIFEST` stored outside that root. The authority manifest must pin the same root and backend ID, require backup, and declare only locally proven capabilities; no provider/root has been activated by this candidate. Backup, restore, and multi-instance durability remain an external environment acceptance gate even though cross-process and exact-byte restore fixtures pass locally.
...
- The concurrent-startup advisory-lock assertion passed in the sanctioned isolated loopback PostgreSQL harness at source commit `c8218d58db0daa2f737862f22324a495f5f1e5cc`; this local receipt does not authorize production migration or deployment.
