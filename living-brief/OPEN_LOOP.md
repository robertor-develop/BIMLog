...
- Preserve exact frozen local source-and-harness head `3736433a7a040461a979e92e5621a3728c658d7f`. The PDF/JSON/ZIP package, reason-audited PostgreSQL follow-up register, submission/reviewer/customer notifications, six-field linked capture upload, durable email sent/failed/skipped audit with retry until sent, visual original/marked screenshot workflow, deterministic PDF proof-output helper, and production-component browser evidence coverage are implemented and locally tested; they are not pushed, synchronized to Replit, published, production-migrated, receiver/provider activated, live verified, or customer accepted.
- Complete independent artifact inspection on representative internal and customer packages: open the ZIP, parse the canonical JSON manifest, render and visually inspect every PDF page, verify customer redaction and secure links, and confirm only scanner-clean exact-hash evidence is embedded. Current deterministic tests prove structure and integrity behavior but do not replace final artifact-family inspection.
- Complete authenticated disposable-PostgreSQL API/browser acceptance for the follow-up CSV register, submission receipt, reviewer notifications, staff response types, in-app read state, idempotent replay/conflict behavior, customer-visibility authorization, and email-copy default-deny/explicit-opt-in behavior. Do not claim an email was delivered without a real approved-provider receipt.
...
- Complete exact-head build and independent acceptance for frozen candidate `3736433a7a040461a979e92e5621a3728c658d7f`, which preserves `.replit` convergence commit `5fa2033e03161ecfdca17304c580c58b6efaf5c7` in its ancestry, then atomically push `main` and `master`, synchronize Replit, and verify `git status --porcelain` is empty before another deployment attempt.
