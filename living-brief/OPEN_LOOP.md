...
# Feedback operations v1.60.35.10-F - Remaining Acceptance Gates

- Preserve exact local product/browser-harness source `96fa5fb12bf43c57ffb45cd5ab402c3424e8b891`. It is not pushed, synchronized, published, deployed, production-migrated, or customer-accepted. Do not represent the generated Living Brief reconciliation commit as product or runtime evidence.
- Run the current controlled browser harness against the exact final source and retain commit/tree-bound English desktop and Spanish exact-mobile evidence for queue loading, stable-ID selection, owner claim, legal status transitions, customer response, package/register actions, reviewer-alert state, per-artifact Telegram outcomes, safe scan-failure visibility, non-super-admin denial, and zero browser/network exceptions. Harness source presence is not browser execution.
- Generate representative current internal and customer PDF, DOCX, XLSX, JSON, and ZIP outputs from accepted exact-head fixtures; reopen and visually inspect PDF/Word/workbook output; verify current evidence metadata, bounded clean-image previews, PDF/OOXML hyperlink targets, customer redaction, custody disclosure, archive inventory, and absence of raw object keys. Existing predecessor documents do not prove `v1.60.35.10-F`.
- Run exact-head disposable-PostgreSQL behavior for durable submission/outbox commit, immediate failure and periodic reconciliation, no-reviewer escalation then recovery, idempotent claim/status/customer-response audit, current-snapshot package lineage, and Telegram aggregation across both required artifacts and multiple opted-in reviewers. Prove mixed and reverse-ordered outcomes never collapse to `sent`, scanner failure becomes reviewer-visible without customer leakage, and cleanup leaves zero fixture residue. No database execution is claimed by this documentation reconciliation.
- Complete the exact clean full build, production artifact closure, startup fail-closed checks, API/UI typechecks, database safety, tracked-secret, mojibake, Living Brief integrity, and clean post-build diff. Preserve a hash-bound acceptance receipt for the exact final head; prior receipts remain predecessor evidence.
- Keep Telegram default-deny. A live document send requires existing approved Telegram configuration, an actively linked super-admin, exact DOCX/XLSX byte/hash authority, and idempotent provider acknowledgments. Otherwise retain durable skipped/not-configured or no-opted-in-reviewer receipts without inventing credentials or claiming delivery.
- Keep evidence quarantined until governed production ClamAV startup plus clean/infected/failure receipts pass and automatic backfill is verified. Keep `F:\BIMLog\Feedback` receiver projection unmounted until its separate reachable TLS, identity, signature, readback, backup/restore, monitoring, and retention gates are accepted.
- Only after exact-head evidence and independent acceptance, perform separately authorized normal push, remote equality verification, clean Replit synchronization, non-destructive preview, one authorized publication, and bounded live verification. None of those external actions occurred here.
