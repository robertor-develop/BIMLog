...
- A BIMLog record that does not yet contain a visual-state package must not trigger click-time guessing or the former `409 identity_not_found` lookup. Historical recovery is a separate one-time Original Lens migration against preserved BIMLog-managed Saved Viewpoints.
- Install Original Lens v1.60.36, open the correct project model, and run **Migrate Existing BIMLog Viewpoint Visuals**. Verify its counts for platform rows, managed local views, exact matches, uploaded/already-current packages, ambiguous records, unmatched records, and failures. The action must not rename, move, reorder, create, or delete Saved Viewpoints and must not save the model.
- Investigate every ambiguous, unmatched, or failed record separately. Never substitute by trade, company, floor, title similarity, or model search. A record is accepted only after its exact Original Lens Saved Viewpoint has supplied the platform package.
- After migration, publish the platform correction and field-verify representative historical records reconstruct camera, selection, visibility, appearance, sectioning, and supported redlines from BIMLog through Lens Next. Installation, migration, publication, and field verification remain separate explicit gates.
