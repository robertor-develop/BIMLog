...
## Lens Next v1.0.08-Pro stale-identity acceptance gate

- Replace the predecessor local-Saved-Viewpoint-only acceptance with platform reconstruction acceptance for implementation unit `47303fe9b6cf5af6a5a6814b01e08980d5e48d0e`: run the reconciled full build, push/synchronize/publish, allow startup to add the nullable visual-state columns, and verify an authorized exact local viewpoint performs one successful capture/backfill. Then remove or rename that local Saved Viewpoint and prove a later **Open Working View** reconstructs the temporary Navisworks view from BIMLog with matching camera, selection, visibility, appearance, sectioning/redline completeness, immutable identity, digest, and model fingerprint. Prove no Saved Viewpoint or model-file write occurs.
- The existing `1185RI-98AA6A` / `EL-001` record cannot be reconstructed until a trustworthy source viewpoint is found because the current platform row predates visual-state custody and the active model has no exact local match. Do not fabricate camera/model state from issue metadata or a screenshot. Backfill from an authoritative source NWD/viewpoint if available, or explicitly retire/replace the stale record under the existing lifecycle workflow.
...
- Restart Navisworks Manage 2021 so it loads installed native SHA-256 `AA550DB157A6183ED1DA098B88399376F1743E0E98F7CDC8358C9F2441E5B5C5`. Against the governed `1185 RIVER AV MODEL-06-11-26.nwd`, select the legacy issue that previously returned `409 identity_not_found` and verify **Open Working View** opens exactly one existing Saved Viewpoint without creating, editing, or saving model content. This real field action is the remaining acceptance gate.
