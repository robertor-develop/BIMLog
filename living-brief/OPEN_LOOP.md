...
I010 in-progress role presentation repair: Team filters, badges, pending invitations, editor and configured-role guide now use the same configuration identities as invitation creation. Historical membership roles remain searchable/displayed but are not offered as new assignments; failed edits keep the persisted role visible. No server permission authority changed. Frontend typecheck and isolated invitation/role-choice regression suite pass; local actual-component Chrome proves bilingual configured-role consistency, historical filtering and rejected-save preservation. Screenshot capture timed out, so visual QA is not PASS. Changes remain uncommitted/unpublished pending full release verification. Nine-project company-scope correction still awaits the outstanding exact action-time confirmation; approved email sender remains unconfigured. No block-complete or live acceptance claim.

I009 candidate: transactional invitation acceptance now creates an atomic existing-inbox notice; rollback, duplicate/replay, concurrent resend, preserved existing roles and conflicting-company tests pass in isolated PostgreSQL. Publish/retest still required. I008-R1 was published and its exact failed role-form test passed on42d06df0; supersedes the pending repair statements below. Actual email remains blocked by absent approved sender configuration. I010 operational binding correction, real recipient acceptance and full role-based live smoke remain open. Team's legacy permission legend also differs from configured role choices and remains a real UX defect to reconcile without changing permissions.
...
- [x] Expose current-actor review eligibility without changing approval authority; make released-version definitions inspectable; show relevant APU prerequisite failures clearly. Eight eligibility combinations and reader-only published-version HTTP boundaries pass. Twenty local Chrome fixture scenarios pass; live deployment verification remains separate.
...
- [x] Implement Builds 21–25 disconnected job/worker foundation: scoped enqueue, idempotency, lease/fence, immutable audit and execution-time revalidation. No production route or scheduler invokes the worker, and no live SharePoint delivery is claimed.
- [ ] Before enabling file publishing, prove transactional job/lease/settlement on isolated real PostgreSQL, reconcile provider file-exists after uncertain completion without overwrite, add explicit authorized user confirmation and scoped status/retry UI, then complete a real tenant/site round-trip.
...
- [ ] Live-test the published Analytics follow-up at exact source `f476c84a1dc0fb330d87d03e384072a215657124`: explicit cross-company Super Administrator read reason for both screen and PDF export, bound to the current account and project. The ordinary BIMCorp project Analytics screen loaded normally; no cross-company Analytics pass is claimed yet.
- [ ] Release and live-retest the focused Operations EDT prerequisite guidance candidate. Before activation, `EDT_SOURCE_NOT_ACTIVATED` now explains in English and Spanish that Intake approvals/activation must come first; the server denial remains intact. Focused behavior and typecheck pass, but publication and Chrome proof remain open.
...
- [x] Builds 6–10 Intake correction passed the full local gate, reached GitHub `master` at `f93cc844f00e7e22ddd18dba33d640fcc2ff88ee`, and was published through Replit Shell. Authenticated Chrome proved Quick/Advanced classification, client/floor guidance, save/reload, and synthetic project 56 activation with a frozen built-in Sleeve workflow; this is not company-template acceptance. Evidence: `F:/BIMLog/TestProof/template-gap-block10-release-20260924.md`.
- [ ] Complete and field-prove company Delivery Workflow and Governance authoring, independent approval, publication, activation, versioning, supersession and audit. Builds 11–45 reached GitHub; Build 40 was published at `6aaea1b4c0439198ae72b636566818083a107c23`, and Builds 41–45 are pushed but not published. Authenticated Chrome proved a synthetic workflow draft save/reopen and correctly blocked self-approval in both workflow and Governance editors, not six-role acceptance. Builds 46–50 are a local candidate for the narrow Operations Director administration screen and real isolated-database grant/revoke proof. Their full gate, push, ten-build publication and live smoke remain separate. Approval thresholds, company-role policy rows and other change rules remain intent rather than full execution authority; guarded EDT mutations remain closed.
...
- [x] Complete Block 13 full clean gate and exact-head GitHub push at `c137fe608c7685c79205196f9beedce6f29c5339`. This five-build-only block was not published; the next publication and full authenticated Chrome smoke occur after the following five builds. No Lens Next Native/installer change occurred, so focused Navisworks smoke is not triggered.
...
- [x] Build 210 passed the exact-head full gate, reached both GitHub refs at `98eeb3da15b497982a73cf13014e187fd4303ca1`, published through Replit Shell without Replit Agents as receipt `d4ff8790`, and passed exact-identity authenticated multi-role Chrome smoke.
...
- [x] Deterministic Platform inventory commit `80c377031960605cff6e898143f89be5fb273e83` records the new regression file; no product, database, Native, installer, or provider behavior changed.
- [x] Build 220 passed the exact complete gate, reached both authoritative refs at `bfcba657f73ed0195957aa2ed6902c9358b74cce`, published through Replit Shell without Replit Agents as receipt `3a6db4ae` with `schemaAction=NONE`, and passed exact-identity authenticated visible-Chrome dashboard, project, reports, control-center, governance, Lens Next, reload, and multi-tab smoke. Live release `v1.05.N18-P36` exposes package `bimlog-bfcba657f73ed019-6da3bfa4ffbd1afd` with bound identity.
- [x] The corrected P36 package is installed in physical Navisworks 2021. Active discovery contains exactly the verified Pulse-only `BIMLog.bundle` and `BIMLogLensNext2021.bundle`; the retired direct-load `BIMLogNavisPlugin` path and Original/Legacy Lens are absent, Pulse remains available, the complete WebView2 tree is installed, and Lens Next starts its authenticated workspace and loopback bridge without changing Navisworks licensing.
...
## Current open-loop authority — post-120 Block 32 — 2026-09-20
...
- [ ] Define and implement the missing runtime interpretation of approval thresholds, change-control rules, and company-role matrix. Do not treat stored policy rows as permissions or claim 100% mockup behavior; retain existing PMO/Finance and assigned task-role enforcement until the governed role mapping and actions are specified and tested.
...
- [ ] Block 2: versioned company Delivery Workflow allocation persistence, canonical Commercial APU resolution, PMO/Finance authority, and maker-checker audit. Focused tests and an authenticated isolated PostgreSQL cycle now pass for published-APU source, exact amounts, PMO/tenant denial, phase identity, separate Finance checker, stale revision/source, save/reload, immutable publication and audit. Exact production-artifact proof and independent review remain before acceptance. Published Commercial APU v1 records without phase defaults must fail closed, not be silently upgraded.
- [ ] Block 3: the existing Commercial APU editor now provides optional Direct Production nodes/phase defaults; the existing Delivery Workflow editor selects a published APU, explicitly aligns phases, edits all four proposals, previews server amounts and guards unsaved/stale-preview states. Frontend typecheck and encoding gate pass. Local Chrome production-component fixtures pass 24 pricing and 20 workflow scenarios across desktop/exact-390, English/Spanish, save/refresh/reopen, PMO/read-only/denied/empty and missing-APU states. Inspected screenshot defects were corrected and retested. The clean-source full workspace/production-assembly build passed at a4258c624dd9bddce28c86f79aa7f3b684fcfc83. Accessibility completion, real authenticated account/browser acceptance, exact mockup parity, isolated production-artifact proof and independent review remain open before acceptance.
- [ ] Block 4 local source candidate: company-scoped Workflow Governance Policy contract, versioned storage, threshold/role/validation rules, PMO draft authority, separate company Finance checker, stale revision, immutable approved versions and audit, publish/supersede/retire lifecycle pass focused tests and guarded authenticated localhost HTTP proof. The configured permissions are policy data, not an authorization bypass; project budget governance remains separate. Independent review, exact production-artifact proof, live customer roles, and release gates remain open before acceptance.
- [ ] Block 5 local candidate: bilingual Governance Policy dashboard, scoped policy binding at new Work Item activation, immutable version/fingerprint snapshot and Operations display, overlap rejection, and guarded localhost API/runtime tests pass. Complete browser usability/accessibility, real PMO/Finance account permissions, broader rule enforcement, independent review and clean-source production artifact proof before acceptance. No historical Work Item is backfilled.
...
- [x] Re-test the corrected runtime assembler: the exact package graph, hashes and closure validated in 496595 ms, below the unchanged 600-second bound.
- [ ] Resolve the repeatable exact-artifact Windows startup miss (11.0/11.2 seconds against 8.0 seconds), then rerun the full pre-push gate and protected Lens Next behaviors; independently review the exact diff and commit. Do not simply increase or bypass the readiness budget.
...
- [x] Replace whole-scope financial reuse for Package and Team Member selections with exact task-linked assignment attribution, including their intersection; do not allocate unlinked assignments by assumption.
...
- Build 32 repairs the accepted create and Open Working View regressions in a Platform-only `v1.05.N12-P09` candidate. Before promotion, retain atomic no-partial creation, project-wide active display uniqueness, exact digest verification, placeholder-only navigation rebind, and all project/viewpoint/lifecycle/revision protections; deployment and Roberto acceptance remain pending.
...
- [x] Corrective Build 7: prove saved APU version, rate, and contract binding across normalization/reload and expose the exact saved binding.
...
- [x] Add a server-side mapped-spreadsheet pricing regression: a $6,000 saved plan must preserve an existing $75 unit rate, leave a newly imported item's rate at zero, and bind only a sole same-currency version. The test runs in the Intake suite; publication and live import acceptance remain open.
...
- [ ] Block 2, Builds 6–10: prototype one exact clash's bounded 3D neighborhood—not just two elements and not the whole model—with spatial/relevance selection, clipping of long elements, stable IDs/transforms, local-only package size/rights controls, 2021/2025 proof, and interactive performance measurements. Gate Blocks 3–4 on its result.
...
- [x] Block 5, Builds 21–25 local source/component-fixture gate: responsible-company, report-type, capture-date and image-availability filters compose with existing issue filters; compact disclosures and outside-filter selection guidance pass focused tests, local build and component-harness Chrome interactions. This is not live-data or published acceptance.
- [x] Block 6, Builds 26–30 local source/component-fixture gate: selected-issue Overview, BIMLog Issue, Properties and Activity views preserve existing governed actions; real capture URL, absent/broken image messaging, truthful unavailable clash metadata, English/Spanish labels and exact-390 containment were exercised locally. Chrome screenshot capture timed out, so visual pixel review, authenticated data and field acceptance remain open.
- [x] Block 7, Builds 31–35 local source/component-fixture gate: issue-record status/evidence summary with active-versus-filtered denominator, responsive Spanish/English table company/report/image-reference columns, and filter recomputation passed focused and local Chrome checks. Image reference is not successful image delivery. Pixel-level, live-data and Native acceptance remain open.
- [x] Block 8, Builds 36–40 local source/component-fixture gate: sync inventory versus filtered-plan scope labels, deterministic plan-category counts and review filters, exact server/local-only row labels, mixed/empty-plan tests, desktop/mobile Chrome component checks. No Native commands, actual reconciliation, or live data were exercised; screenshot capture timed out.
- [x] Block 9, Builds 41–45 local source/component-fixture gate: keyed image loading/success/error states, honest thumbnail and selected-capture labels, explicit retry, bounded lazy/async thumbnail requests, 10k synthetic calculation characterization, desktop and exact-390 Chrome image-state checks. This is not real clash imagery, customer-image delivery or live performance acceptance.
- [ ] Block 10, Builds 46–50: Builds 46–49 have a local integration candidate. The Build 50 short-window UX continuation shows multiple issue cards beside selected details, keeps expanded Model tools and the list reachable by left-pane scrolling, passes 37/37 Lens Next behavior files, typecheck, bundle and exact-390 containment. Real project/model-bound clash linkage, bounded 3D imagery, Native 2021/2025 field runs, authenticated/live Chrome and customer acceptance remain open. No screenshot, synthetic image, title similarity or fixture may close those gates. Blocks 5–10 are local candidates, not a published accepted release; no Block 11 is approved in this roadmap.
...
- [x] Builds 006-010: triage and correct production dependency advisories, lock supply-chain provenance, run the full security regression, push Build 010, publish through Replit, and complete authenticated visible-Chrome smoke.
...
- [x] Build 010: pass the exact clean pre-push gate with dependency provenance/audit first, push the exact head, publish once through Replit Shell without Replit Agents, and capture the full authenticated visible-Chrome production receipt.
- [x] Complete clean local release gate passed through source commit `4472b222215cbb5f4d8173abcfa0a0b37e883d3f`; Block 02 evidence and ledger are committed at `385731167397bc6204a6c43e358e675a9331e09f`. Repeat once on the reconciled evidence-bearing head before push.
...
- [x] Preserve truthful liveness/readiness separation on valid cold starts. The package now binds through a small `dist/index.cjs` bootstrap, defers the heavy graph to native-ESM `dist/app.mjs`, supplies an ESM-local `createRequire` bridge for bundled CommonJS dependencies, answers `/api` as starting, and keeps `/api/v1/healthz` at 503 until the application barrier completes. Do not raise the existing timing budgets; repeat the complete clean gate before push.
- [x] Complete the Build 010 publication correction: rerun the exact full gate with governed local/CI pnpm 11.17.0 and Replit constrained to `>=10.26.1 <12`, push the corrective exact head to both remote branches, repeat Replit publication without Agents, and require authenticated visible-Chrome smoke before starting Build 011. Both failed provider builds changed neither production nor the database.
- [x] Block 02 ultimately published and passed authenticated Chrome at source `fdb2be13622aa0f8ed2fc1e4f60792a7bfbcc7d0`, source tree `dc3252c270d8449e21ebf8a444cb8ee6256a54df`, and Replit receipt `e89dc3b4`; the earlier correction entries remain historical evidence, not current blockers.
- [x] Block 03 Builds 011–015: run the exact full gate and focused Navisworks 2021/2025 package/test smoke, then push the clean head normally. No install or publication is due at Build 015.
- [x] Builds 016–019: exact schema receipt, repeatable Windows fixture, zero-pending migration preview, backup/restore equality, startup serialization, and two restored-database artifact readiness cycles pass without production mutation.
- [x] Build 020: pass the exact clean full gate, push the block head, run the read-only publication database operator in Replit Shell, publish once with development-data copy off, and pass exact-identity authenticated visible-Chrome smoke plus restart/session restoration. Replit Agents remain prohibited.
- [x] Builds 021–025: reconcile current release truth, classify every unchecked loop, align executable documentation, establish Lens Next as the sole supported Lens product, pass the exact gate, and push Block 05.
- [x] Builds 026–030: classify all current worktrees and branches, map product-default/Next-200 and dirty candidate behavior, define non-executable recoverable cleanup plans, and integrate only current source-authority tooling. No stale product patch was imported.
...
## Live template/Intake acceptance — 2026-09-24

- [ ] Production `v1.05.N18-P36` Quick Intake autosave fails when a company-master discipline is selected. The authoritative validation query unions integer enterprise IDs with text company-catalog IDs; production read-only SQL reproduces `UNION types integer and text cannot be matched`. Cast the enterprise selected ID to text, pass local and provider release gates, publish, then repeat the exact live Chrome Intake path before calling this fixed. Internal partial evidence: `evidence/template-intake-acceptance-20260924/LIVE_ACCEPTANCE_HOLD.md`.
- [ ] Two separate company rows are named `BIMTECH CORP`; Rubén's account belongs to company ID 31 and Lorena's to company ID 35. Neither tenant currently has company Delivery Workflow, Governance, or APU/Pricing versions. Resolve the intended company authority and create/approve actual production templates from Rubén's confirmed definitions; do not relabel synthetic QA drafts as approved templates.
- [ ] Complete BIMCorp synthetic maker/checker, pricing publication, Intake/budget/EDT/Work Item/roles/hours/QC/reporting live acceptance. The separate reviewer account exists; its independent company-scoped PMO approval authority remains to be granted and tested. Hold external smoke-test DOCX, definitive Spanish guide, and Telegram delivery until the user-specified acceptance gates pass.

## BIMTECH identity and template repair — 2026-09-25

- [x] Preserve a pre-mutation production backup, reconcile Rubén and Lorena to canonical BIMTECH company ID 31, retain company ID 35 as a historical alias, and preserve append-only company/project binding history.
