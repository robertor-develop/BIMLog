...
- Preserve exact frozen local source-and-harness head `c7a945acb3bb26b263be36cd8107a8d78ed0cfcf` with product correction parent `298585c62d6f8521449e0467286d33ace1cd67ca`. The PDF/JSON/ZIP package, reason-audited PostgreSQL follow-up register, submission/reviewer/customer notifications, opt-in email-copy control, visual original/marked screenshot workflow, linked capture-role upload, and production-component browser evidence coverage are implemented and locally tested; they are not pushed, synchronized to Replit, published, production-migrated, receiver/provider activated, live verified, or customer accepted.
...
- Complete exact-head build and independent acceptance for frozen candidate `c7a945acb3bb26b263be36cd8107a8d78ed0cfcf`, which preserves `.replit` convergence commit `5fa2033e03161ecfdca17304c580c58b6efaf5c7` in its ancestry, then atomically push `main` and `master`, synchronize Replit, and verify `git status --porcelain` is empty before another deployment attempt.
