...
- After that authority preflight, `dist/index.cjs` binds the liveness listener before dynamically loading the native-ESM `dist/app.mjs` bundle. `/api` may report only that startup is alive; `/api/v1/healthz` must remain 503 until the application startup barrier completes. Do not recombine the bundles, restore a CommonJS wrapper around the deferred graph, or relax the existing cold-start budgets to hide a regression.
