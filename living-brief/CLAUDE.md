...
- The deterministic runtime assembler permits only three explicit out-of-range security edges: `uuid` declarations `^8.3.0` and `^9.0.1` may resolve to exact patched `11.1.1`, and the legacy storage helper's `form-data` declaration `^2.5.5` may resolve to exact patched `4.0.6`. Each requires the matching root lockfile override, issuer edge, exact snapshot, integrity binding, and passing production audit. Every other declared-spec mismatch remains fail closed.
