...
- The deterministic runtime assembler permits only two explicit out-of-range security edges: `uuid` declarations `^8.3.0` and `^9.0.1` may resolve to exact patched `11.1.1`, and only while the root lockfile override, issuer edge, exact snapshot, integrity binding, and production audit all agree. Every other declared-spec mismatch remains fail closed.
