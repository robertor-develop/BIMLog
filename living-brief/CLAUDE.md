...
- After that authority preflight, `dist/index.cjs` binds the liveness listener before dynamically loading the native-ESM `dist/app.mjs` bundle. The generated ESM banner provides `createRequire(import.meta.url)` solely for bundled CommonJS dependencies that require Node built-ins. `/api` may report only that startup is alive; `/api/v1/healthz` must remain 503 until the application startup barrier completes. Do not recombine the bundles, restore a CommonJS wrapper around the deferred graph, or relax the existing cold-start budgets to hide a regression.
- Every push candidate must run `check:dependency-provenance` and `audit:production` before the existing secret, database, build, and production-artifact checks. The governed local/CI toolchain is exact `pnpm@11.17.0`; Replit publication is allowed only through the explicit compatible engine range `>=10.26.1 <12` because its supported builder is currently pnpm 10.26.1. The `packageManager` field is prohibited because Replit incorrectly turns it into a failing package-manager self-install. Lockfile v9, the one-day minimum package-release age, exact native install-script allowlists, no npm/yarn lockfiles, integrity-bound patched production resolutions, and zero-advisory audit remain mandatory on both paths. Do not weaken or omit these gates to publish.
- The deterministic runtime assembler permits only three explicit out-of-range security edges: `exceljs>uuid` declaration `^8.3.0`, `teeny-request>uuid` declaration `^9.0.0`, and `gaxios>uuid` declaration `^9.0.1` may resolve to exact patched `11.1.1`. Each requires its matching issuer-scoped lockfile override, issuer edge, exact snapshot, integrity binding, and passing production audit. `form-data` is patched only on the semver-compatible `axios>form-data` edge. Every other declared-spec mismatch remains fail closed.
- Issuer-scoped override verification must extract the package identity before its version and peer suffix. Peer-qualified lock keys and scoped package names must not be confused with peer-version suffixes; regression proof must cover this parser before publication.
- Runtime-closure fixtures must contain the same three packaged entry artifacts as production: `dist/start.cjs`, `dist/index.cjs`, and `dist/app.mjs`; a legacy index-only fixture is not valid proof.
- A runtime-closure fixture may narrow the production-required package list only when it supplies a non-canonical isolated workspace root. The canonical production workspace rejects that override.
- Each five-build block must commit its build reports and machine-readable ledger state before the exact-head push gate. Provider publication and live-smoke receipts stay external to the immutable source candidate so recording them cannot silently change the deployed commit.
- Combined release identity is governed by `contracts/release-identity.json`; generated TypeScript, MSBuild and C# identity files must pass `check:release-identity`. Public health may expose only safe immutable identity fields; full release diagnostics require authenticated Super Administrator authority. Publication receipts must bind exact source/tree, asset manifest, database migration level, provider IDs and matching live identity through the release-receipt contract.
- Before each Replit publication milestone, generate the read-only publication database receipt against development and production. Publish with `schemaAction=NONE` only when both match the exact source contract; catalog-proven PostgreSQL constraint indexes are classified separately from unexplained drift.
- Any real schema difference requires the existing complete provider preview, additive inventory, verified backup/restore point, and pre/post record-count proof. Development-to-production data copy remains off, Replit Shell is the provider execution path, and Replit Agents remain prohibited.
