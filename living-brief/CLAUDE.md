...
- Every push candidate must run `check:dependency-provenance` and `audit:production` before the existing secret, database, build, and production-artifact checks. The workspace is bound to `pnpm@11.17.0` through the exact `engines.pnpm` contract (the `packageManager` field is prohibited because Replit incorrectly turns it into a failing package-manager self-install), lockfile v9, a one-day minimum package-release age, exact native install-script allowlists, no npm/yarn lockfiles, and integrity-bound patched production resolutions. Do not weaken or omit these gates to publish.
