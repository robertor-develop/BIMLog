...
const packageBuilder=read("plugins/BIMLogLensNext/native/Build-LensNextReadOnlyPackages.ps1");
...
assert.match(packageBuilder,/Assert-NoReparseTraversal \$output \$allowedRoot/g);
assert.match(packageBuilder,/FileAttributes\]::ReparsePoint/);
assert.match(packageBuilder,/package output may not traverse a reparse point/);
console.log(JSON.stringify({status:"PASS",checks:21,loadableSource:true,readOnly:true,installTargetChosen:false,legacyIo:false,reparseTraversalRefused:true}));
