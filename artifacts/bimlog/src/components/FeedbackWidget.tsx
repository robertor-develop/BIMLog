...
export function FeedbackWidget() {
  ...
  const [mine, setMine] = useState<Array<{ id:number;stableId:string;message:string;status:string;version:number;dispositionReason?:string;targetRelease?:string;relayState?:string|null;transcription?:{id:number;state:string;result?:string|null;reviewState:string}|null }>>([]);
  ...
  function fileGroup(file:File):"audio"|"screenshot"|"attachment"{if(file.type.startsWith("audio/")||/\.(webm|ogg|wav|m4a)$/i.test(file.name))return "audio";if(file.name.startsWith("feedback-capture-"))return "screenshot";return "attachment";}
  ...
  function addFiles(incoming: File[]) {
    ...
    for (const file of accepted) fileId(file);
  ...
  async function captureScreen() {
    ...
    try {
      ...
      if (!blob) throw new Error("capture"); const capture = new File([blob], `feedback-capture-${Date.now()}.png`, { type: "image/png" }); addFiles([capture]); setTransformations(current => ({ ...current, [capture.name]: { origin: "browser-display-capture", originalWidth: canvas.width, originalHeight: canvas.height, capturedAt: new Date().toISOString() } }));
  ...
  async function cropScreenshot() {
    ...
    try {
      ...
      setTransformations(current => ({ ...current, [cropped.name]: { ...(current[source.name] || {}), sourceName: source.name, sourceSha256: originalSha256, cropPercent: crop, cropPixels: { x: sx, y: sy, width: sw, height: sh }, outputWidth: sw, outputHeight: sh, transformedAt: new Date().toISOString() } }));
  ...
  async function uploadOne(feedbackId:number,file:File){const group=fileGroup(file),clientFileId=fileId(file),uploadKey=`${idempotencyRef.current}:${clientFileId}`;setUploadResults(current=>({...current,[clientFileId]:{state:"uploading",message:tt("Uploading","Cargando")}}));const form=new FormData();form.append("files",file);form.append("kind",group);if(group!=="attachment"){const consentId=captureConsents[group];if(!consentId){setUploadResults(current=>({...current,[clientFileId]:{state:"error",message:tt("Capture consent expired; renew it before retrying this file.","El consentimiento venció; renuévelo antes de reintentar este archivo.")}}));return false;}form.append("consentId",consentId);}form.append("transformations",JSON.stringify({[uploadKey]:transformations[file.name]||{origin:group==="audio"?"browser-microphone":group==="screenshot"?"browser-display-capture":"user-file-import"}}));const uploaded=await fetch(`${API_BASE}/api/v1/feedback/${feedbackId}/assets`,{method:"POST",headers:{Authorization:`Bearer ${token}`,"Idempotency-Key":uploadKey},body:form});const payload=await uploaded.json().catch(()=>({}));if(!uploaded.ok){setUploadResults(current=>({...current,[clientFileId]:{state:"error",message:payload.error||tt("Upload failed; retry this file.","La carga falló; reintente este archivo.")}}));return false;}setUploadResults(current=>({...current,[clientFileId]:{state:"success",message:payload.replayed?tt("Identical evidence already linked","La evidencia idéntica ya estaba vinculada"):tt("Uploaded","Cargado")}}));return true;}
          ...
          >
            ...
            <div style={{ padding: 16 }}>
              ...
              {reviewing ? <section aria-label={tt("My feedback backlog", "Mi lista de comentarios")}>
                ...
                {!!reportedAssets.length && <ul aria-label={tt("Feedback attachments", "Archivos del comentario")}>{reportedAssets.map(asset => <li key={asset.id}>{asset.downloadUrl?<button type="button" onClick={()=>downloadAsset(asset)}>{asset.name}</button>:asset.name} · {stateLabel(asset.scanState)}</li>)}</ul>}
              </section> : <>
