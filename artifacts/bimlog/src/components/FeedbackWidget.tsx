...
export function FeedbackWidget() {
  ...
  const [feedbackNotifications,setFeedbackNotifications]=useState<Array<{eventId:number;feedbackId:number;stableId:string;type:string;message?:string|null;createdAt:string;delivery:{channel:string;state:string;notificationId?:number|null}}>>([]);
  const [feedbackNotificationsLoading,setFeedbackNotificationsLoading]=useState(false);
  const [feedbackNotificationError,setFeedbackNotificationError]=useState("");
  const [pendingNotificationRead,setPendingNotificationRead]=useState<number|null>(null);
  ...
  const stateLabel=(value:string)=>({new:tt("New","Nuevo"),triaged:tt("Triaged","Clasificado"),accepted:tt("Accepted","Aceptado"),in_progress:tt("In progress","En curso"),blocked:tt("Blocked","Bloqueado"),fixed:tt("Fixed","Corregido"),verified:tt("Verified","Verificado"),rejected:tt("Rejected","Rechazado"),deferred:tt("Deferred","Aplazado"),clean:tt("Approved","Aprobado"),quarantined:tt("Quarantined","En cuarentena"),pending:tt("Pending","Pendiente"),completed:tt("Completed","Completada"),queued:tt("Queued","En cola"),transferring:tt("Transferring","Transfiriendo"),"receipt-verified":tt("Receipt verified","Recibo verificado"),"cleanup-pending":tt("Cleanup pending","Limpieza pendiente"),delivered:tt("Delivered","Entregado"),held:tt("On hold","Retenido"),"manual-review":tt("Manual review","Revisión manual"),expired:tt("Expired","Vencido"),recording:tt("Recording","Grabando"),paused:tt("Paused","Pausado"),ready:tt("Ready","Listo"),idle:tt("Idle","Inactivo"),"metadata-only":tt("Metadata recorded","Metadatos registrados"),"awaiting-scan":tt("Evidence awaiting scan","Evidencia esperando análisis"),"rejected-evidence":tt("Evidence rejected","Evidencia rechazada")}[value]||tt("Unavailable","No disponible"));
  const relayReasonLabel=(value?:string|null)=>value?({"awaiting-delivery":tt("Awaiting secure delivery","Esperando entrega segura"),"delivery-in-progress":tt("Secure delivery in progress","Entrega segura en curso"),"delivery-received":tt("Delivery received","Entrega recibida"),"delivery-confirmed":tt("Delivery confirmed","Entrega confirmada"),"cleanup-in-progress":tt("Secure cleanup in progress","Limpieza segura en curso"),"support-review":tt("Support review required","Se requiere revisión de soporte"),"on-hold":tt("Delivery is on hold","La entrega está en espera"),"retention-ended":tt("Retention period ended","Finalizó el período de retención")} as Record<string,string>)[value]||tt("Status detail unavailable","Detalle de estado no disponible"):"";
  const transcriptionReasonLabel=(value?:string|null)=>value==="provider-unavailable"?tt("Transcription provider is not available yet","El proveedor de transcripción aún no está disponible"):value?tt("Transcription status detail unavailable","Detalle del estado de transcripción no disponible"):"";
  ...
  const feedbackNotificationLabel=(value:string)=>({acknowledgment:tt("Receipt acknowledged","Recepción confirmada"),response:tt("Reviewer response","Respuesta del revisor"),decision:tt("Reviewer decision","Decisión del revisor"),fix:tt("Resolution update","Actualización de resolución"),answer:tt("Reviewer answer","Respuesta del revisor")}[value]||tt("Feedback update","Actualización del comentario"));
  const notificationDeliveryLabel=(value:string)=>({created:tt("Unread","No leída"),read:tt("Read","Leída"),unavailable:tt("Delivery record unavailable","Registro de entrega no disponible")}[value]||tt("Delivery state unavailable","Estado de entrega no disponible"));
  ...
  function clearReviewScope() { setMine([]); setHistory([]); setReportedAssets([]); setFeedbackNotifications([]); setFeedbackNotificationError(""); setSelectedFeedbackId(null);setReviewing(false); }
  ...
  async function loadFeedbackNotifications(){setFeedbackNotificationsLoading(true);setFeedbackNotificationError("");try{const response=await fetch(`${API_BASE}/api/v1/feedback/notifications`,{headers:{Authorization:`Bearer ${token}`}});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||tt("Feedback updates could not be loaded.","No se pudieron cargar las actualizaciones."));setFeedbackNotifications(Array.isArray(data.notifications)?data.notifications:[]);}catch(cause){setFeedbackNotificationError(cause instanceof Error?cause.message:tt("Feedback updates could not be loaded.","No se pudieron cargar las actualizaciones."));}finally{setFeedbackNotificationsLoading(false);}}
  async function markFeedbackNotificationRead(eventId:number){setPendingNotificationRead(eventId);setFeedbackNotificationError("");try{const response=await fetch(`${API_BASE}/api/v1/feedback/notifications/${eventId}/read`,{method:"POST",headers:{Authorization:`Bearer ${token}`}});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||tt("The update could not be marked read.","No se pudo marcar la actualización como leída."));setFeedbackNotifications(current=>current.map(item=>item.eventId===eventId?{...item,delivery:{...item.delivery,state:"read"}}:item));setSuccess(tt("Feedback update marked read.","Actualización marcada como leída."));}catch(cause){setFeedbackNotificationError(cause instanceof Error?cause.message:tt("The update could not be marked read.","No se pudo marcar la actualización como leída."));}finally{setPendingNotificationRead(null);}}
  async function loadMine() { setError(""); const response = await fetch(`${API_BASE}/api/v1/feedback/mine`, { headers: { Authorization: `Bearer ${token}` } }); const data = await response.json().catch(() => ({})); if (!response.ok) { if([401,403].includes(response.status))clearReviewScope(); setError(data.error || tt("Your feedback could not be loaded.", "No se pudieron cargar sus comentarios.")); return; } setMine(data.feedback || []); setHistory([]); setReportedAssets([]); setReviewing(true); void loadFeedbackNotifications(); }
  ...
  async function downloadSnapshot(item:{id:number;stableId:string},format:"pdf"|"json"){const response=await fetch(`${API_BASE}/api/v1/feedback/${item.id}/package-snapshot.${format}`,{headers:{Authorization:`Bearer ${token}`}});if(!response.ok){const data=await response.json().catch(()=>({}));setError(data.error||tt("The automatic snapshot is still being prepared.","La instantánea automática aún se está preparando."));return;}const url=URL.createObjectURL(await response.blob());const anchor=document.createElement("a");anchor.href=url;anchor.download=`${item.stableId}-feedback-snapshot.${format}`;anchor.click();window.setTimeout(()=>URL.revokeObjectURL(url),0);}
          ...
          >
            ...
            <div style={{ padding: 16 }}>
              ...
              {reviewing ? <section aria-label={tt("My feedback backlog", "Mi lista de comentarios")}>
                <section aria-labelledby="feedback-customer-updates-title" style={{ marginBottom: 12, padding: 10, border: "1px solid hsl(var(--border))", borderRadius: 7 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}><h3 id="feedback-customer-updates-title" style={{ margin: 0, fontSize: 13 }}>{tt("Feedback updates", "Actualizaciones del comentario")}</h3><button type="button" disabled={feedbackNotificationsLoading} onClick={() => void loadFeedbackNotifications()}>{feedbackNotificationsLoading?tt("Refreshing…","Actualizando…"):tt("Refresh","Actualizar")}</button></div>{feedbackNotificationsLoading&&<p role="status" aria-live="polite" style={{fontSize:11}}>{tt("Loading feedback updates…","Cargando actualizaciones…")}</p>}{feedbackNotificationError&&<div role="alert" aria-live="assertive" style={{marginTop:8,fontSize:11,color:"#b91c1c"}}>{feedbackNotificationError} <button type="button" onClick={() => void loadFeedbackNotifications()}>{tt("Try again","Intentar de nuevo")}</button></div>}{!feedbackNotificationsLoading&&!feedbackNotificationError&&!feedbackNotifications.length&&<p style={{fontSize:11,color:"hsl(var(--muted-foreground))"}}>{tt("No feedback updates yet.","Aún no hay actualizaciones.")}</p>}{!!feedbackNotifications.length&&<ol aria-label={tt("Customer feedback notifications","Notificaciones de comentarios del cliente")} style={{display:"grid",gap:7,paddingLeft:18}}>{feedbackNotifications.map(item=><li key={item.eventId} style={{fontSize:11}}><strong>{feedbackNotificationLabel(item.type)} · {item.stableId}</strong><div>{item.message||tt("A feedback update is available.","Hay una actualización disponible.")}</div><div style={{color:"hsl(var(--muted-foreground))"}}>{new Date(item.createdAt).toLocaleString(language)} · {notificationDeliveryLabel(item.delivery?.state)}</div>{item.delivery?.state!=="read"&&item.delivery?.notificationId&&<button type="button" disabled={pendingNotificationRead===item.eventId} onClick={()=>void markFeedbackNotificationRead(item.eventId)}>{pendingNotificationRead===item.eventId?tt("Marking read…","Marcando como leída…"):tt("Mark read","Marcar como leída")}</button>}</li>)}</ol>}</section>
                {!mine.length ? <p>{tt("No feedback has been reported.", "No se han reportado comentarios.")}</p> : mine.map(item => <article key={item.id} style={{ border: "1px solid hsl(var(--border))", borderRadius: 6, padding: 10, marginBottom: 8 }}><strong>{item.stableId}</strong> · {stateLabel(item.status)}<p style={{ margin: "5px 0" }}>{item.message}</p>{item.packageSnapshot?<p style={{fontSize:11,color:"#15803d"}}>{tt("Automatic PDF/JSON snapshot","Instantánea PDF/JSON automática")}: {stateLabel(item.packageSnapshot.state)}{item.packageSnapshot.generatedAt?` · ${new Date(item.packageSnapshot.generatedAt).toLocaleString(language)}`:""}</p>:<p style={{fontSize:11,color:"#b45309"}}>{tt("Automatic package snapshot is being prepared.","Se está preparando la instantánea automática del paquete.")}</p>}{item.relays.map((relay,index)=><details key={`${relay.assetId}-${relay.version}-${index}`}><summary>{tt("Evidence relay","Retransmisión de evidencia")} #{relay.assetId} · {stateLabel(relay.state)} v{relay.version}</summary><p>{new Date(relay.updatedAt).toLocaleString(language)}{relay.reason?` — ${relayReasonLabel(relay.reason)}`:""}</p><ol>{relay.history.map(event=><li key={event.sequence}>{stateLabel(event.state)} · {new Date(event.at).toLocaleString(language)}{event.reason?` — ${relayReasonLabel(event.reason)}`:""}</li>)}</ol></details>)}{item.targetRelease && <p>{tt("Target", "Objetivo")}: {item.targetRelease}</p>}{item.transcription&&<details><summary>{tt("Transcript", "Transcripción")} · {stateLabel(item.transcription.state)}</summary>{item.transcription.reason&&<p>{transcriptionReasonLabel(item.transcription.reason)}</p>}<p>{item.transcription.result||tt("No transcript is available yet.","Aún no hay transcripción disponible.")}</p>{item.transcription.state==="completed"&&item.transcription.reviewState==="pending"&&<><button type="button" onClick={()=>reviewTranscription(item.id,item.transcription!.id,"accepted")}>{tt("Accept transcript","Aceptar transcripción")}</button><button type="button" onClick={()=>reviewTranscription(item.id,item.transcription!.id,"rejected")}>{tt("Reject transcript","Rechazar transcripción")}</button></>}</details>}<button type="button" onClick={() => loadHistory(item.id)}>{tt("History", "Historial")}</button><button type="button" onClick={() => downloadPackage(item)}>{tt("Download ZIP", "Descargar ZIP")}</button><button type="button" disabled={!item.packageSnapshot} onClick={() => downloadSnapshot(item,"pdf")}>{tt("Automatic PDF", "PDF automático")}</button><button type="button" disabled={!item.packageSnapshot} onClick={() => downloadSnapshot(item,"json")}>{tt("Automatic JSON", "JSON automático")}</button>{["verified", "rejected", "deferred"].includes(item.status) && <button type="button" onClick={() => reopen(item)}>{tt("Reopen", "Reabrir")}</button>}</article>)}
