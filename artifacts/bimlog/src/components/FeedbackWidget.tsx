...
export function FeedbackWidget() {
  ...
  async function requestTranscription(feedbackId:number,asset:{id:number;transcriptionConsentId?:string|null}){if(!asset.transcriptionConsentId){setError(tt("Imported-audio transcription is awaiting the governed processing-consent schema update.","La transcripción de audio importado espera la actualización controlada del esquema de consentimiento de procesamiento."));return;}const response=await fetch(`${API_BASE}/api/v1/feedback/${feedbackId}/transcription`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`,"Idempotency-Key":`transcription:${feedbackId}:${asset.id}`},body:JSON.stringify({assetId:asset.id,consentId:asset.transcriptionConsentId})});const data=await response.json().catch(()=>({}));if(!response.ok&&response.status!==424){setError(data.error||tt("Transcription request failed.","Falló la solicitud de transcripción."));return;}setSuccess(response.status===424?tt("Transcription is safely queued but the provider is not activated.","La transcripción está en cola segura, pero el proveedor no está activado."):tt("Transcription requested.","Transcripción solicitada."));await loadMine();}
