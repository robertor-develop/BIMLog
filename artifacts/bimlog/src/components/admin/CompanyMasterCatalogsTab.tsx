...
export function CompanyMasterCatalogsTab({ token, spanish }: { token: string; spanish: boolean }) {
  ...
  return <section aria-labelledby="company-master-title" style={{ display: "grid", gap: 16 }}>
    ...
    {ready && capability && <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 14 }}>{kinds.map(kind => <section key={kind} style={{ border: "1px solid #cbd5e1", borderRadius: 10, padding: 14 }}><h3>{spanish ? labels[kind][1] : labels[kind][0]}</h3>
