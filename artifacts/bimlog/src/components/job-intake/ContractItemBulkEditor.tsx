    ...
    <div className="ji-bulk">
      ...
      {props.capabilities.budget && props.snapshots.length > 0 && (
        <div className="ji-bulk-budget">
          <label>
            {props.tt(
              "Approved budget link (optional)",
              "Vínculo de presupuesto aprobado (opcional)",
            )}
            <select
              ...
              {props.capabilities.costValuePlanner && (
                <>
                  ...
                  {props.apuVersions.length > 0 ? (
                    ...
                    <div className="ji-apu-empty-row" role="note">
                      <strong>{props.tt("Saved APU version (optional)", "Versión APU guardada (opcional)")}</strong>
                      <span>{props.tt("None saved yet. The editable unit rate above remains valid.", "Aún no hay ninguna guardada. La tarifa unitaria editable anterior sigue siendo válida.")}</span>
                    </div>
                  )}
