...
export function ProjectCompanyCreator({ request, projectId, onCreated, tt }: Props) {
  ...
  const save = async () => {
    ...
    if (catalogState !== "ready" || (catalogGoverned && !selectedClient)) {
  ...
  return <div className="ji-company-create" aria-label={tt("Add client company", "Agregar empresa cliente")}>
    ...
    <div className="ji-grid three">
      {catalogState === "error" ? <p role="alert">{tt("Client catalog could not be loaded. No company will be created.", "No se pudo cargar el catálogo de clientes. No se creará ninguna empresa.")}</p> : catalogState === "loading" ? <p>{tt("Loading client catalog…", "Cargando catálogo de clientes…")}</p> : <>{catalog.length > 0 && <label>{tt(catalogGoverned ? "Approved client — required" : "Company catalog client — optional", catalogGoverned ? "Cliente aprobado — obligatorio" : "Cliente del catálogo — opcional")}<select autoFocus value={selectedClientId} onChange={event => setSelectedClientId(event.target.value)}><option value="">{tt("Select client", "Seleccione cliente")}</option>{catalog.map(entry => <option key={entry.id} value={entry.id}>{entry.code} — {entry.name}</option>)}</select></label>}{catalogGoverned && catalog.length === 0 ? <p role="status">{tt("No active clients are in the company catalog. Ask your Company PMO administrator to add one before connecting it to this project.", "No hay clientes activos en el catálogo de la empresa. Pida al administrador PMO de su empresa que agregue uno antes de conectarlo con este proyecto.")}</p> : !catalogGoverned && <label>{tt("Or enter a company name", "O ingrese un nombre de empresa")}<input value={name} onChange={(event) => setName(event.target.value)} /></label>}</>}
    ...
    <div className="ji-actions"><button type="button" className="primary" disabled={saving || catalogState !== "ready" || (catalogGoverned ? !selectedClientId : !selectedClientId && !name.trim())} onClick={() => void save()}>{saving ? tt("Adding…", "Agregando…") : tt("Add and select company", "Agregar y seleccionar empresa")}</button><button type="button" disabled={saving} onClick={close}>{tt("Cancel", "Cancelar")}</button></div>
  </div>;
}
