...
export function CompanyJobMap({ data, setData, tt }: Props) {
  ...
  const update = (nextParticipants: any[], nextEngagements: any[]) => setData((old: any) => {
    const keptEngagementIds = new Set(nextEngagements.map((item: any) => item.id));
    const contracts = (old.commercial?.contracts ?? []).map((contract: any) => contract.engagementId && !keptEngagementIds.has(contract.engagementId) ? { ...contract, engagementId: "", counterpartyName: "" } : contract);
    return { ...old, relationships: { participants: nextParticipants, engagements: nextEngagements }, commercial: { ...old.commercial, contracts }, review: { ...old.review, contractConfirmed: false } };
  });
  const seed = () => {
  ...
  const removeRelationship = (id: string) => update(participants, engagements.filter((item: any) => item.id !== id));
  return <section className="cjm" aria-label={tt("Company job map", "Mapa de empresas del trabajo")}>
    ...
    {!participants.length ? <div className="cjm-empty"><Building2 size={28}/><h3>{tt("Start with your company and customer", "Comience con su empresa y el cliente")}</h3><p>{tt("BIMLog will use the customer already entered above. You can rename either company and add the GC, owner, trades, consultants, or vendors afterward.", "BIMLog usará el cliente ingresado arriba. Después puede renombrar cualquiera y agregar contratista general, propietario, especialidades, consultores o proveedores.")}</p><button type="button" className="primary" onClick={seed}>{tt("Create the starting map", "Crear el mapa inicial")}</button></div> : <>
      ...
      <div className="cjm-edges">{engagements.map((edge: any) => <div className="cjm-edge" key={edge.id}><strong>{participant(edge.providerParticipantId)?.companyName || "—"}</strong><ArrowRight size={18}/><strong>{participant(edge.customerParticipantId)?.companyName || "—"}</strong>{edge.description && <small>{edge.description}</small>}<button type="button" className="danger" aria-label={tt("Remove relationship", "Eliminar relación")} onClick={()=>removeRelationship(edge.id)}><Trash2 size={14}/></button></div>)}</div>
