    ...
    <section className="ji-quick" aria-label={tt("Quick Job Intake", "Ingreso rápido del trabajo")}>
      ...
      <div className="ji-quick-question">
        ...
        {step === 1 && <>
          ...
          <p>{tt("Select the client for the project and its contracts here. Floors are locations for work packages and tasks, not separate client assignments.", "Seleccione aquí el cliente del proyecto y sus contratos. Los pisos son ubicaciones de paquetes y tareas, no asignaciones independientes de cliente.")}</p>
          <div className="ji-grid">
            <label>{tt("Customer company — required", "Empresa cliente — obligatoria")}<select autoFocus value={data.identity?.clientCompanyId || ""} onChange={(event) => selectCustomer(event.target.value)}><option value="">{tt("Select a project company", "Seleccione una empresa del proyecto")}</option>{data.identity?.clientCompanyId && !companies.some(company => company.id === Number(data.identity.clientCompanyId)) && <option value={data.identity.clientCompanyId}>{data.identity.clientCompany} ({tt("historical selection", "selección histórica")})</option>}{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label>
