...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, padding: 12, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label={title} tabIndex={-1} style={{ background: "hsl(var(--background))", border: "1px solid hsl(var(--border))", borderRadius: 10, padding: 24, minWidth: 0, maxWidth: 560, width: "min(100%, 560px)", maxHeight: "85vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h3 style={{ fontWeight: 700, fontSize: 16, margin: 0 }}>{title}</h3>
          <button type="button" aria-label={isSpanishUi() ? "Cerrar diálogo" : "Close dialog"} onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, color: "hsl(var(--muted-foreground))" }}>X</button>
        </div>
        {children}
      </div>
    </div>
...
type FeedbackOperationsStatus = {
  readiness?: { state?: string; explanation?: string; checkedAt?: string };
  storage?: { backend?: string; location?: string; metadataAuthority?: string; healthy?: boolean; capabilities?: unknown; maxReadBytes?: number; lastCheckedAt?: string };
  backup?: { configured?: boolean; state?: string; queued?: number; eligible?: number; deferred?: number; leased?: number; verified?: number; manualReview?: number; oldestEligibleAgeSeconds?: number | null; backendId?: string | null; restoreVerification?: string; independentFromPrimary?: boolean; encryption?: string | null };
  scanner?: { configured?: boolean; mode?: string; state?: string; backfill?: { quarantined?: number; eligible?: number; deferred?: number; leased?: number; manualReview?: number; oldestEligibleAgeSeconds?: number | null }; lastSuccessfulScanAt?: string | null; lastFailureAt?: string | null; recentFailures?: number };
  reviewerNotifications?: { state?: string; pending?: number; blocked?: number; oldestPendingAgeSeconds?: number | null };
  telegramDocuments?: { configured?: boolean; mode?: string; eligibleRecipients?: number; sent?: number; sending?: number; failed?: number; manualReview?: number; oldestActionableAt?: string | null };
  permanentComputerReceiver?: { connected?: boolean; root?: string; state?: string; explanation?: string; lastCheckedAt?: string | null };
};

const feedbackActivityLabel = (value: unknown, t: FeedbackTranslate) => ({
  created: t("Feedback submitted", "Comentario enviado"),
  submission_acknowledged: t("Submission acknowledged", "Recepción confirmada"),
  internal_reviewer_notifications_created: t("Review team notified", "Equipo revisor notificado"),
  assets_added: t("Evidence attached", "Evidencia adjunta"),
  evidence_scan_clean: t("Evidence verified safe", "Evidencia verificada como segura"),
  evidence_scan_rejected: t("Evidence rejected by controlled scanning", "Evidencia rechazada por el escaneo controlado"),
  triage_updated: t("Review status updated", "Estado de revisión actualizado"),
  customer_response: t("Customer response sent", "Respuesta enviada al cliente"),
  customer_decision: t("Decision shared with customer", "Decisión compartida con el cliente"),
  customer_fix: t("Resolution shared with customer", "Resolución compartida con el cliente"),
  customer_answer: t("Answer shared with customer", "Respuesta compartida con el cliente"),
  reopened: t("Feedback reopened", "Comentario reabierto"),
}[String(value)] || t("Recorded update", "Actualización registrada"));

const operationalStateLabel = (value: unknown, t: FeedbackTranslate) => ({
  healthy: t("Healthy", "Disponible"),
  ready: t("Ready", "Preparado"),
  configured: t("Configured", "Configurado"),
  pending: t("Pending", "Pendiente"),
  degraded: t("Degraded", "Degradado"),
  blocked: t("Blocked", "Bloqueado"),
  "action-required": t("Action required", "Acción requerida"),
  "not-configured": t("Not configured", "No configurado"),
  "not-mounted": t("Not mounted", "No montado"),
  unavailable: t("Unavailable", "No disponible"),
}[String(value)] || t("Unavailable", "No disponible"));

const reviewerAlertCopy = (value: unknown, t: FeedbackTranslate) => {
  const alert = value && typeof value === "object" && !Array.isArray(value) ? value as Record<string, unknown> : { state: value };
  const copy = ({
  delivered: { label: t("Delivered", "Entregada"), detail: t("Reviewer intake notification was recorded.", "Se registró la notificación de entrada al revisor."), color: "#166534" },
  blocked: { label: t("Blocked", "Bloqueada"), detail: t("No active reviewer received the alert; assignment is required.", "Ningún revisor activo recibió la alerta; se requiere asignación."), color: "#b91c1c" },
  pending: { label: t("Pending reconciliation", "Reconciliación pendiente"), detail: t("Intake is durable, but alert settlement is not yet recorded.", "La entrada es durable, pero aún no se registra el resultado de la alerta."), color: "#b45309" },
  "retry-required": { label: t("Reconciliation required", "Requiere reconciliación"), detail: t("Alert delivery failed closed; the notification worker must reconcile it.", "La entrega de la alerta falló de forma segura; el trabajador debe reconciliarla."), color: "#b91c1c" },
  }[String(alert.state || "pending")] || { label: t("Unavailable", "No disponible"), detail: t("Reviewer alert state is unavailable; do not assume delivery.", "El estado de la alerta no está disponible; no asumas que fue entregada."), color: "#64748b" }) as { label: string; detail: string; color: string };
  const safeReason = ({
    "notification-write-failed": t("Notification persistence failed; backend reconciliation is required.", "Falló la persistencia de la notificación; se requiere reconciliación del backend."),
    "no-active-superadmin": t("No eligible active superadmin recipient was available.", "No había un superadministrador activo elegible como destinatario."),
    "no-eligible-reviewer": t("No eligible reviewer recipient was available.", "No había un revisor elegible como destinatario."),
  } as Record<string, string>)[String(alert.reasonCode || "")];
  const when = alert.reconciledAt ? new Date(String(alert.reconciledAt)) : null;
  return { ...copy, detail: `${copy.detail}${safeReason ? ` ${safeReason}` : ""}${when && !Number.isNaN(when.getTime()) ? ` ${t("Reconciled", "Reconciliada")}: ${when.toLocaleString()}.` : ""}` };
};
...
function TelegramDeliveryStatus({ value, t, compact = false }: { value: unknown; t: FeedbackTranslate; compact?: boolean }) {
  ...
  const overall = ({
    ...
    failed: { label: t("Action required — failed", "Acción requerida — falló"), explanation: t("At least one document failed. Inspect the matrix and correct package or provider authority; no UI retry contract is available.", "Falló al menos un documento. Revisa la matriz y corrige la autoridad del paquete o proveedor; no hay contrato de reintento en la interfaz."), color: "#b91c1c" },
    ...
    "manual-review": { label: t("Manual review required", "Requiere revisión manual"), explanation: t("At least one provider result is indeterminate. Do not treat any unknown row as sent; backend reconciliation owns the next attempt.", "Al menos un resultado del proveedor es indeterminado. No trates ninguna fila desconocida como enviada; la reconciliación del backend controla el próximo intento."), color: "#b91c1c" },
  ...
  const outcomeCopy = (outcome: Record<string, unknown>) => {
    ...
    const reasonAction = ({
      "provider-not-configured": t("Configure the governed Telegram provider.", "Configura el proveedor gobernado de Telegram."),
      "no-opted-in-reviewer": t("An eligible superadmin must opt in to Telegram delivery.", "Un superadministrador elegible debe habilitar la entrega por Telegram."),
      "invalid-artifact-authority": t("Regenerate the governed package before a successor delivery attempt.", "Regenera el paquete gobernado antes de un intento sucesor."),
    } as Record<string,string>)[String(outcome.reasonCode || "")];
    ...
    if (state === "failed") return { label: t("Failed", "Falló"), action: reasonAction || t("Correct package/provider authority. No UI retry contract is available.", "Corrige la autoridad del paquete/proveedor. No hay contrato de reintento en la interfaz."), color: "#b91c1c" };
    if (state === "skipped") return { label: t("Not sent", "No enviado"), action: reasonAction || t("Configure the provider and an opted-in superadmin recipient.", "Configura el proveedor y un superadministrador destinatario habilitado."), color: "#92400e" };
    ...
    if (state === "unknown") return { label: t("Unknown — manual review", "Desconocido — revisión manual"), action: t("Provider settlement is indeterminate. Do not assume delivery; backend reconciliation owns follow-up.", "El resultado del proveedor es indeterminado. No asumas la entrega; la reconciliación del backend controla el seguimiento."), color: "#b91c1c" };
...
function FeedbackTab({ token }: { token: string }) {
  ...
  const [operations, setOperations] = useState<FeedbackOperationsStatus | null>(null);
  ...
  const detailBackdropRef = useRef<HTMLButtonElement>(null);
  ...
  const loadOperations = useCallback(() => {
    ...
    return apiFetch("/feedback/admin/operations-status", token)
  ...
  useEffect(() => {
    if (selectedId === null || !detailRef.current) return;
    const dialog = detailRef.current; dialog.focus();
    const onKey = (event: KeyboardEvent) => { if (event.key === "Escape") { event.preventDefault(); closeDetail(); return; } if (event.key !== "Tab") return; const focusable = Array.from(dialog.querySelectorAll<HTMLElement>('button:not([disabled]),select:not([disabled]),a[href],[tabindex]:not([tabindex="-1"])')).filter(node => node.offsetParent !== null); if (!focusable.length) return; const first=focusable[0],last=focusable[focusable.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus();}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus();} };
    window.addEventListener("keydown", onKey);
    const restored:Array<[HTMLElement,boolean,string|null]>=[];let child:HTMLElement|null=dialog;
    while(child?.parentElement&&child.parentElement!==document.body){for(const sibling of Array.from(child.parentElement.children))if(sibling!==child&&sibling!==detailBackdropRef.current&&sibling instanceof HTMLElement){restored.push([sibling,sibling.inert,sibling.getAttribute("aria-hidden")]);sibling.inert=true;sibling.setAttribute("aria-hidden","true");}child=child.parentElement;}
    return()=>{window.removeEventListener("keydown",onKey);for(const [node,inert,hidden] of restored){node.inert=inert;if(hidden===null)node.removeAttribute("aria-hidden");else node.setAttribute("aria-hidden",hidden);}};
  }, [selectedId]);
  ...
  async function openDetail(id: unknown) {
    ...
    if(typeof document!=="undefined"&&document.activeElement instanceof HTMLElement)detailOpenerRef.current=document.activeElement;
  ...
  return (
    <div aria-busy={loading || Boolean(pendingAction)}>
      <div className="feedback-command-bar" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 800, margin: 0 }}>{t("Feedback reviewer", "Revisión de comentarios")} <span style={{ fontSize: 10, color: "hsl(var(--muted-foreground))", whiteSpace: "nowrap" }}>v1.60.35.12-F</span></h2>
      ...
      {operations && <><div role="status" style={{ marginBottom: 10, fontSize: 11, color: "hsl(var(--muted-foreground))" }}>{operations.readiness?.state ? <>{t("Overall readiness", "Preparación general")}: <strong>{operationalStateLabel(operations.readiness.state, t)}</strong></> : t("Aggregate operational readiness is unavailable; use the individual states below.", "La preparación operativa agregada no está disponible; usa los estados individuales siguientes.")}</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: 10, marginBottom: 16 }}>
        <div style={{ border: "1px solid hsl(var(--border))", borderRadius: 9, padding: 11, background: "hsl(var(--card))" }}><strong style={{ fontSize: 12 }}>{t("Evidence custody", "Custodia de evidencia")}</strong><div style={{ fontSize: 11, marginTop: 4 }}>{String(operations.storage?.location || operations.storage?.backend || t("location unavailable", "ubicación no disponible"))} · {operations.storage?.healthy === true ? t("healthy", "disponible") : operations.storage?.healthy === false ? t("unavailable", "no disponible") : t("health unavailable", "salud no disponible")}</div><div style={{ fontSize: 10, marginTop: 4, color: "hsl(var(--muted-foreground))" }}>{t("Metadata", "Metadatos")}: {String(operations.storage?.metadataAuthority || "PostgreSQL")} · {t("Access: private through BIMLog", "Acceso: privado mediante BIMLog")}</div><div style={{ fontSize: 10, marginTop: 3, color: "hsl(var(--muted-foreground))" }}>{Array.isArray(operations.storage?.capabilities) || Number.isFinite(operations.storage?.maxReadBytes) ? `${t("Capabilities", "Capacidades")}: ${Array.isArray(operations.storage?.capabilities) ? operations.storage.capabilities.map(String).join(", ") : t("unavailable", "no disponibles")}${Number.isFinite(operations.storage?.maxReadBytes) ? ` · ${t("maximum read", "lectura máxima")}: ${Number(operations.storage?.maxReadBytes).toLocaleString()} bytes` : ""}` : t("Detailed storage limits are unavailable.", "Los límites detallados de almacenamiento no están disponibles.")}</div></div>
        <div style={{ border: "1px solid hsl(var(--border))", borderRadius: 9, padding: 11, background: "hsl(var(--card))" }}><strong style={{ fontSize: 12 }}>{t("Independent encrypted backup", "Copia cifrada independiente")}</strong><div style={{ fontSize: 11, marginTop: 4 }}>{operations.backup?.configured === true ? `${t("Configured", "Configurada")} · ${String(operations.backup.encryption || "AES-256-GCM")} · ${t("exact restore verified before settlement", "restauración exacta verificada antes de confirmar")}` : t("Not configured; primary custody remains available but backup readiness is blocked", "No configurada; la custodia primaria sigue disponible, pero la preparación de copia está bloqueada")}</div><div style={{ fontSize: 10, marginTop: 4, color: "hsl(var(--muted-foreground))" }}>{operations.backup ? `${Number(operations.backup.verified || 0)} ${t("verified", "verificadas")} · ${Number(operations.backup.eligible || 0)} ${t("eligible", "elegibles")} · ${Number(operations.backup.deferred || 0)} ${t("deferred", "aplazadas")} · ${Number(operations.backup.leased || 0)} ${t("leased", "arrendadas")} · ${Number(operations.backup.manualReview || 0)} ${t("manual review", "revisión manual")}` : t("Backup telemetry is unavailable.", "La telemetría de copia no está disponible.")}</div></div>
        <div style={{ border: "1px solid hsl(var(--border))", borderRadius: 9, padding: 11, background: "hsl(var(--card))" }}><strong style={{ fontSize: 12 }}>{t("Controlled scanner", "Escáner controlado")}</strong><div style={{ fontSize: 11, marginTop: 4 }}>{operations.scanner?.configured === true ? t("Configured; startup authority is present. This does not prove the worker is running.", "Configurado; la autoridad de inicio está presente. Esto no demuestra que el trabajador esté ejecutándose.") : operations.scanner?.configured === false ? t("Not configured; files remain safely quarantined and cannot be opened", "No configurado; los archivos permanecen en cuarentena y no pueden abrirse") : t("Configuration state unavailable; treat evidence as quarantined.", "Estado de configuración no disponible; trata la evidencia como en cuarentena.")}{operations.scanner?.state ? ` · ${t("State", "Estado")}: ${operationalStateLabel(operations.scanner.state, t)}` : ""}</div><div style={{ fontSize: 10, marginTop: 4, color: "hsl(var(--muted-foreground))" }}>{operations.scanner?.backfill ? `${t("Backfill", "Reproceso")}: ${Number(operations.scanner.backfill.quarantined || 0)} ${t("quarantined", "en cuarentena")} · ${Number(operations.scanner.backfill.eligible || 0)} ${t("eligible", "elegibles")} · ${Number(operations.scanner.backfill.deferred || 0)} ${t("deferred", "aplazados")} · ${Number(operations.scanner.backfill.leased || 0)} ${t("leased", "arrendados")} · ${Number(operations.scanner.backfill.manualReview || 0)} ${t("manual review", "revisión manual")} · ${Number(operations.scanner.recentFailures || 0)} ${t("recent failures", "fallos recientes")}` : t("Live worker and backfill telemetry are unavailable.", "La telemetría del trabajador y reproceso no está disponible.")}</div></div>
        <div style={{ border: "1px solid hsl(var(--border))", borderRadius: 9, padding: 11, background: "hsl(var(--card))" }}><strong style={{ fontSize: 12 }}>{t("Reviewer intake notifications", "Notificaciones de entrada al revisor")}</strong><div style={{ fontSize: 11, marginTop: 4 }}>{operations.reviewerNotifications?.state ? `${t("State", "Estado")}: ${operationalStateLabel(operations.reviewerNotifications.state, t)}` : t("Notification health is unavailable; intake remains available in this queue.", "La salud de notificaciones no está disponible; la entrada permanece visible en esta cola.")}</div><div style={{ fontSize: 10, marginTop: 4, color: "hsl(var(--muted-foreground))" }}>{operations.reviewerNotifications ? `${Number(operations.reviewerNotifications.pending || 0)} ${t("pending", "pendientes")} · ${Number(operations.reviewerNotifications.blocked || 0)} ${t("blocked", "bloqueadas")}${Number.isFinite(operations.reviewerNotifications.oldestPendingAgeSeconds) ? ` · ${t("oldest pending age", "antigüedad máxima pendiente")}: ${Number(operations.reviewerNotifications.oldestPendingAgeSeconds)}s` : ""}` : t("Reconciliation metrics are unavailable.", "Las métricas de reconciliación no están disponibles.")}</div></div>
        <div style={{ border: "1px solid hsl(var(--border))", borderRadius: 9, padding: 11, background: "hsl(var(--card))" }}><strong style={{ fontSize: 12 }}>{t("Internal document alert", "Alerta interna de documentos")}</strong><div style={{ fontSize: 11, marginTop: 4 }}>{operations.telegramDocuments?.configured === true ? t("Configured; verify every recipient/document outcome in the queue. Configuration does not mean sent.", "Configurada; verifica cada resultado por destinatario/documento en la cola. La configuración no significa enviado.") : operations.telegramDocuments?.configured === false ? t("Not configured; intake remains available in this queue", "No configurada; la entrada permanece visible en esta cola") : t("Configuration state unavailable; do not assume delivery.", "Estado de configuración no disponible; no asumas la entrega.")}</div><div style={{ fontSize: 10, marginTop: 4, color: "hsl(var(--muted-foreground))" }}>{operations.telegramDocuments && Number.isFinite(operations.telegramDocuments.eligibleRecipients) ? `${Number(operations.telegramDocuments.eligibleRecipients)} ${t("eligible recipients", "destinatarios elegibles")} · ${Number(operations.telegramDocuments.sent || 0)} ${t("sent", "enviados")} · ${Number(operations.telegramDocuments.sending || 0)} ${t("sending", "en curso")} · ${Number(operations.telegramDocuments.failed || 0)} ${t("failed", "fallidos")} · ${Number(operations.telegramDocuments.manualReview || 0)} ${t("manual review", "revisión manual")}` : t("Live worker telemetry is unavailable.", "La telemetría del trabajador no está disponible.")}</div></div>
        <div style={{ border: "1px solid hsl(var(--border))", borderRadius: 9, padding: 11, background: "hsl(var(--card))" }}><strong style={{ fontSize: 12 }}>{t("Permanent computer receiver", "Receptor permanente")}</strong><div style={{ fontSize: 11, marginTop: 4 }}>{operations.permanentComputerReceiver?.connected === true ? t("Connected", "Conectado") : operations.permanentComputerReceiver?.connected === false ? t("Not connected", "No conectado") : t("Connection state unavailable", "Estado de conexión no disponible")}{operations.permanentComputerReceiver?.state ? ` · ${operationalStateLabel(operations.permanentComputerReceiver.state, t)}` : ""}</div><div style={{ fontSize: 10, marginTop: 4, color: "hsl(var(--muted-foreground))" }}>{operations.permanentComputerReceiver?.state === "not-mounted" ? t("The governed receiver is not mounted; no delivery to the permanent computer is occurring.", "El receptor gobernado no está montado; no se está entregando al equipo permanente.") : `${t("Target", "Destino")}: ${String(operations.permanentComputerReceiver?.root || "F:\\BIMLog\\Feedback")}`}</div></div>
      </div></>}
              ...
              {visibleItems.map((item) => (
                <tr key={String(item.id)}>
                  ...
                  <Td style={{ minWidth: 170 }}>
                    {(() => { const evidence = (item.evidence || {}) as Record<string, unknown>; const total = Number(evidence.total || 0), quarantined = Number(evidence.quarantined || 0), clean = Number(evidence.clean || 0), rejected = Number(evidence.rejected || 0); const alert = reviewerAlertCopy(item.reviewerAlert || item.reviewerAlertState, t); return <><div style={{ fontSize: 12, fontWeight: 800 }}>{total} {t(total === 1 ? "file" : "files", total === 1 ? "archivo" : "archivos")}</div><div style={{ fontSize: 11, color: quarantined ? "#b45309" : rejected ? "#b91c1c" : "#15803d" }}>{clean} {t("clean", "seguros")} · {quarantined} {t("awaiting scan", "esperando escaneo")} · {rejected} {t("rejected", "rechazados")}</div><div style={{ fontSize: 11 }}>{t("Package", "Paquete")}: {String(item.packageState || "metadata-only").replace(/-/g, " ")}</div><div style={{ marginTop: 4, fontSize: 10, color: "hsl(var(--muted-foreground))" }}>{packageExplanation(item)}</div><div style={{ marginTop: 7, paddingTop: 7, borderTop: "1px solid hsl(var(--border))" }}><div style={{ marginBottom: 3, fontSize: 10, fontWeight: 800 }}>{t("Internal document delivery", "Entrega interna de documentos")}</div><TelegramDeliveryStatus value={item.telegramDelivery} t={t} compact /></div><div style={{ marginTop: 5, fontSize: 10 }}><strong style={{ color: alert.color }}>{t("Reviewer intake alert", "Alerta de entrada al revisor")}: {alert.label}</strong><div style={{ marginTop: 2, color: "hsl(var(--muted-foreground))" }}>{alert.detail}</div></div></>; })()}
                  </Td>
      ...
      {selectedId !== null && <><button ref={detailBackdropRef} type="button" aria-label={t("Close feedback details", "Cerrar detalles del comentario")} onClick={closeDetail} style={{ position: "fixed", inset: 0, zIndex: 1499, border: 0, background: "rgba(15,23,42,.42)" }} /><section ref={detailRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label={t("Feedback details", "Detalles del comentario")} style={{ position: "fixed", top: 16, right: 16, bottom: 16, zIndex: 1500, width: "min(720px, calc(100vw - 32px))", overflowY: "auto", padding: 22, border: "1px solid hsl(var(--border))", borderRadius: 14, background: "hsl(var(--card))", boxShadow: "0 28px 80px rgba(15,23,42,.38)" }}>
        ...
        {detailLoading ? <p role="status">{t("Loading complete package…", "Cargando paquete completo…")}</p> : detail ? <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10, marginBottom: 20 }}><div style={{ padding: 12, borderRadius: 10, background: "hsl(var(--muted) / .55)" }}><div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))" }}>{t("Feedback ID", "ID del comentario")}</div><strong>{String(((detail.feedback || {}) as Record<string, unknown>).stableId || selectedId)}</strong></div><div style={{ padding: 12, borderRadius: 10, background: "hsl(var(--muted) / .55)" }}><div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))" }}>{t("Evidence package", "Paquete de evidencia")}</div><strong>{String(detail.packageState || "unknown").replace(/-/g, " ")}</strong></div><div style={{ padding: 12, borderRadius: 10, background: "hsl(var(--muted) / .55)" }}><div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))" }}>{t("Current report", "Informe actual")}</div><strong>{t("Generated when downloaded", "Generado al descargar")}</strong></div></div>
