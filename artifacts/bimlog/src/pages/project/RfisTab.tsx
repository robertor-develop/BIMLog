...
function RfiDetailPanel({ projectId, rfi, canWrite, lang, members, user, onClose, onRevise, onExportPdf, onExportCompletePdf, onUpdate }: {
  ...
  const loadNotificationContext=async()=>{const token=JSON.parse(localStorage.getItem("bimlog-auth")||"{}").state?.token;const response=await fetch(`/api/v1/projects/${projectId}/rfis/${rfi.id}/notification-context`,{headers:{Authorization:`Bearer ${token}`}});const data=await response.json();if(!response.ok)throw new Error(data.error||w("Notification settings could not be loaded.","No se pudo cargar la configuración de notificaciones.",lang));setNotificationContext(data);setNotificationError(null);};
  useEffect(()=>{void loadNotificationContext().catch(error=>{const message=error instanceof Error?error.message:w("Notification settings could not be loaded.","No se pudo cargar la configuración de notificaciones.",lang);setNotificationError(message);toast({title:message,variant:"destructive"});});},[projectId,rfi.id]);
  ...
  return (
    <>
      <style>{`@media(max-width:720px){.app-shell:has([data-testid="rfi-notification-controls"])>.sidebar{display:none}.app-shell:has([data-testid="rfi-notification-controls"])>.main-area{width:100%;max-width:100%;min-width:0}.app-shell:has([data-testid="rfi-notification-controls"]) .project-context-bar{padding-left:12px;padding-right:12px}.app-shell:has([data-testid="rfi-notification-controls"]) .page-content{padding-left:12px!important;padding-right:12px!important}[data-testid="rfi-notification-controls"]{display:grid!important;grid-template-columns:minmax(0,1fr)!important;align-items:stretch!important}[data-testid="rfi-notification-controls"]>div{min-width:0;overflow-wrap:anywhere}[data-testid="rfi-notification-controls"]>div:last-child{display:grid!important;grid-template-columns:minmax(0,1fr)!important;width:100%}[data-testid="rfi-notification-controls"] button{width:100%;white-space:normal;height:auto;min-height:36px}}`}</style>
      <div data-testid="rfi-notification-controls" style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap",padding:"10px 12px",marginBottom:10,border:"1px solid hsl(var(--border))",borderRadius:8,background:"hsl(var(--card))"}}><div><strong style={{fontSize:13}}>{w("RFI notifications","Notificaciones de RFI",lang)}</strong><div style={{fontSize:11,color:notificationError?"hsl(var(--destructive))":"hsl(var(--muted-foreground))"}}>{notificationError|| (notificationContext?(!notificationContext.telegramConnected?w("Connect a verified private Telegram chat from Profile before delivery can occur.","Conecta un chat privado verificado de Telegram desde Perfil antes de que pueda realizarse la entrega.",lang):w(`Effective behavior: ${notificationContext.effectiveFrequency.replaceAll("_"," ")}${notificationContext.inherited?" (inherited from Notification Center)":""}`,`Comportamiento efectivo: ${notificationContext.effectiveFrequency.replaceAll("_"," ")}${notificationContext.inherited?" (heredado del Centro de Notificaciones)":""}`,lang)):w("Loading notification behavior…","Cargando comportamiento de notificación…",lang))}</div></div><div style={{display:"flex",gap:7,flexWrap:"wrap"}}>{notificationError&&<Button type="button" size="sm" variant="outline" disabled={notificationSaving} onClick={()=>void loadNotificationContext().catch(error=>setNotificationError(error instanceof Error?error.message:w("Retry failed.","El reintento falló.",lang)))}>{w("Retry","Reintentar",lang)}</Button>}<Button type="button" size="sm" variant="outline" disabled={!notificationContext||notificationSaving} onClick={()=>void toggleRfiWatch()}>{notificationContext?.watched?w("Stop watching","Dejar de seguir",lang):w("Watch this RFI","Seguir este RFI",lang)}</Button>{notificationContext&&!notificationContext.telegramConnected&&<Button type="button" size="sm" variant="outline" onClick={()=>setPage("/profile")}>{w("Connect Telegram","Conectar Telegram",lang)}</Button>}<Button type="button" size="sm" variant="outline" onClick={()=>setPage("/settings/notifications")}>{w("Notification Settings","Configuración de Notificaciones",lang)}</Button></div></div>
      <RfiCanonicalForm
        lang={lang}
