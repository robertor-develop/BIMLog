...
interface Meeting {
  ...
  createdAt: string;
  linkedRfis?: LinkedRfi[];
  legacyRfis?: RFIRow[];
  linkedSubmittals?: LinkedSubmittal[];
  legacyDeliverables?: LegacyDeliverableRow[];
...
interface LinkedSubmittal {
  ...
  disciplineBucket: DisciplineBucket;
  status: string;
  responsible?: string | null;
...
interface LensViewpointCandidate extends Omit<
  LinkedLensViewpoint,
  "lensViewpointId"
> {
  id: number;
...
interface LegacyDeliverableRow {
  raw: string;
}
interface LegacyViewpointRow {
  raw: string;
}
...
const humanLabel = (value?: string | null) =>
  value
    ? value
        .replace(/[_-]+/g, " ")
...
export function MeetingsTab({
  ...
  const [selectorMeetingId, setSelectorMeetingId] = useState<
  ...
  const [selectorCandidateCache, setSelectorCandidateCache] = useState<
    Map<number, RfiCandidate>
  >(new Map());
  const [selectorSelectedIds, setSelectorSelectedIds] = useState<Set<number>>(
    new Set(),
  );
  ...
  const [selectedSubmittals, setSelectedSubmittals] = useState<
    LinkedSubmittal[]
  >([]);
  const [submittalSelectorMeetingId, setSubmittalSelectorMeetingId] = useState<
    number | null | undefined
  >(undefined);
  ...
  const [submittalCandidates, setSubmittalCandidates] = useState<
    SubmittalCandidate[]
  >([]);
  const [submittalCandidateCache, setSubmittalCandidateCache] = useState<
    Map<number, SubmittalCandidate>
  >(new Map());
  const [submittalSelectedIds, setSubmittalSelectedIds] = useState<Set<number>>(
    new Set(),
  );
  ...
  const [selectedLensViewpoints, setSelectedLensViewpoints] = useState<
    LinkedLensViewpoint[]
  >([]);
  const [lensSelectorMeetingId, setLensSelectorMeetingId] = useState<
    number | null | undefined
  >(undefined);
  const [lensQuery, setLensQuery] = useState("");
  const [lensCandidates, setLensCandidates] = useState<
    LensViewpointCandidate[]
  >([]);
  const [lensCandidateCache, setLensCandidateCache] = useState<
    Map<number, LensViewpointCandidate>
  >(new Map());
  const [lensSelectedIds, setLensSelectedIds] = useState<Set<number>>(
    new Set(),
  );
  ...
  const [scheduleDialogMeeting, setScheduleDialogMeeting] =
  ...
  useEffect(() => {
    ...
    if (
      ...
      actionId <= 0 ||
      !actionItems.some((item) => item.id === actionId)
    )
      return;
    setView("actions");
    const timer = window.setTimeout(() => {
      const row = document.querySelector(
        `[data-meeting-action-id="${actionId}"]`,
      ) as HTMLElement | null;
      if (row) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      meetingId <= 0 ||
      !meetings.some((meeting) => meeting.id === meetingId)
    )
      return;
    setView("list");
    const timer = window.setTimeout(() => {
  ...
  const handleAudioUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (fileSizeMB > 100) {
      setAudioProgress(
        `Large file (${fileSizeMB}MB) ? compressing and splitting into parts. This may take a few minutes...`,
      );
    } else if (fileSizeMB > 25) {
      setAudioProgress(
        `File is ${fileSizeMB}MB ? compressing before upload...`,
      );
    } else {
    ...
    try {
      ...
      const r = await fetch(
        `${API}/projects/${projectId}/meetings/transcribe-audio`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
      ...
      if (!r.ok) {
        ...
        if (d.error === "no_openai_key") {
          ...
          setError(
            `Error: ${d.message || d.error || "Unknown ? check API server logs"}`,
          );
        }
        return;
      }
      ...
      setError(
        `Audio upload failed: ${err instanceof Error ? err.message : String(err)}${isDebug() ? " (debug: check console)" : ""}`,
      );
    } finally {
  ...
  const openEditMeeting = async (meeting: Meeting) => {
    ...
    if (!response.ok) {
      setError(
        t(
          "Could not open this Meeting for editing.",
          "No se pudo abrir esta Reunión para editar.",
        ),
      );
      return;
    }
    ...
    const when = new Date(detail.meetingDate);
    setEditingMeeting(detail);
    setMeetingNumber(String(meeting.id));
    setTitle(detail.title || "");
    setMeetingDate(
      Number.isNaN(when.getTime()) ? "" : when.toISOString().slice(0, 10),
    );
    setMeetingTime(
    ...
    setSelectedSubmittals(detail.linkedSubmittals ?? []);
    setSelectedLensViewpoints(detail.linkedLensViewpoints ?? []);
    setDeliverables([
      {
        floor: "UNDERGROUND",
        description: "",
    ...
    setViewpoints([]);
    setAiSummaryText(detail.aiSummary || "");
    setError("");
    setValidationSummary([]);
    setFieldErrors({});
    setView("new");
  ...
  const loadRfiCandidates = async () => {
    ...
    try {
      ...
      if (selectorMeetingId !== null)
        params.set("meeting_id", String(selectorMeetingId));
      const response = await fetch(
        `${API}/projects/${projectId}/meetings/rfi-candidates?${params.toString()}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      if (!response.ok)
        throw new Error(
          t(
            "Could not load project RFIs.",
            "No se pudieron cargar los RFI del proyecto.",
          ),
        );
      ...
      const normalizedRows = rows.map((row) => ({
        ...
        alreadyAdded:
          row.alreadyAdded ||
          (selectorMeetingId === null && locallySelected.has(row.id)),
      }));
  ...
  const openRfiSelector = (meetingId: number | null) => {
    ...
    setSelectorCandidateCache(new Map());
    setSelectorError("");
    setSelectorMeetingId(meetingId);
  };

  const closeRfiSelector = () => {
    if (!selectorSaving) setSelectorMeetingId(undefined);
  };
  ...
  const addSelectedExistingRfis = async () => {
    ...
    try {
      if (selectorMeetingId === null) {
        ...
        const response = await fetch(
          `${API}/projects/${projectId}/meetings/${selectorMeetingId}/rfis`,
          {
            method: "POST",
            headers,
        ...
        if (!response.ok) {
          ...
          throw new Error(
            body.error ||
              t(
                "Could not add the selected RFIs.",
                "No se pudieron añadir los RFI seleccionados.",
              ),
          );
        }
        await loadMeetings();
  ...
  const removeMeetingRfi = async (meetingId: number, rfiId: number) => {
    const response = await fetch(
      `${API}/projects/${projectId}/meetings/${meetingId}/rfis/${rfiId}`,
      { method: "DELETE", headers },
    );
    if (!response.ok) {
      setError(
        t(
          "Could not remove the meeting link.",
          "No se pudo quitar el enlace del acta.",
        ),
      );
      return;
    }
  ...
  const RfiSelectorModal = () =>
    selectorMeetingId !== undefined ? (
      <div
        role="dialog"
        aria-modal="true"
        aria-label={t("Add Existing RFI", "Añadir RFI existente")}
        style={{
          position: "fixed",
          inset: 0,
          zIndex: 1100,
        ...
        >
          ...
          >
            <div>
              <div style={{ fontSize: 17, fontWeight: 800 }}>
                {t("Add Existing RFI", "Añadir RFI existente")}
              </div>
              <div style={{ fontSize: 12, color: "#6B7280", marginTop: 2 }}>
                {t(
                  "Select one or more RFIs from this project.",
                  "Seleccione uno o más RFI de este proyecto.",
                )}
              </div>
            </div>
            <button
              aria-label={t("Close", "Cerrar")}
              onClick={closeRfiSelector}
              style={{
                border: 0,
          ...
          <div style={{ padding: "0 16px 12px", position: "relative" }}>
            ...
            <input
              ...
              placeholder={t(
                "Search RFI number, title, or description",
                "Buscar número, título o descripción del RFI",
              )}
              style={{ width: "100%", paddingLeft: 34, margin: 0 }}
            />
          </div>
          <div style={{ overflowY: "auto", padding: "0 16px", minHeight: 160 }}>
            ...
            {!selectorLoading && selectorError && (
              ...
              >
                ...
                <button
                  ...
                  onClick={() => void loadRfiCandidates()}
                  style={{ marginTop: 10 }}
                >
            ...
            {!selectorLoading &&
              ...
              selectorCandidates.length === 0 && (
                ...
                >
                  {selectorQuery
                    ? t("No matching RFIs.", "No hay RFI coincidentes.")
                    : t(
                        "No RFIs exist in this project.",
                        "No existen RFI en este proyecto.",
                      )}
                </div>
              )}
            {!selectorLoading &&
              !selectorError &&
              selectorCandidates.map((rfi) => {
                ...
                return (
                  <label
                    ...
                    style={{
                      ...
                      cursor: rfi.alreadyAdded ? "default" : "pointer",
                      opacity: rfi.alreadyAdded ? 0.65 : 1,
                    }}
                  >
                    <input
                      type="checkbox"
                      checked={checked || rfi.alreadyAdded}
                      disabled={rfi.alreadyAdded}
                      onChange={() =>
                        setSelectorSelectedIds((prev) => {
                          const next = new Set(prev);
                          checked ? next.delete(rfi.id) : next.add(rfi.id);
                          return next;
                        })
                      }
                      style={{ marginTop: 3 }}
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      >
                        <strong style={{ fontSize: 13 }}>{rfi.number}</strong>
                        <span style={{ fontSize: 13 }}>{rfi.title}</span>
                        {rfi.alreadyAdded && (
                          <span
                            style={{
                              fontSize: 10,
          ...
          >
            <button className="btn btn-outline" onClick={closeRfiSelector}>
              {t("Cancel", "Cancelar")}
            </button>
            <button
              className="btn btn-primary"
              onClick={() => void addSelectedExistingRfis()}
              disabled={!selectorSelectedIds.size || selectorSaving}
            >
              {selectorSaving
                ? t("Adding?", "Añadiendo?")
                : t(
                    `Add Selected (${selectorSelectedIds.size})`,
                    `Añadir seleccionados (${selectorSelectedIds.size})`,
                  )}
            </button>
          </div>
        </div>
  ...
  const loadSubmittalCandidates = async () => {
    ...
    try {
      ...
      if (submittalDiscipline.trim())
        params.set("discipline", submittalDiscipline.trim());
      if (submittalStatus.trim()) params.set("status", submittalStatus.trim());
      if (submittalResponsible.trim())
        params.set("responsible", submittalResponsible.trim());
      if (submittalSelectorMeetingId !== null)
        params.set("meeting_id", String(submittalSelectorMeetingId));
      const response = await fetch(
        `${API}/projects/${projectId}/meetings/submittal-candidates?${params.toString()}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      if (!response.ok)
        throw new Error(
          t(
            "Could not load project Submittals.",
            "No se pudieron cargar los Submittals del proyecto.",
          ),
        );
      const rows = (await response.json()) as SubmittalCandidate[];
      const locallySelected = new Set(
        selectedSubmittals.map((row) => row.submittalId),
      );
      const normalized = rows.map((row) => ({
        ...row,
        alreadyAdded:
          row.alreadyAdded ||
          (submittalSelectorMeetingId === null && locallySelected.has(row.id)),
      }));
  ...
  useEffect(() => {
    if (submittalSelectorMeetingId === undefined) return;
    const timer = window.setTimeout(() => {
      void loadSubmittalCandidates();
    }, 200);
    return () => window.clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    submittalSelectorMeetingId,
    submittalQuery,
    submittalFloor,
    submittalDiscipline,
    submittalStatus,
    submittalResponsible,
    projectId,
  ]);
  ...
  const openSubmittalSelector = (meetingId: number | null) => {
    setSubmittalQuery("");
    setSubmittalFloor("");
    setSubmittalDiscipline("");
    setSubmittalStatus("");
    setSubmittalResponsible("");
    setSubmittalSelectedIds(new Set());
    setSubmittalCandidates([]);
    setSubmittalCandidateCache(new Map());
    setSubmittalError("");
    setSubmittalSelectorMeetingId(meetingId);
  };

  const closeSubmittalSelector = () => {
    if (!submittalSaving) setSubmittalSelectorMeetingId(undefined);
  };
  ...
  const addSelectedSubmittals = async () => {
    ...
    try {
      if (submittalSelectorMeetingId === null) {
            ...
            ...ids
              ...
              .map((id) => {
                ...
                return {
                  ...
                  disciplineBucket: row.disciplineBucket,
                  status: row.status,
                  responsible: row.responsible,
        ...
        const response = await fetch(
          `${API}/projects/${projectId}/meetings/${submittalSelectorMeetingId}/submittals`,
          {
            method: "POST",
            headers,
            body: JSON.stringify({ submittal_ids: ids }),
          },
        );
        if (!response.ok) {
          throw new Error(
            t(
              "Could not add the selected Submittals.",
              "No se pudieron añadir los Submittals seleccionados.",
            ),
          );
        }
        await loadMeetings();
  ...
  const removeMeetingSubmittal = async (
    meetingId: number,
    submittalId: number,
  ) => {
    const response = await fetch(
      `${API}/projects/${projectId}/meetings/${meetingId}/submittals/${submittalId}`,
      { method: "DELETE", headers },
    );
    if (!response.ok) {
      setError(
  ...
  const openOriginalSubmittal = (submittalId: number) =>
    window.location.assign(
      `/projects/${projectId}/submittals?submittal=${submittalId}`,
    );

  const loadLensCandidates = async () => {
    if (lensSelectorMeetingId === undefined) return;
    setLensLoading(true);
    setLensError("");
    try {
      const params = new URLSearchParams();
      if (lensQuery.trim()) params.set("q", lensQuery.trim());
      if (lensSelectorMeetingId !== null)
        params.set("meeting_id", String(lensSelectorMeetingId));
      const response = await fetch(
        `${API}/projects/${projectId}/meetings/lens-viewpoint-candidates?${params.toString()}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      if (!response.ok)
        throw new Error(
          t(
            "Could not load Lens Viewpoints.",
            "No se pudieron cargar los Puntos de vista Lens.",
          ),
        );
      const rows = (await response.json()) as LensViewpointCandidate[];
      const locallySelected = new Set(
        selectedLensViewpoints.map((row) => row.lensViewpointId),
      );
      const normalized = rows.map((row) => ({
        ...row,
        alreadyAdded:
          row.alreadyAdded ||
          (lensSelectorMeetingId === null && locallySelected.has(row.id)),
      }));
      setLensCandidates(normalized);
      setLensCandidateCache((previous) => {
        const next = new Map(previous);
        normalized.forEach((row) => next.set(row.id, row));
        return next;
      });
  ...
  useEffect(() => {
    if (lensSelectorMeetingId === undefined) return;
    const timer = window.setTimeout(() => {
      void loadLensCandidates();
    }, 200);
    return () => window.clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lensSelectorMeetingId, lensQuery, projectId]);

  const openLensSelector = (meetingId: number | null) => {
    setLensQuery("");
    setLensSelectedIds(new Set());
    setLensCandidates([]);
    setLensCandidateCache(new Map());
    setLensError("");
    setLensSelectorMeetingId(meetingId);
  };

  const closeLensSelector = () => {
    if (!lensSaving) setLensSelectorMeetingId(undefined);
  };

  const addSelectedLensViewpoints = async () => {
    const ids = [...lensSelectedIds];
    if (!ids.length) return;
    setLensSaving(true);
    setLensError("");
    try {
      if (lensSelectorMeetingId === null) {
        setSelectedLensViewpoints((previous) => {
          const existing = new Set(previous.map((row) => row.lensViewpointId));
          return [
            ...previous,
            ...ids
              .filter((id) => !existing.has(id))
              .map((id) => {
                const row = lensCandidateCache.get(id)!;
                return { ...row, lensViewpointId: row.id };
              }),
          ];
        });
      }
      setLensSelectorMeetingId(undefined);
    } catch (err) {
      setLensError(
        err instanceof Error
          ? err.message
          : t(
              "Could not add Lens Viewpoints.",
              "No se pudieron añadir los Puntos de vista Lens.",
            ),
      );
    } finally {
      setLensSaving(false);
  ...
  const LensSelectorModal = () =>
    lensSelectorMeetingId !== undefined ? (
      <div
        role="dialog"
        aria-modal="true"
        aria-label={t(
          "Add Existing Lens Viewpoint",
          "Añadir punto de vista Lens existente",
        )}
        style={{
          position: "fixed",
          inset: 0,
        ...
        >
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 12, color: "#6B7280", marginTop: 2 }}>
                {t(
                  "Select active canonical Lens Viewpoints from this project. Meeting saves snapshots and never edits Lens.",
                  "Seleccione Puntos de vista Lens canónicos activos de este proyecto. El acta guarda capturas y nunca edita Lens.",
                )}
              </div>
            </div>
            <button
          ...
          <div style={{ padding: "0 16px 12px", position: "relative" }}>
            ...
            <input
              ...
              placeholder={t(
                "Search code, note, floor, trade, or responsible",
                "Buscar código, nota, piso, oficio o responsable",
              )}
              style={{ width: "100%", paddingLeft: 34, margin: 0 }}
            />
          </div>
          <div style={{ overflowY: "auto", padding: "0 16px", minHeight: 160 }}>
            {lensLoading && (
              ...
              >
                {t(
                  "Loading Lens Viewpoints?",
                  "Cargando Puntos de vista Lens?",
                )}
              </div>
            )}
            {!lensLoading && lensError && (
              ...
              >
                ...
                <button
                  type="button"
                  className="btn btn-sm btn-outline"
                  onClick={() => void loadLensCandidates()}
                  style={{ marginTop: 10 }}
                >
                  <RefreshCw size={12} style={{ marginRight: 5 }} />
                  {t("Retry", "Reintentar")}
            ...
            {!lensLoading && !lensError && lensCandidates.length === 0 && (
              <div
                style={{ padding: 32, textAlign: "center", color: "#6B7280" }}
              >
                {lensQuery
                  ? t(
                      "No Lens Viewpoints match this search.",
                      "Ningún Punto de vista Lens coincide con esta búsqueda.",
                    )
                  : t(
                      "No active Lens Viewpoints exist in this project.",
                      "No existen Puntos de vista Lens activos en este proyecto.",
                    )}
              </div>
            )}
            {!lensLoading &&
              !lensError &&
              lensCandidates.map((viewpoint) => {
                const checked = lensSelectedIds.has(viewpoint.id);
                return (
                  <label
                    key={viewpoint.id}
                    style={{
                      ...
                      padding: "12px 4px",
                      borderBottom: "1px solid #E5E7EB",
                      cursor: viewpoint.alreadyAdded ? "default" : "pointer",
                      opacity: viewpoint.alreadyAdded ? 0.65 : 1,
                    }}
                  >
                    <input
                      type="checkbox"
                      checked={checked || viewpoint.alreadyAdded}
                      disabled={viewpoint.alreadyAdded}
                      onChange={() =>
                        setLensSelectedIds((previous) => {
                          const next = new Set(previous);
                          checked
                            ? next.delete(viewpoint.id)
                            : next.add(viewpoint.id);
                          return next;
                        })
                      }
                      style={{ marginTop: 3 }}
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: 13 }}>
                          {viewpoint.note || t("No note", "Sin nota")}
                        </span>
                        {viewpoint.alreadyAdded && (
                          <span
                            style={{
                              fontSize: 10,
                              color: "#166534",
                      ...
                      >
                        <span>
                          {t("Floor", "Piso")}: {viewpoint.floor || "?"}
                        </span>
                        <span>
                          {t("Trade", "Oficio")}: {viewpoint.trade || "?"}
                        </span>
                        <span>
                          {t("Status", "Estado")}:{" "}
                          {humanLabel(viewpoint.status)}
                        </span>
                        <span>
                          {t("Responsible", "Responsable")}:{" "}
                          {viewpoint.responsible || "?"}
                        </span>
                        <span>
                          {t("Revision", "Revisión")}:{" "}
                          {viewpoint.revisionNumber}
                        </span>
                      </div>
                    </div>
                  </label>
          ...
          >
            ...
            <button
              type="button"
              className="btn btn-primary"
              onClick={() => void addSelectedLensViewpoints()}
              disabled={!lensSelectedIds.size || lensSaving}
            >
              {lensSaving
                ? t("Adding?", "Añadiendo?")
                : t(
                    `Add Selected (${lensSelectedIds.size})`,
                    `Añadir seleccionados (${lensSelectedIds.size})`,
                  )}
            </button>
          </div>
        </div>
  ...
  const openScheduleDialog = async (meeting: Meeting) => {
    const defaultDate = meeting.meetingDate
    ...
    setScheduleForm({
      ...
      responsibleCompany: "",
      includeMode: "all",
      selectedLinkIds: (meeting.linkedSubmittals ?? [])
        .map((link) => link.id)
  ...
  const schedulePayload = () => ({
    ...
    target_bucket_id: scheduleForm.targetBucketId
      ? Number(scheduleForm.targetBucketId)
      : undefined,
    general_deadline: scheduleForm.generalDeadline,
    responsible_user_id: scheduleForm.responsibleUserId
      ? Number(scheduleForm.responsibleUserId)
      : undefined,
  ...
  const loadSchedulePreview = async () => {
    ...
    try {
      const response = await fetch(
        `${API}/projects/${projectId}/meetings/${scheduleDialogMeeting.id}/schedule-bucket/preview`,
      ...
      if (!response.ok) {
        setScheduleError(
          data.error ||
            t(
              "Could not preview Schedule work.",
              "No se pudo previsualizar el trabajo de planificacion.",
            ),
        );
        return;
      }
  ...
  const createScheduleBucket = async () => {
    ...
    try {
      const response = await fetch(
        `${API}/projects/${projectId}/meetings/${scheduleDialogMeeting.id}/schedule-bucket`,
        { method: "POST", headers, body: JSON.stringify(schedulePayload()) },
      );
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
        setScheduleError(
          data.error ||
            t(
              "Could not create Schedule Bucket.",
              "No se pudo crear el grupo de planificacion.",
            ),
        );
        return;
      }
  ...
  const syncScheduleBucket = async (
    meetingId: number,
    bucketLinkId: number,
  ) => {
    const meeting = meetings.find((row) => row.id === meetingId);
    const bucketLink = meeting?.scheduleBuckets?.find(
      (row) => row.id === bucketLinkId,
    );
    if (!meeting || !bucketLink) return;
    const response = await fetch(
      `${API}/projects/${projectId}/meetings/${meetingId}/schedule-bucket/${bucketLinkId}/sync`,
      {
        method: "POST",
        headers,
        body: JSON.stringify({
          general_deadline: bucketLink.deadline
            ? new Date(bucketLink.deadline).toISOString().slice(0, 10)
            : new Date().toISOString().slice(0, 10),
          include_mode: "all",
          selected_meeting_submittal_link_ids: (meeting.linkedSubmittals ?? [])
            .map((link) => link.id)
            .filter(Boolean),
    ...
    if (!response.ok) {
      setError(
        t(
          "Could not sync Schedule Bucket.",
          "No se pudo sincronizar el grupo de planificacion.",
        ),
      );
      return;
    }
  ...
  const SubmittalSelectorModal = () =>
    submittalSelectorMeetingId !== undefined ? (
      <div
        role="dialog"
        aria-modal="true"
        aria-label={t(
          "Add from Submittal Log",
          "Añadir desde el Registro de Submittals",
        )}
        style={{
          position: "fixed",
          inset: 0,
      ...
      >
        ...
        >
          ...
          >
            <div>
              <div style={{ fontSize: 17, fontWeight: 800 }}>{t("Add from Submittal Log", "Añadir desde el Registro de Submittals")}</div>
              <div style={{ fontSize: 12, color: "#6B7280", marginTop: 2 }}>
                {t(
                  "Select one or more existing Submittals from this project.",
                  "Seleccione uno o más Submittals existentes de este proyecto.",
                )}
              </div>
            </div>
            <button
              aria-label={t("Close", "Cerrar")}
              onClick={closeSubmittalSelector}
              style={{
                border: 0,
                background: "transparent",
                cursor: "pointer",
          ...
          >
            <div style={{ position: "relative", gridColumn: "1 / -1" }}>
              ...
              <input
                autoFocus
                className="input"
                value={submittalQuery}
                onChange={(event) => setSubmittalQuery(event.target.value)}
                placeholder={t(
                  "Search number, title, or description",
                  "Buscar número, título o descripción",
                )}
                style={{ width: "100%", paddingLeft: 34, margin: 0 }}
              />
            </div>
            <input
              className="input"
              value={submittalFloor}
              onChange={(event) => setSubmittalFloor(event.target.value)}
              placeholder={t("Floor / area", "Piso / área")}
              style={{ margin: 0, minWidth: 0 }}
            />
            <input
              className="input"
              value={submittalDiscipline}
              onChange={(event) => setSubmittalDiscipline(event.target.value)}
              placeholder={t("Discipline / trade", "Disciplina / oficio")}
              style={{ margin: 0, minWidth: 0 }}
            />
            <input
              className="input"
              value={submittalStatus}
              onChange={(event) => setSubmittalStatus(event.target.value)}
              placeholder={t("Status", "Estado")}
              style={{ margin: 0, minWidth: 0 }}
            />
            <input
              className="input"
              value={submittalResponsible}
              onChange={(event) => setSubmittalResponsible(event.target.value)}
              placeholder={t(
                "Responsible person / company",
                "Persona / empresa responsable",
              )}
              style={{ margin: 0, minWidth: 0 }}
            />
          </div>
          <div style={{ overflowY: "auto", padding: "0 16px", minHeight: 160 }}>
            {submittalLoading && (
              <div
                style={{ padding: 32, textAlign: "center", color: "#6B7280" }}
              >
                {t("Loading Submittals?", "Cargando Submittals?")}
              </div>
            )}
            {!submittalLoading && submittalError && (
              <div
                style={{ padding: 24, textAlign: "center", color: "#B91C1C" }}
              >
                <div>{submittalError}</div>
                <button
                  className="btn btn-sm btn-outline"
                  onClick={() => void loadSubmittalCandidates()}
                  style={{ marginTop: 10 }}
                >
                  <RefreshCw size={12} style={{ marginRight: 5 }} />
                  {t("Retry", "Reintentar")}
                ...
                >
                  ...
                  submittalResponsible
                    ? t(
                        ...
                        "Ningún Submittal coincide con estos filtros.",
                      )
                    : t(
                        "No Submittals exist in this project.",
                        "No existen Submittals en este proyecto.",
                      )}
                </div>
              )}
            {!submittalLoading &&
              !submittalError &&
              submittalCandidates.map((submittal) => {
                ...
                return (
                  <label
                    ...
                    style={{
                      ...
                      padding: "12px 4px",
                      borderBottom: "1px solid #E5E7EB",
                      cursor: submittal.alreadyAdded ? "default" : "pointer",
                      opacity: submittal.alreadyAdded ? 0.65 : 1,
                    }}
                  >
                    <input
                      type="checkbox"
                      checked={checked || submittal.alreadyAdded}
                      disabled={submittal.alreadyAdded}
                      onChange={() =>
                        setSubmittalSelectedIds((previous) => {
                          const next = new Set(previous);
                          checked
                            ? next.delete(submittal.id)
                            : next.add(submittal.id);
                          return next;
                        })
                      }
                      style={{ marginTop: 3 }}
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: 13 }}>{submittal.title}</span>
                        {submittal.alreadyAdded && (
                          <span
                            style={{
                              fontSize: 10,
                              color: "#166534",
                      ...
                      {submittal.description &&
                        submittal.description !== submittal.title && (
                          <div
                            style={{
                              fontSize: 12,
                              color: "#4B5563",
                      ...
                      >
                        ...
                        <span>
                          {t("Responsible", "Responsable")}:{" "}
                          {submittal.responsible || "?"}
                        </span>
                        <span>
                          {t("Deadline", "Fecha límite")}:{" "}
                          {submittal.deadline
                            ? new Date(submittal.deadline).toLocaleDateString()
                            : "?"}
                        </span>
                      </div>
                    </div>
          ...
          >
            ...
            <button
              className="btn btn-primary"
              onClick={() => void addSelectedSubmittals()}
              disabled={!submittalSelectedIds.size || submittalSaving}
            >
              {submittalSaving
                ? t("Adding?", "Añadiendo?")
                : t(
                    `Add Selected (${submittalSelectedIds.size})`,
                    `Añadir seleccionados (${submittalSelectedIds.size})`,
                  )}
            </button>
          </div>
        </div>
      ...
      parts.push(
        "ATTENDEES:\n" +
          att
            .map(
              (a) =>
                `${a.trade} | ${a.company} | ${a.fullName} | ${a.role} | ${a.email} | ${a.phone}`,
            )
            .join("\n"),
      );
      ...
      parts.push(
        "RFIS:\n" +
          rfiRows
            .map(
              (r) =>
                `${r.rfiNumber} | ${r.description} | ${r.status} | ${r.responsible}`,
            )
            .join("\n"),
      );
    const delRows = deliverables.filter(
      (d) => d.description || d.plumbing || d.hvac,
    );
    if (delRows.length)
      parts.push(
        "DELIVERABLES:\n" +
          delRows
            .map(
              (d) =>
                `${d.floor} | ${d.description} | PL:${d.plumbing} | HVAC:${d.hvac} | FP:${d.fireProt} | ELE:${d.electrical} | DEADLINE:${d.deadline}`,
            )
            .join("\n"),
      );
      ...
      parts.push(
        "VIEWPOINTS:\n" +
          vpRows
            .map(
              (v) =>
                `${v.floor} | ${v.responsible} | ${v.viewpoint} | ${v.description} | ${v.deadline}`,
            )
            .join("\n"),
      );
    return parts.join("\n\n");
  ...
  const validateMeetingForm = () => {
    ...
    if (!title.trim())
      nextErrors.title = t(
        "Enter a meeting title before saving.",
        "Ingrese un título de reunión antes de guardar.",
      );
    if (!meetingDate)
      nextErrors.meetingDate = t(
        "Choose the meeting date before saving.",
        "Seleccione la fecha de la reunión antes de guardar.",
      );
    const incompleteAttendeeIndex = attendees.findIndex(
      (attendee) =>
    ...
    if (incompleteAttendeeIndex >= 0)
      nextErrors.attendees = t(
        `Attendee row ${incompleteAttendeeIndex + 1} needs a name, or clear the row.`,
        `La fila de asistente ${incompleteAttendeeIndex + 1} necesita nombre, o limpie la fila.`,
      );
    return nextErrors;
  };

  const showValidationErrors = (nextErrors: Record<string, string>) => {
    ...
    window.setTimeout(() => {
      ...
      else if (nextErrors.meetingDate) meetingDateInputRef.current?.focus();
      else bottomValidationRef.current?.focus();
      bottomValidationRef.current?.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
    }, 0);
  ...
  const saveMeeting = async () => {
    const nextErrors = validateMeetingForm();
    ...
    try {
      ...
      const body = {
        ...
        location,
        notes,
        ai_summary: aiSummaryText || undefined,
        expected_updated_at: editingMeeting?.updatedAt,
        attendees: attendees
          .filter((a) => a.fullName)
          .map((a) => ({
        ...
        submittal_ids: selectedSubmittals.map(
          (submittal) => submittal.submittalId,
        ),
        lens_viewpoint_ids: selectedLensViewpoints.map(
          (viewpoint) => viewpoint.lensViewpointId,
        ),
      };
      const r = await fetch(
        editingMeeting
          ? `${API}/projects/${projectId}/meetings/${editingMeeting.id}`
      ...
      if (!r.ok) {
        ...
        const message =
          d.error === "meeting_stale_update"
            ? t(
                "This Meeting changed after you opened it. Reload and review before saving.",
                "Esta Reunión cambió después de abrirla. Recargue y revise antes de guardar.",
              )
            : d.error ||
              t("Error saving Meeting.", "Error al guardar la Reunión.");
        showValidationErrors({ server: message });
        return;
      }
      await loadMeetings();
      setView("list");
  ...
  const generateAISummary = async () => {
    if (!title || !meetingDate) {
      setError("Enter title and date before generating summary");
      return;
    }
    setAiLoading(true);
    try {
      ...
      const r = await fetch(
        `${API}/projects/${projectId}/meetings/${meeting.id}/ai-summary`,
        { method: "POST", headers },
      );
      if (r.ok) {
  ...
  const SectionHeader = ({
    ...
    <div
      ...
      style={{
        ...
        cursor: "pointer",
        marginBottom: expandedSections[sectionKey] ? 0 : 8,
        userSelect: "none",
      }}
    >
      <span
    ...
    return (
      <div className="tab-content-wrapper">
        ...
        >
          <div>
            <h2 style={{ fontWeight: 700, fontSize: 18, margin: 0 }}>
              {t("Meeting Minutes", "Actas de Reunión")}
            </h2>
              ...
              <label style={{ cursor: importing ? "not-allowed" : "pointer" }}>
                ...
                >
                  ...
                  {importing
                    ...
                    : t("Import Minutes", "Importar Acta")}
                </div>
              </label>
            )}
            {canWrite && (
        ...
        >
          {[
            {
              label: t("Total Meetings", "Total Reuniones"),
              value: meetings.length,
              color: "#1D4ED8",
            },
            {
              label: t("Open Actions", "Acciones Abiertas"),
            ...
            },
            {
              ...
              color: overdueActions.length > 0 ? "#DC2626" : "#16A34A",
            },
            {
              label: t("Completed", "Completadas"),
              value: actionItems.filter((a) => a.status === "completed").length,
              color: "#16A34A",
            },
          ].map((s) => (
            <div
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          ...
          <button
            className={`btn btn-sm ${view === "actions" ? "btn-primary" : "btn-outline"}`}
                ...
                >
                  <div style={{ flex: "1 1 260px" }}>
                    ...
                    >
                      ...
                      <span>
                        ...
                        {m.attendeeCount} {t("attendees", "asistentes")}
                      </span>
                      <span>
                        <CheckCircle2 size={11} style={{ marginRight: 3 }} />
                        {m.openActionItems} {t("open", "abiertas")}
                      </span>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 6 }}>
                    {canWrite && (
                      <button
                        className="btn btn-sm btn-outline"
                        onClick={() => void openEditMeeting(m)}
                      >
                    ...
                    {canWrite && (
                      <button
                        className="btn btn-sm btn-outline"
                        onClick={() => openSubmittalSelector(m.id)}
                      >
                        <Plus size={12} style={{ marginRight: 4 }} />
                        {t(
                          "Add from Submittal Log",
                    ...
                    {canWrite && !!m.linkedSubmittals?.length && (
                      <button
                        className="btn btn-sm btn-primary"
                        onClick={() => void openScheduleDialog(m)}
                      >
                        <Calendar size={12} style={{ marginRight: 4 }} />
                        {t(
                          "Create Schedule Bucket",
                          "Crear grupo de planificacion",
                        )}
                      </button>
                    )}
                    {canWrite && (
                      <button
                        ...
                        onClick={() =>
                          setDeleteTarget({ id: m.id, label: m.title })
                        }
                        style={{
                          padding: "5px 9px",
                        ...
                        >
                          ...
                          {canWrite && (
                            <button
                              className="btn btn-sm btn-outline"
                              onClick={() =>
                                void removeMeetingRfi(m.id, rfi.rfiId)
                              }
                            >
                              {t("Remove link", "Quitar enlace")}
                            </button>
                          )}
                        </div>
                      ))}
                  ...
                  {!!m.linkedSubmittals?.length && (
                    <div style={{ flexBasis: "100%", display: "grid", gap: 8 }}>
                      {m.linkedSubmittals.map((submittal) => (
                        <div
                        ...
                        >
                          ...
                          <button
                            className="btn btn-sm btn-outline"
                            onClick={() =>
                              openOriginalSubmittal(submittal.submittalId)
                            }
                          >
                            <ExternalLink
                              size={12}
                            ...
                            />
                            {t(
                              "Open Original Submittal",
                              "Abrir Submittal original",
                            )}
                          </button>
                          {canWrite && (
                            <button
                  ...
                  {!!m.linkedLensViewpoints?.length && (
                    <div style={{ flexBasis: "100%", display: "grid", gap: 8 }}>
                      {m.linkedLensViewpoints.map((viewpoint) => (
                        <div
                          key={viewpoint.lensViewpointId}
                          style={{
                        ...
                        >
                          <div style={{ flex: "1 1 240px", minWidth: 0 }}>
                            <div style={{ fontSize: 13, fontWeight: 700 }}>
                              {viewpoint.code} ·{" "}
                              {viewpoint.note ||
                                t("Lens Viewpoint", "Punto de vista Lens")}
                            </div>
                            <div
                              style={{
                                fontSize: 11,
                            ...
                            >
                              {[
                                ...
                                humanLabel(viewpoint.status),
                                viewpoint.responsible,
                                `${t("Revision", "Revisión")} ${viewpoint.revisionNumber}`,
                              ]
                                .filter(Boolean)
                                .join(" · ")}
                            </div>
                          ...
                          <button
                            className="btn btn-sm btn-outline"
                            onClick={() =>
                              openOriginalLensViewpoint(
                                viewpoint.lensViewpointId,
                              )
                            }
                          >
                            <ExternalLink
                  ...
                  {!!m.legacyRfis?.length && (
                    ...
                    >
                      ...
                      >
                        {t(
                          "Legacy manual RFI notes",
                          "Notas RFI manuales anteriores",
                        )}
                      </div>
                      {m.legacyRfis.map((row, index) => (
                        <div
                          key={`${row.rfiNumber}-${index}`}
                          style={{ fontSize: 12 }}
                        >
                          {[
                            row.rfiNumber,
                  ...
                  {!!m.legacyDeliverables?.length && (
                    ...
                    >
                      ...
                      >
                        {t(
                          "Legacy manual Deliverable notes",
                          "Notas manuales anteriores de Entregables",
                        )}
                      </div>
                      {m.legacyDeliverables.map((row, index) => (
                        <div
                          key={`${row.raw}-${index}`}
                        ...
                        >
                          <div style={{ flex: "1 1 240px", minWidth: 0 }}>
                            <div style={{ fontSize: 13, fontWeight: 800 }}>
                              {t("Schedule Bucket", "Grupo de planificacion")}:{" "}
                              {bucket.bucketName}
                            </div>
                            <div
                              style={{
                            ...
                            >
                              ...
                              {t("Last outcome", "Ultimo resultado")}:{" "}
                              {bucket.summary?.created ?? 0}{" "}
                              {t("created", "creadas")},{" "}
                              {bucket.summary?.linked ?? 0}{" "}
                              {t("linked", "enlazadas")},{" "}
                              {bucket.summary?.updated ?? 0}{" "}
                              {t("updated", "actualizadas")},{" "}
                              {bucket.summary?.skipped ?? 0}{" "}
                              {t("skipped", "omitidas")}
                            </div>
                          </div>
                          <button
                            className="btn btn-sm btn-outline"
                            onClick={() =>
                              window.location.assign(bucket.openPath)
                            }
                          >
                            <ExternalLink
                              size={12}
                            ...
                            />
                            {t(
                              "Open Schedule Bucket",
                              "Abrir grupo de planificacion",
                            )}
                          </button>
                          {canWrite && (
                            <button
                              className="btn btn-sm btn-outline"
                              onClick={() =>
                                void syncScheduleBucket(m.id, bucket.id)
                              }
                            >
                              <RefreshCw size={12} style={{ marginRight: 4 }} />
                              {t("Sync Schedule Bucket", "Sincronizar grupo")}
                            </button>
                          )}
                        </div>
                      ))}
                  ...
                  <MeetingClashesPanel
                    projectId={projectId}
                    meetingId={m.id}
                    links={m.linkedClashes ?? []}
                    canWrite={canWrite}
                    reload={loadMeetings}
                  />
                  {!!m.legacyViewpoints?.length && (
                    <div
                      style={{
                        flexBasis: "100%",
                        padding: 10,
                    ...
                    >
                      ...
                      >
                        {t(
                          "Legacy manual Viewpoint / Clash rows",
                          "Filas manuales anteriores de Viewpoints / Clashes",
                        )}
                      </div>
                      {m.legacyViewpoints.map((row, index) => (
                        <div
                          key={`${row.raw}-${index}`}
                          style={{ fontSize: 12, overflowWrap: "anywhere" }}
                        >
          ...
          >
            ...
            >
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#6B7280", marginTop: 3 }}>
                    {t(
                      "Review linked Submittals, choose Create/Link/Update behavior, then create traceable Schedule tasks.",
                      "Revise Submittals enlazados, elija Crear/Enlazar/Actualizar y cree tareas trazables.",
                    )}
                  </div>
                </div>
                <button
                  aria-label={t("Close", "Cerrar")}
                  onClick={() => setScheduleDialogMeeting(null)}
                  style={{
                    border: 0,
                    background: "transparent",
                    cursor: "pointer",
              ...
              >
                <label>
                  <span className="label">
                    {t("Bucket name", "Nombre del grupo")}
                  </span>
                  <input
                    className="input"
                    value={scheduleForm.bucketName}
                    onChange={(e) =>
                      setScheduleForm((f) => ({
                        ...f,
                ...
                <label>
                  <span className="label">
                    {t("General deadline", "Fecha limite general")}
                  </span>
                  <input
                    className="input"
                    type="date"
                    value={scheduleForm.generalDeadline}
                    onChange={(e) =>
                      setScheduleForm((f) => ({
                        ...f,
                        generalDeadline: e.target.value,
                      }))
                    }
                  />
                </label>
                <label>
                  <span className="label">
                    {t(
                      "Target Schedule / Sprint",
                      "Cronograma / Sprint destino",
                    )}
                  </span>
                  <select
                    className="input"
                    value={scheduleForm.targetBucketId}
                    onChange={(e) =>
                      setScheduleForm((f) => ({
                        ...f,
                  ...
                  >
                    <option value="">
                      {t("Create named bucket", "Crear grupo con este nombre")}
                    </option>
                ...
                <label>
                  <span className="label">
                    {t("Default responsible", "Responsable por defecto")}
                  </span>
                  <select
                    className="input"
                    value={scheduleForm.responsibleUserId}
                    onChange={(e) =>
                      setScheduleForm((f) => ({
                        ...f,
                        ...
                        responsibleCompany:
                          memberList.find(
                            (m) => String(m.userId) === e.target.value,
                          )?.userCompanyName || f.responsibleCompany,
                      }))
                    }
                  >
                    <option value="">
                      {t("Needs user review", "Requiere revision")}
                    </option>
                    {memberList.map((m) => (
                      <option key={m.userId} value={m.userId}>
                        {m.userFullName} - {m.userCompanyName || m.userEmail}
                      </option>
                    ))}
                  </select>
                </label>
                <label>
                  ...
                  <input
                    className="input"
                    list="meeting-schedule-companies"
                    value={scheduleForm.responsibleCompany}
                    onChange={(e) =>
                      setScheduleForm((f) => ({
                        ...f,
                        responsibleCompany: e.target.value,
                      }))
                    }
                  />
                  <datalist id="meeting-schedule-companies">
                ...
                <label>
                  <span className="label">{t("Include", "Incluir")}</span>
                  <select
                    className="input"
                    value={scheduleForm.includeMode}
                    onChange={(e) =>
                      setScheduleForm((f) => ({
                        ...f,
                        includeMode: e.target.value,
                      }))
                    }
                  >
                    <option value="all">
                      {t(
                        "All linked Submittals",
                        "Todos los Submittals enlazados",
                      )}
                    </option>
                    <option value="pending">
                      {t(
                        "Only pending meeting actions",
                        "Solo acciones pendientes",
                      )}
                    </option>
                    <option value="selected">
                      {t("Selected only", "Solo seleccionados")}
                    </option>
                  </select>
                </label>
                <label
                ...
                >
                  <input
                    type="checkbox"
                    checked={scheduleForm.createMissing}
                    onChange={(e) =>
                      setScheduleForm((f) => ({
                        ...f,
                  ...
                  />
                  {t("Create missing tasks", "Crear tareas faltantes")}
                </label>
                <label
                  style={{ display: "flex", gap: 8, alignItems: "center" }}
                >
                  <input
                    type="checkbox"
                    checked={scheduleForm.linkExisting}
                    onChange={(e) =>
                      setScheduleForm((f) => ({
                        ...f,
                        linkExisting: e.target.checked,
                      }))
                    }
                  />
                  {t("Link existing tasks", "Enlazar tareas existentes")}
                </label>
                <label
                  style={{ display: "flex", gap: 8, alignItems: "center" }}
                >
                  <input
                    type="checkbox"
                    checked={scheduleForm.updateExisting}
                    onChange={(e) =>
                      setScheduleForm((f) => ({
                        ...f,
                  ...
                  />
                  {t(
                    "Update existing deadlines/assignments",
                    "Actualizar fechas/asignaciones existentes",
                  )}
                </label>
              </div>
              <div style={{ padding: "0 16px 12px", display: "grid", gap: 8 }}>
                {(scheduleDialogMeeting.linkedSubmittals ?? []).map((link) => (
                  <label
                    key={link.id || link.submittalId}
                    style={{
                      display: "flex",
                      gap: 10,
                      alignItems: "flex-start",
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={scheduleForm.selectedLinkIds.includes(link.id!)}
                      onChange={(e) =>
                        setScheduleForm((f) => ({
                          ...f,
                          selectedLinkIds: e.target.checked
                            ? [...f.selectedLinkIds, link.id!]
                            : f.selectedLinkIds.filter((id) => id !== link.id),
                        }))
                      }
                    />
                    <span style={{ minWidth: 0 }}>
                      <strong>{link.number}</strong> {link.title}
              ...
              {schedulePreview && (
                ...
                >
                  <strong>
                    {schedulePreview.summary.selected}{" "}
                    {t("selected", "seleccionados")}
                  </strong>
                  : {schedulePreview.summary.create}{" "}
                  {t(
                    "new tasks will be created",
                    "tareas nuevas seran creadas",
                  )}
                  , {schedulePreview.summary.link}{" "}
                  {t("existing tasks linked", "tareas existentes enlazadas")},{" "}
                  {schedulePreview.summary.update}{" "}
                  {t("updated", "actualizadas")},{" "}
              ...
              >
                ...
                <button
                  ...
                  onClick={() => void createScheduleBucket()}
                >
                  {scheduleBusy
                    ? t("Creating...", "Creando...")
                    : t(
        ...
        {deleteTarget && (
          <DeleteConfirmModal
            ...
            warning={t(
              "Attendees and linked items will be removed.",
              "Asistentes y elementos enlazados serán eliminados.",
            )}
          />
        )}
  ...
  return (
    <div className="tab-content-wrapper">
        ...
        >
          ...
          >
            ...
            <input
              ...
              placeholder={t(
                "e.g. Underground Coordination",
                "ej. Coordinación Subterránea",
              )}
              style={{
                marginTop: 0,
                borderColor: fieldErrors.title ? "#DC2626" : undefined,
              }}
            />
            {fieldErrors.title && (
              <div style={{ color: "#B91C1C", fontSize: 11, marginTop: 3 }}>
                {fieldErrors.title}
              </div>
            )}
          </div>
          ...
          >
            ...
            <input
              ...
              style={{
                marginTop: 0,
                borderColor: fieldErrors.meetingDate ? "#DC2626" : undefined,
              }}
            />
            ...
            {agendaItems.map((item, i) => (
              ...
              >
                ...
                <input
                  ...
                  placeholder={t(`Agenda item ${i + 1}`, `Punto ${i + 1}`)}
                  style={{ flex: 1 }}
                />
                <button
                  onClick={() =>
                    setAgendaItems(agendaItems.filter((_, j) => j !== i))
                  }
                  style={{
                    background: "none",
                    border: "none",
        ...
        {expandedSections.attendees && (
          ...
          >
                        ...
                        <td style={CELL_STYLE}>
                          ...
                          >
                            ...
                            {t(
                              ...
                              "Agregar empresa fuera de lista",
                            )}
                          </button>
                        </td>
                        <td style={CELL_STYLE}>
                          {isFreeTextPerson ? (
                            ...
                            <select
                              value={a.fullName}
                              onChange={(e) => {
                                const sel = people.find(
                                  (m) => m.userFullName === e.target.value,
                                );
                                const arr = [...attendees];
                                arr[i] = {
                                  ...arr[i],
                          ...
                          {a.company && people.length > 0 && (
                            <button
                              ...
                              style={{
                                ...
                                borderRadius: 4,
                                border: "1px dashed #2563EB",
                                background: freeTextPersons.includes(i)
                                  ? "#EFF6FF"
                                  : "transparent",
                                cursor: "pointer",
                                color: "#2563EB",
                            ...
                            >
                              <UserPlus size={11} />
                              {freeTextPersons.includes(i)
                                ? t("Select from list", "Seleccionar de lista")
                                : t(
                          ...
                          <button
                            onClick={() => {
                              ...
                              if (addCompanyIndex === i) resetAddCompanyForm();
                                ...
                                addCompanyIndex !== null &&
                                addCompanyIndex > i
                              )
                                setAddCompanyIndex(addCompanyIndex - 1);
                              setFreeTextPersons((prev) =>
                                prev
                                  .filter((j) => j !== i)
                                  .map((j) => (j > i ? j - 1 : j)),
                            ...
                            >
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  >
                                    {t("Contact Person", "Persona de Contacto")}
                                  </div>
                                  <input
            ...
            <div style={{ padding: 10 }}>
              ...
              >
                <Plus size={12} style={{ marginRight: 4 }} />
                {t("Add Attendee", "Agregar Asistente")}
              </button>
            </div>
          </div>
        )}
        ...
        {expandedSections.rfis && (
          ...
          >
              ...
              >
                ...
                {!editingMeeting && (
                  <button
                    ...
                    onClick={() =>
                      setSelectedRfis((prev) =>
                        prev.filter((item) => item.rfiId !== rfi.rfiId),
                      )
                    }
                  >
                    {t("Remove", "Quitar")}
            ...
            {rfis.length > 0 && (
              ...
              >
                {t(
                  "Imported manual RFI notes (preserved as legacy text)",
                  "Notas RFI manuales importadas (conservadas como texto anterior)",
                )}
              </div>
            )}
            {rfis.length > 0 && (
      ...
      <div style={{ marginBottom: 12 }}>
        <SectionHeader
          label={t("Submittals / Deliverables", "Submittals / Entregables")}
          ...
          >
            <div style={{ overflowX: "auto" }}>
              ...
              >
                <thead>
                  <tr>
                    {[
                      ...
                      <th key={h} style={{ ...TH_STYLE, fontSize: 10 }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {selectedSubmittals.map((submittal) => (
                    <tr
                      key={submittal.submittalId}
                      ...
                      {(
                        ...
                        <td
                          ...
                          style={{
                            ...
                            width: 90,
                            fontWeight:
                              submittal.disciplineBucket === bucket ? 700 : 400,
                          }}
                        >
                            ...
                            <button
                              ...
                              onClick={() =>
                                setSelectedSubmittals((previous) =>
                                  previous.filter(
                                    (row) =>
                                      row.submittalId !== submittal.submittalId,
                                  ),
                                )
                              }
                              style={{
            ...
            {!!deliverables.some(
              ...
              >
                ...
                >
                  {t(
                    "Legacy manual Deliverable rows (preserved)",
                    "Filas manuales anteriores de Entregables (conservadas)",
                  )}
                </div>
                {deliverables
                  .filter(
                  ...
                  .map((row, index) => (
                    <div key={index} style={{ fontSize: 12 }}>
                      {[
                        row.floor,
                        row.description,
                        row.plumbing && `PL:${row.plumbing}`,
                        row.hvac && `HVAC:${row.hvac}`,
                        row.fireProt && `FP:${row.fireProt}`,
                        row.electrical && `ELE:${row.electrical}`,
            ...
            <div style={{ padding: 10 }}>
              {!editingMeeting && (
                <button
                  className="btn btn-sm btn-outline"
                  onClick={() => openSubmittalSelector(null)}
                >
                  <Plus size={12} style={{ marginRight: 4 }} />
                  {t(
                    "Add from Submittal Log",
      ...
      <div style={{ marginBottom: 12 }}>
        <SectionHeader
          label={t("Lens Viewpoints", "Puntos de vista Lens")}
          sectionKey="viewpoints"
        />
        {expandedSections.viewpoints && (
          ...
          >
            <div style={{ padding: 10, display: "grid", gap: 8 }}>
              {selectedLensViewpoints.length === 0 && (
                <div style={{ color: "#6B7280", fontSize: 12 }}>
                  {t(
                    "No Lens Viewpoints selected yet.",
                    "No hay Puntos de vista Lens seleccionados todavía.",
                  )}
                </div>
              )}
              {selectedLensViewpoints.map((viewpoint) => (
                <div
                  key={viewpoint.lensViewpointId}
                  style={{
                    border: "1px solid #DDD6FE",
                ...
                >
                  ...
                  <button
                    className="btn btn-sm btn-outline"
                    onClick={() =>
                      openOriginalLensViewpoint(viewpoint.lensViewpointId)
                    }
                  >
                    <ExternalLink size={12} style={{ marginRight: 4 }} />
                    {t(
                    ...
                    <button
                      ...
                      onClick={() =>
                        setSelectedLensViewpoints((previous) =>
                          previous.filter(
                            (row) =>
                              row.lensViewpointId !== viewpoint.lensViewpointId,
                          ),
                        )
                      }
                    >
              ...
              {!!viewpoints.some((row) => row.description || row.viewpoint) && (
                <div
                  style={{
                    padding: 10,
                    color: "#6B7280",
                ...
                >
                  {t(
                    "Imported manual Viewpoint / Clash rows are preserved as legacy meeting text and are separate from Lens links.",
                    "Las filas manuales anteriores de Viewpoint / Clash se conservan como texto anterior del acta y están separadas de los enlaces Lens.",
                  )}
                </div>
              )}
            </div>
      ...
      >
        <div
          style={{
            ...
            justifyContent: "space-between",
            alignItems: "center",
            marginBottom: aiSummaryText ? 12 : 0,
          }}
        >
          <div
            style={{
          ...
          >
            ...
            {aiLoading
              ...
              : t("Generate Summary", "Generar Resumen")}
          </button>
        </div>
        {aiSummaryText && (
          <div
        ...
        >
          <div style={{ fontWeight: 800, marginBottom: 4 }}>
            {t(
              ...
              "Corrija estos puntos antes de guardar:",
            )}
          </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              <br />
              <br />
              <strong>Cost:</strong> ~$0.006 per minute of audio (~$0.36 per
              hour). You pay OpenAI directly ? BIMLog never charges for
              transcription.
              <br />
              <br />
