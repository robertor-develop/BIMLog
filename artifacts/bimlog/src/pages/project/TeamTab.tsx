...
const safePdfFileNameFromTitle = (title: string) => {
  const base =
    title
      .trim()
      .replace(/[^A-Za-z0-9]+/g, "-")
      .replace(/^-|-$/g, "") || "project-team-current-view";
  return `${base}.pdf`;
};
...
const parseContentDispositionFileName = (header: string | null) => {
  ...
  const rawMatch = quotedMatch
    ? null
    : header.match(/(?:^|;)\s*filename\s*=\s*([^;]*)/i);
  const value =
...
export function TeamTab({
  ...
  const companies = Array.from(
    new Set(
      memberRows.map(
        (m) =>
          m.userCompanyName || tt("Unknown company", "Empresa desconocida"),
      ),
    ),
  ...
  const filteredMembers = [...memberRows]
    ...
    .filter((m) => {
      if (!q) return true;
      return [m.userFullName, m.userEmail, m.userCompanyName, roleLabel(m.role)]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(q);
    ...
    .sort((a, b) => {
      const aCompany =
        a.userCompanyName || tt("Unknown company", "Empresa desconocida");
      const bCompany =
        b.userCompanyName || tt("Unknown company", "Empresa desconocida");
      ...
      if (sort === "role_asc")
        return (
          roleLabel(a.role).localeCompare(roleLabel(b.role)) ||
  ...
  const byCompany = filteredMembers.reduce<
  ...
  const adminCount = memberRows.filter((m) =>
    adminRoles.includes(m.role),
  ).length;
  const filteredAdminCount = filteredMembers.filter((m) =>
    adminRoles.includes(m.role),
  ).length;
  const activeSummary = [
  ...
  const exportCurrentViewPdf = async () => {
    ...
    try {
      const res = await fetch(
        `${API}/projects/${projectId}/members/current-view/pdf?${params.toString()}`,
        {
      ...
      if (!res.ok) {
        let message = tt(
          "Project Team PDF export failed.",
          "Fallo la exportacion PDF del equipo del proyecto.",
        );
        try {
    ...
    <div>
      ...
      <div className="section-header" style={{ marginBottom: 16 }}>
        <div>
          ...
          <div className="section-sub">
            ...
            {tt("members", "miembros")} · {filteredCompanyCount}/{companyCount}{" "}
            {tt("companies", "empresas")} · {filteredAdminCount}/{adminCount}{" "}
            {tt("admins", "admins")}
          </div>
        </div>
        <div
      ...
      >
          ...
          <div style={{ minWidth: 220, flex: "1 1 260px" }}>
            ...
            >
              {tt(
                ...
                "Reporte de vista actual del equipo",
              )}
            </div>
            <div
              style={{
            ...
            >
              {tt(
                "The PDF uses the filters, grouping, and selected columns shown here. Invitations and management controls are not exported.",
                "El PDF usa los filtros, agrupacion y columnas seleccionadas aqui. Invitaciones y controles administrativos no se exportan.",
              )}
            </div>
          </div>
          <div
        ...
        >
          ...
          >
            ...
            <input
              ...
              placeholder={tt(
                ...
                "Buscar nombre, correo, empresa, rol...",
              )}
              style={{
                border: "none",
                outline: "none",
          ...
          >
            <option value="all">
              {tt("All companies", "Todas las empresas")}
            </option>
          ...
          <select
            value={sort}
            onChange={(e) => setSort(e.target.value as TeamSortKey)}
            style={{
              border: "1px solid hsl(var(--border))",
              borderRadius: 6,
              padding: "6px 10px",
          ...
          >
            <option value="company_asc">
              {tt("Company, then name", "Empresa, luego nombre")}
            </option>
            ...
            <option value="joined_desc">
              {tt("Joined newest first", "Ingreso mas reciente")}
            </option>
            <option value="joined_asc">
              {tt("Joined oldest first", "Ingreso mas antiguo")}
            </option>
          </select>
          <select
            value={groupBy}
            onChange={(e) => setGroupBy(e.target.value as TeamGroupKey)}
            style={{
              border: "1px solid hsl(var(--border))",
              borderRadius: 6,
              padding: "6px 10px",
          ...
          >
            <option value="company">
              {tt("Group by company", "Agrupar por empresa")}
            </option>
        ...
        >
          ...
          >
            <input
              ...
              onChange={(e) => setIncludeJoined(e.target.checked)}
            />
      ...
      {showTransfer && myMembership && (
        <TransferAdminForm
          ...
          candidates={memberRows
            ...
            .map((m) => ({
              id: m.id,
              name: m.userFullName ?? m.userEmail ?? "Unknown",
              company: m.userCompanyName ?? "",
            }))}
          ...
          {Object.entries(byCompany).map(([company, companyMembers]) => (
            <div key={company}>
              ...
              >
                ...
                >
                  · {companyMembers!.length} member
                  {companyMembers!.length !== 1 ? "s" : ""}
                </span>
              </div>
                      ...
                      return (
                        <tr key={member.id}>
                          <td>
                            ...
                            >
                              <div
                                className={`avatar avatar-sm ${avatarColor}`}
                              >
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                {includeEmail && <th>{t("team.email")}</th>}
                {showCompanyColumn && (
                  <th style={{ width: 150 }}>{tt("Company", "Empresa")}</th>
                )}
                <th style={{ width: 130 }}>{t("team.role")}</th>
                {includeJoined && (
                  <th style={{ width: 110 }}>{t("team.joined")}</th>
                ...
                return (
                  <tr key={member.id}>
                    ...
                    {showCompanyColumn && (
                      ...
                      >
                        {member.userCompanyName ||
                          tt("Unknown company", "Empresa desconocida")}
                      </td>
                    )}
                    <td>
                      <div
      ...
      {!isLoading && (members ?? []).length === 0 && (
        <div className="empty-state">
          ...
          <div className="empty-desc">
            Add team members to grant them access to this project. Each member
            is assigned a role that controls their permissions.
          </div>
          {isAdmin && (
            <Button
              variant="outline"
      ...
      {!isLoading &&
        ...
        filteredMembers.length === 0 && (
          <div className="empty-state">
            ...
            <div className="empty-desc">
              {tt(
                "Adjust search, company, role, or grouping options to expand the current view.",
                "Ajusta busqueda, empresa, rol o agrupacion para ampliar la vista actual.",
              )}
            </div>
          </div>
        )}
...
function AddMemberForm({
  ...
  const { mutate, isPending } = useAddMember({
    mutation: {
      ...
      onError: () =>
        toast({
          title: "User not found - check the email address",
          variant: "destructive",
        }),
    },
  });
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      const r = await fetch(
        `${BASE}/api/v1/projects/${projectId}/invitations`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="inline-form" style={{ marginBottom: 16 }}>
      ...
      >
        ...
        >
          <UserCheck
            style={{ width: 12, height: 12, display: "inline", marginRight: 5 }}
          />
          {lang === "es" ? "Usuario existente" : "Existing User"}
        </button>
        <button
          style={tabStyle(activeTab === "invite")}
      ...
      {activeTab === "existing" && (
        <>
          ...
          >
            {lang === "es"
              ? "El usuario debe tener una cuenta BIMLog. Ingrese su correo registrado."
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <Input
                style={{ flex: 1 }}
                aria-label={
                  lang === "es" ? "Empresa asignada" : "Assigned company"
                }
...
function PendingInvitations({ projectId }: { projectId: number }) {
  ...
  const handleCancel = async (id: number) => {
    ...
    try {
      ...
      const BASE = import.meta.env.BASE_URL?.replace(/\/$/, "") || "";
      const r = await fetch(
        `${BASE}/api/v1/projects/${projectId}/invitations/${id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      toast({
        title: lang === "es" ? "Invitación cancelada" : "Invitation cancelled",
      });
    } catch (_) {
      toast({
        title:
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: 11, color: "#B45309", marginLeft: 4 }}>
          {lang === "es"
            ? `${invitations.length} en espera de registro`
            : `${invitations.length} awaiting registration`}
        </span>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {invitations.map((inv) => (
          ...
          >
            <Mail
              style={{ width: 13, height: 13, color: "#B45309", flexShrink: 0 }}
            />
            ...
            <button
              ...
              title={
                lang === "es" ? "Cancelar invitación" : "Cancel invitation"
              }
              aria-label={
                lang === "es" ? "Cancelar invitación" : "Cancel invitation"
              }
...
function TransferAdminForm({
  ...
  const handleTransfer = async () => {
    ...
    if (
      !confirm(
        "Transfer Project Admin? You will be demoted to Discipline Lead. Only one admin can exist per project.",
      )
    )
      return;
    setBusy(true);
    try {
      ...
      await updateMember({
        projectId,
        memberId: currentAdminMemberId,
        data: { role: "discipline_lead" },
      });
      queryClient.invalidateQueries({
        queryKey: [`/api/v1/projects/${projectId}/members`],
      });
      toast({
        ...
        description: "The new admin now has full control of the project.",
      });
      onClose();
    } catch (e) {
      toast({
  ...
  return (
    ...
    >
      ...
      >
        Only one Project Admin can exist per project. Choose the team member who
        will take over - you will be demoted to Discipline Lead. This action is
        logged.
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        <select
