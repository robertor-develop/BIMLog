...
export function CompanyWorkflowGovernance() {
  ...
  return <div className="wgp-shell"><MasterSidebar /><main className="wgp-main">
    ...
    {!loadFailed && !loading && <div className="wgp-layout"><aside className="wgp-list"><h2>{t("Policies","Políticas")}</h2>
      ...
      <p role="note">{t("Published validation rules are checked during workflow preview, approval and publication. Replacement versions also enforce prohibitions on changing phases, tasks or roles, APU association and economic allocation. A policy cannot be approved or published over incompatible workflows, and a forbidden retirement blocks the workflow API. Approval thresholds, approved Work Item change rules, and company-role rows remain recorded intent, not execution grants. Work Items still use assigned task roles and existing PMO/Finance authority.","Las reglas de validación publicadas se verifican durante la vista previa, aprobación y publicación del flujo. Las versiones de reemplazo también aplican las prohibiciones de cambiar fases, tareas o roles, asociación APU y distribución económica. Una política no se puede aprobar ni publicar sobre flujos incompatibles, y un retiro prohibido bloquea la API del flujo. Los umbrales de aprobación, las reglas de cambio de elementos de trabajo aprobados y las filas de roles siguen siendo intención registrada, no permisos de ejecución. Los elementos de trabajo siguen usando roles de tareas asignados y autoridad PMO/Finanzas existente.")}</p>
      ...
      <section className="wgp-card"><h3>{t("Approval hierarchy","Jerarquía de aprobaciones")}</h3>
        <p>{t("Role codes describe policy intent; they do not grant platform access yet. Threshold is in minor currency units.","Los códigos de rol describen la intención de la política; aún no otorgan acceso a la plataforma. El umbral se expresa en unidades menores de moneda.")}</p>
