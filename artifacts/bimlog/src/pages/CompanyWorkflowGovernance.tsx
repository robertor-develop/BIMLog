...
export function CompanyWorkflowGovernance() {
  ...
  return <div className="wgp-shell"><MasterSidebar /><main className="wgp-main">
    ...
    {!loadFailed && !loading && <div className="wgp-layout"><aside className="wgp-list"><h2>{t("Policies","Políticas")}</h2>
      ...
      <p role="note">{t("Published validation rules are checked when a workflow is approved or published. A policy cannot be approved or published over incompatible workflows, and a forbidden retirement blocks the workflow API. Approval thresholds, other change rules, and company-role rows remain recorded intent, not execution grants. Work Items still use assigned task roles and existing PMO/Finance authority.","Las reglas de validación publicadas se verifican al aprobar o publicar un flujo. Una política no se puede aprobar ni publicar sobre flujos incompatibles y una jubilación prohibida bloquea la API del flujo. Los umbrales de aprobación, las demás reglas de cambio y las filas de roles siguen siendo intención registrada, no permisos de ejecución. Los elementos de trabajo siguen usando roles de tareas asignados y autoridad PMO/Finanzas existente.")}</p>
