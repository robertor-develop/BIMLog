...
export function JobOperationsWorkspace() {
  ...
  return <FinancialProjectShell projectId={projectId} activeTab="operations"><style>{css}</style><main className="jo">
    ...
    {(data.reportingContracts?.length ?? 0) > 0 && <section className="jo-card"><h2>{tt("Contract reporting identity", "Identidad contractual para reportes")}</h2><div style={{overflow:"auto"}}><table className="jo-table"><thead><tr><th>{tt("Job", "Trabajo")}</th><th>{tt("Client", "Cliente")}</th><th>{tt("Contract / quote", "Contrato / cotización")}</th><th>{tt("Type", "Tipo")}</th><th>{tt("State", "Estado")}</th><th>{tt("Reference", "Referencia")}</th></tr></thead><tbody>{data.reportingContracts.map((contract:any)=><tr key={contract.id}><td>{contract.projectName}<br/><small>{contract.projectCode}</small></td><td>{contract.clientCompany}</td><td>{contract.contractName}</td><td>{contract.contractType?.replaceAll("_"," ")}</td><td>{contract.reportingStatus?.replaceAll("_"," ")} · {contract.lifecycleStatus}</td><td>{contract.contractNumber || contract.quotationNumber || "—"}</td></tr>)}</tbody></table></div></section>}
