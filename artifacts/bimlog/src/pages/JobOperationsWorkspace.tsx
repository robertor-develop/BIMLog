...
export function JobOperationsWorkspace() {
  ...
  return <FinancialProjectShell projectId={projectId} activeTab="operations"><style>{css}</style><main className="jo">
    ...
    {(data.reportingContracts?.length ?? 0) > 0 && <section className="jo-card"><h2>{tt("Contract reporting identity", "Identidad contractual para reportes")}</h2><div style={{overflow:"auto"}}><table className="jo-table"><thead><tr><th>{tt("Job", "Trabajo")}</th><th>{tt("Client", "Cliente")}</th><th>{tt("Contract / quote", "Contrato / cotización")}</th><th>{tt("Type", "Tipo")}</th><th>{tt("State", "Estado")}</th><th>{tt("Reference", "Referencia")}</th><th>{tt("APUs", "APUs")}</th></tr></thead><tbody>{data.reportingContracts.map((contract:any)=><tr key={contract.id}><td>{contract.projectName}<br/><small>{contract.projectCode}</small></td><td>{contract.clientCompany}</td><td>{contract.contractName}</td><td>{contract.contractType?.replaceAll("_"," ")}</td><td>{contract.reportingStatus?.replaceAll("_"," ")} · {contract.lifecycleStatus}</td><td>{contract.contractNumber || contract.quotationNumber || "—"}</td><td>{contract.apuCount} · {(contract.apuPlanVersions||[]).join(", ") || "—"}</td></tr>)}</tbody></table></div></section>}
    {(data.apuSnapshots?.length ?? 0) > 0 && <section className="jo-card"><h2>{tt("Immutable APU history", "Historial APU inmutable")}</h2><div style={{overflow:"auto"}}><table className="jo-table"><thead><tr><th>{tt("Contract Item", "Partida de Contrato")}</th><th>{tt("APU version", "Versión APU")}</th><th>{tt("Quantity", "Cantidad")}</th><th>{tt("Unit rate", "Tarifa unitaria")}</th><th>{tt("Value", "Valor")}</th><th>{tt("Fingerprint", "Huella")}</th></tr></thead><tbody>{data.apuSnapshots.map((snapshot:any)=><tr key={snapshot.snapshotFingerprint}><td>{snapshot.stableLineId}</td><td>{snapshot.pricingSnapshot?.apuPlanVersion ?? "—"}</td><td>{snapshot.pricingSnapshot?.quantity} {snapshot.pricingSnapshot?.unit}</td><td>{snapshot.pricingSnapshot?.unitRate}</td><td>{snapshot.pricingSnapshot?.contractValue}</td><td><code>{String(snapshot.snapshotFingerprint).slice(0,12)}</code></td></tr>)}</tbody></table></div></section>}
    {error && <div className="jo-error" role="alert">{error}</div>}{notice && <div className="jo-ok">{notice}</div>}
    {!data.available && <div className="jo-empty"><BriefcaseBusiness size={22}/><h2>{tt("Activate Job Intake first", "Primero active el Ingreso del Trabajo")}</h2><p>{tt("Assignment controls appear here only after Job Intake creates the operational tasks. Nothing can be assigned on this page before activation.", "Los controles de asignación aparecen aquí solo después de que el Ingreso del Trabajo crea las tareas operativas. No se puede asignar nada en esta página antes de activar.")}</p><ol><li>{tt("Open Job Intake & Setup.", "Abra Ingreso y Configuración del Trabajo.")}</li><li>{tt("In Team & resource plan, choose the project leader and add each assignment.", "En Equipo y plan de recursos, seleccione al líder del proyecto y agregue cada asignación.")}</li><li>{tt("In Review & activate, complete the confirmations and select Activate operational job.", "En Revisar y activar, complete las confirmaciones y seleccione Activar trabajo operativo.")}</li><li>{tt("Return here to update assignees, progress, hours, and work packages.", "Regrese aquí para actualizar responsables, avance, horas y paquetes de trabajo.")}</li></ol><Link className="primary" style={{display:"inline-flex",alignItems:"center",background:"#1d4ed8",border:"1px solid #1d4ed8",borderRadius:8,color:"#fff",fontWeight:700,padding:"9px 12px",textDecoration:"none"}} href={`/projects/${projectId}/intake`}>{tt("Open Job Intake & Setup", "Abrir Ingreso y Configuración del Trabajo")}</Link></div>}
    {data.available && <>
