...
assert.match(view, /Link RFI/);
assert.match(view, /Link Submittal/);
assert.match(view, /onRemoveLinkedItem/);
assert.match(view, /Help &amp; Guide/);
assert.match(view, /Only items from the current authorized BIMLog project can be linked/);
console.log("BUILD28_LINKING_FOCUSED=PASS cases=client-load-create-remove,authoritative-ids,project-scope,deleted-filter,duplicate-lock,write-auth,no-target-mutation,explicit-rfi-submittal-ui,customer-guide");
