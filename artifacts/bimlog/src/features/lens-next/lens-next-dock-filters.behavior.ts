...
assert.match(view, /onClick=\{\(\) => \{ setCompactDetailOpen\(false\); activateWorkspace\("viewpoints"\); \}\}[^>]*>\{tt\("View results", "Ver resultados"\)\}/);
assert.match(view, /onFiltersChange\(\{ \.\.\.filters, search: event\.target\.value \}\)/);
assert.match(view, /if \(next === "filters"\) \{\s*setWorkspaceLayout\(\(current\) => current\.filtersCollapsed\s*\? \{ \.\.\.current, filtersCollapsed: false \}/);
