...
assert.match(panel, /loadExportableLensNextPackages\(candidates, issue => apiClient\.loadVisualState\(issue\)\)/);
assert.match(panel, /bridgeClient\.exportViewpointsXml\(loaded\.exportable, loaded\.packages, bridgeContext\)/);
assert.match(panel, /No verified Visual Packages could be exported/);
assert.match(panel, /result\.cancelled[\s\S]*No destination file was written/);
assert.match(panel, /Partial export[\s\S]*serialized[\s\S]*skipped/);
