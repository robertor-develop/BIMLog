  ...
  return (
    <aside ref={lensRootRef} className="lens-next" data-dock-width={dockWidth} data-workspace={activeWorkspace} data-detail-open={compactDetailOpen && !!selectedIssue} aria-label="BIMLog Lens Next controlled issue workspace" aria-busy={refreshState === "refreshing" || reconciliationState === "running"}>
      ...
      <section ref={filterPaneRef} className={`lens-next__filters lens-next__filter-pane${workspaceLayout.filtersCollapsed?" lens-next__filter-pane--collapsed":""}`} aria-label="Issue filters">
        ...
        <button type="button" className="lens-next__filter-results" onClick={() => { setCompactDetailOpen(false); activateWorkspace("viewpoints"); }}>{tt("View results", "Ver resultados")} ({filteredIssues.length})</button>
      </section>
