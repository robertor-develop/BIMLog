...
const generatedIdentity = readFileSync(`${root}/../plugins/BIMLogLensNext/src/ReleaseIdentity.g.cs`, "utf8");
const releaseIdentity = JSON.parse(readFileSync(`${root}/../contracts/release-identity.json`, "utf8")) as { label: string };
...
assert.ok(generatedIdentity.includes(`ProductVersionLabel = "${releaseIdentity.label}"`), "generated Native release label must match the canonical release identity");
