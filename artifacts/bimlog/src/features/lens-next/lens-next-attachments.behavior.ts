...
assert.match(view, /Add Reference Attachment/);
assert.match(view, /max 5 MB/);
assert.match(view, /Open\/Download/);
assert.match(view, /PDF, JPG, JPEG, and PNG files up to 5 MB are accepted/);
console.log("BUILD29_ATTACHMENTS_FOCUSED=PASS cases=pdf-image-policy,5mb-server-limit,magic-signatures,project-scope,write-auth,persist-list,authenticated-download,orphan-safe-delete,compensation,explicit-customer-ui,customer-guide");
