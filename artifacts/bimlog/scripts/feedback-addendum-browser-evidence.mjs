...
async function runAdmin() {
  ...
  await page.getByText("Customer Feedback").waitFor();check(sid,"super-admin deep link opens reviewer queue",await page.getByRole("button",{name:"Feedback",exact:true}).count()===1);
  ...
  const deniedPage=await denied.newPage();await deniedPage.goto(`${baseUrl}/admin?tab=feedback`,{waitUntil:"networkidle"});check(deniedSid,"non-superadmin cannot see reviewer tab or panel",await deniedPage.getByRole("button",{name:"Feedback",exact:true}).count()===0&&await deniedPage.getByText("Customer Feedback").count()===0);check(deniedSid,"non-superadmin never calls reviewer API",await deniedPage.evaluate(()=>!window.__feedbackDeniedRequests.some(r=>r.pathname.startsWith("/api/v1/feedback/admin"))));scenarios.push({id:deniedSid,locale:"en-US",viewport,requests:await deniedPage.evaluate(()=>window.__feedbackDeniedRequests),consoleErrors:[],pageErrors:[],failedRequests:[],screenshots:[]});await denied.close();
}
