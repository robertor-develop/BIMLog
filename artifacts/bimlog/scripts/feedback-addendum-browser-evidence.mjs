...
async function runAdmin() {
  ...
  await context.addInitScript(()=>{
    ...
    const json=(body,status=200)=>new Response(JSON.stringify(body),{status,headers:{"Content-Type":"application/json"}});
    const feedback=[
      {id:801,stableId:"FB-OPS801",userEmail:"customer@example.invalid",userFullName:"Customer One",projectName:"Field Project",projectCode:"FP-1",feedbackType:"bug",priority:"urgent",module:"financial-controls",pageUrl:"/settings/financial-controls",message:"The governed intake must be visible.",status:"new",version:1,ownerUserId:null,createdAt:"2026-08-20T12:00:00Z",evidence:{total:2,clean:1,quarantined:1,rejected:0},packageState:"awaiting-scan"},
      {id:802,stableId:"FB-OPS802",userEmail:"customer2@example.invalid",userFullName:"Customer Two",projectName:"Ready Project",projectCode:"RP-2",feedbackType:"improvement",priority:"normal",module:"files",pageUrl:"/files",message:"A complete reviewed package.",status:"triaged",version:2,ownerUserId:501,createdAt:"2026-08-20T12:10:00Z",evidence:{total:1,clean:1,quarantined:0,rejected:0},packageState:"ready"},
    ];
  ...
  await page.getByRole("heading",{name:"Feedback",exact:true}).waitFor();check(sid,"super-admin deep link opens reviewer queue",await page.getByRole("button",{name:"Feedback",exact:true}).count()===1);
  await page.locator("tr").filter({hasText:"FB-OPS801"}).waitFor();check(sid,"ticket metadata is immediately visible while evidence is quarantined",await page.getByText("1 clean · 1 awaiting scan · 0 rejected").count()===1);
  await page.getByRole("region",{name:"Selected feedback package"}).waitFor();check(sid,"stable ticket deep link opens evidence and activity",await page.getByText("marked.png").count()===1&&await page.getByText("voice.webm").count()===1&&await page.getByText("submission_acknowledged").count()===1);
  const firstRow=page.locator("tr").filter({hasText:"FB-OPS801"});await firstRow.getByRole("button",{name:"Claim"}).click();await firstRow.getByText("Owner #501").waitFor();check(sid,"claim uses governed optimistic update",await page.evaluate(()=>window.__feedbackAdminEvidence.requests.some(r=>r.pathname.endsWith("/801")&&r.method==="PATCH"&&r.body?.claimToMe===true&&r.body?.observedVersion===1)));
  await firstRow.getByRole("button",{name:"Send update"}).click();check(sid,"customer update uses customer visibility",await page.evaluate(()=>window.__feedbackAdminEvidence.requests.some(r=>r.pathname.endsWith("/801/events")&&r.method==="POST"&&r.body?.visibility==="customer"&&r.body?.responseType==="response")));
  await page.getByRole("button",{name:"Follow-up register"}).click();const readyRow=page.locator("tr").filter({hasText:"FB-OPS802"});await readyRow.getByRole("button",{name:"Download ZIP"}).click();await page.waitForFunction(()=>window.__feedbackAdminEvidence.downloads.length===2);check(sid,"register and ready package are authenticated downloads",await page.evaluate(()=>{const S=window.__feedbackAdminEvidence;return S.requests.some(r=>r.pathname.endsWith("/follow-up.csv")&&r.headers.authorization==="Bearer bounded-admin-token")&&S.requests.some(r=>r.pathname.endsWith("/802/package.zip")&&r.headers.authorization==="Bearer bounded-admin-token")&&S.downloads.length===2;}));
...
try { await run("en",{width:1440,height:900});await run("es",{width:390,height:844});await runAdmin(); } finally { await browser.close(); }
