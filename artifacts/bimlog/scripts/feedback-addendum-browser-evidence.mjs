...
async function runAdmin() {
  ...
  await page.getByRole("button",{name:"Follow-up register"}).click();const readyRow=page.locator("tr").filter({hasText:"FB-OPS802"});await readyRow.getByRole("button",{name:"Download ZIP"}).click();check(sid,"register and ready package are authenticated downloads",await page.evaluate(()=>{const S=window.__feedbackAdminEvidence;return S.requests.some(r=>r.pathname.endsWith("/follow-up.csv")&&r.headers.authorization==="Bearer bounded-admin-token")&&S.requests.some(r=>r.pathname.endsWith("/802/package.zip")&&r.headers.authorization==="Bearer bounded-admin-token")&&S.downloads.length===2;}));
