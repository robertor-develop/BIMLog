...
async function run(language, viewport) {
  ...
  await context.addInitScript(({ language }) => {
    ...
    HTMLCanvasElement.prototype.getContext = () => ({ drawImage() {} }); HTMLCanvasElement.prototype.toBlob = cb => cb(new Blob(["png-fixture"], { type: "image/png" }));
  ...
  await page.evaluate(()=>window.__feedbackEvidence.displayMode="success");await page.getByText(tt("Capture screen","Capturar pantalla")).click();await page.getByText(/feedback-capture-.*\.png/).waitFor();await page.getByText(tt("Apply crop","Aplicar recorte")).click();await page.getByText(/-cropped\.png/).waitFor();check(sid,"screen capture track cleanup",await page.evaluate(()=>window.__feedbackEvidence.tracks.some(t=>t.kind==="display"&&t.stopped)));
  ...
  const uploads=await page.evaluate(()=>window.__feedbackEvidence.requests.filter(r=>r.pathname.endsWith("/assets")&&r.method==="POST"));check(sid,"stable per-file retry id",uploads.length>=4&&uploads.at(-1).headers["idempotency-key"]===uploads[1].headers["idempotency-key"],uploads.map(r=>r.headers["idempotency-key"]));check(sid,"capture origin and crop provenance",uploads.some(r=>r.form?.origin==="browser-display-capture"&&/sourceSha256/.test(r.form?.transformations||"")));
