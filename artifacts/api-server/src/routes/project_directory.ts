...
const DIRECTORY_SCOPES = new Set<DirectoryScope>([
  "all",
  "members",
  "contacts",
]);
const DIRECTORY_ROLES = new Set<DirectoryRoleFilter>([
  "all",
  "admin",
  "member",
  "external",
]);
const DIRECTORY_STATUSES = new Set<DirectoryStatusFilter>([
  "all",
  "active",
  "invited",
  "external",
]);
const DIRECTORY_SORTS = new Set<DirectorySort>([
  "name",
  "company",
  "role",
  "status",
...
function directoryLabels(lang: DirectoryLang) {
  ...
  return {
    reportTitle: es
      ? "Directorio del Proyecto — Vista actual"
      : "Project Directory — Current View",
    reportSubtitle: es
      ? "Directorio autorizado del proyecto"
    ...
    emptyProject: es
      ? "No se han agregado registros al directorio de este proyecto."
      : "No directory records have been added to this project.",
    emptyFiltered: es
      ? "Ningún registro del directorio coincide con los filtros actuales."
      : "No directory records match the current filters.",
    noMemberMatches: es
      ? "Ningún miembro del proyecto coincide con esta vista."
      : "No project members match this view.",
    noContactMatches: es
      ? "Ningún contacto adicional coincide con esta vista."
      : "No additional contacts match this view.",
    duplicateConsolidation: es
      ? "Correos duplicados consolidados bajo Miembros del Proyecto"
      : "Duplicate contact emails consolidated under Project Members",
  };
}
...
function directoryStatusLabel(
  status: DirectoryExportRow["status"],
  labels: ReturnType<typeof directoryLabels>,
) {
  if (status === "active") return labels.bimlogActive;
  if (status === "invited") return labels.invited;
...
function filterDirectoryRows(
  ...
  const filtered = rows.filter((row) => {
    ...
    if (
      options.role === "admin" &&
      !["admin", "project_admin"].includes(row.role)
    )
      return false;
    if (
      options.role === "member" &&
      (row.source !== "member" || ["admin", "project_admin"].includes(row.role))
    )
      return false;
  ...
  return filtered.sort(
    (a, b) =>
      read(a).localeCompare(read(b)) || a.fullName.localeCompare(b.fullName),
  );
}
...
router.get(
  "/projects/:projectId/directory/export-pdf",
  authMiddleware,
  requireProjectMember(),
  async (req, res) => {
    const projectId = Number(req.params.projectId);
    ...
    try {
      ...
      if (
        ...
        res
          .status(400)
          .json({ error: "Invalid directory export boolean option" });
        return;
      }
      ...
      if (!options.includeMembers && !options.includeContacts) {
        res
          .status(400)
          .json({ error: "Select at least one directory section" });
        return;
      }
      ...
      const contactRows = filteredRows.filter(
        (row) => row.source === "contact",
      );
      const duplicateContactCount = Math.max(
        0,
        (
          await db
            .select()
            .from(projectDirectoryTable)
            .where(eq(projectDirectoryTable.projectId, projectId))
        ).length - allRows.filter((row) => row.source === "contact").length,
      );
      const reportNumber = `DIR-${project.code}-${reportDate
        .toISOString()
        .replace(/[-:TZ.]/g, "")
        .slice(0, 14)}`;
      ...
      const snapshot = {
        ...
        filters: {
          ...options,
          labels: {
            scope: scopeLabel(scope, labels),
            role: roleLabel(role, labels),
            status: statusLabel(status, labels),
            sort: sortLabel(sort, labels),
          },
        },
        selectedSections,
        visibleColumns,
      ...
      doc
        ...
        .text(
          `${labels.preparedBy}: ${req.user!.fullName} | ${labels.generated}: ${reportDate.toISOString()} | ${labels.matching}: ${filteredRows.length} ${labels.of} ${allRows.length}`,
          40,
          doc.y,
          { width: 532 },
        );
      ...
      doc
        ...
        .text(
          `${labels.currentViewFilters}: ${filterSummary.join(" | ")}`,
          40,
          doc.y,
          { width: 532 },
        );
      ...
      doc
        ...
        .fillColor(PALETTE.MUTED)
        .text(
          `${labels.selectedSections}: ${selectedSections.join(", ") || labels.none} | ${labels.visibleColumns}: ${visibleColumns.join(", ")} | ${labels.sourceView}: ${labels.projectDirectory}`,
          40,
          doc.y,
          { width: 532 },
        );
      if (duplicateContactCount > 0) {
        ...
        doc
          ...
          .fillColor(PALETTE.MUTED)
          .text(
            `${labels.duplicateConsolidation}: ${duplicateContactCount}`,
            40,
            doc.y,
            { width: 532 },
          );
      ...
      const makeColumns = (): TableColumn[] => {
        const columns: TableColumn[] = [
          {
            ...
            wrap: true,
            bold: true,
            format: (row) => cleanPdfText(row.fullName),
          },
        ];
        if (options.includeEmail)
          columns.push({
            ...
            width: 135,
            wrap: true,
            format: (row) => cleanPdfText(row.email),
          });
        if (options.includeCompany)
          columns.push({
            ...
            width: 115,
            wrap: true,
            format: (row) => cleanPdfText(row.companyName),
          });
        if (options.includeRole)
          columns.push({
            ...
            width: 80,
            wrap: true,
            format: (row) => cleanPdfText(row.role),
          });
        if (options.includeStatus)
          columns.push({
            label: labels.status,
            width: 75,
            format: (row) => directoryStatusLabel(row.status, labels),
          });
        return columns;
      };
      ...
      if (filteredRows.length === 0) {
        const message =
          allRows.length === 0 ? labels.emptyProject : labels.emptyFiltered;
        y = sectionBar(
          doc,
          allRows.length === 0
            ? labels.directoryEmpty
            : labels.filteredViewEmpty,
          y,
          { theme },
        );
        doc
          ...
          .font(PALETTE.FONT)
          .fillColor(PALETTE.TEXT)
          .text(message, 40, y, { width: 532 });
      } else {
        if (options.includeMembers) {
          y = sectionBar(
            doc,
            `${labels.projectMembers} (${memberRows.length})`,
            y,
            { theme },
          );
          if (memberRows.length)
            ...
            doc
              ...
              .font(PALETTE.FONT)
              .fillColor(PALETTE.MUTED)
              .text(labels.noMemberMatches, 40, y, { width: 532 });
          y = doc.y + 12;
        }
        if (options.includeContacts) {
          ...
          y = sectionBar(
            doc,
            `${labels.additionalContacts} (${contactRows.length})`,
            y,
            { theme },
          );
          if (contactRows.length)
            ...
            doc
              ...
              .font(PALETTE.FONT)
              .fillColor(PALETTE.MUTED)
              .text(labels.noContactMatches, 40, y, { width: 532 });
        }
      }
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!full_name || !email || !role) {
      res.status(400).json({ error: "full_name, email, role required" });
      return;
    }
    try {
...
router.post(
  ...
  async (req, res) => {
    ...
    const companyType = normalizeCompanyName(
      String(req.body?.company_type ?? ""),
    );
    const profileDescription = normalizeCompanyName(
      String(req.body?.profile_description ?? ""),
    );
    const contactName = normalizeCompanyName(
      String(req.body?.primary_contact_name ?? ""),
    );
    const contactEmail = String(req.body?.primary_contact_email ?? "")
      .trim()
      .toLowerCase();
    const contactPhone = normalizeCompanyName(
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        if (!company) {
          ...
          if (profileDescription && !company.profileDescription)
        ...
        const [createdEntry] = await tx
          ...
          .values({
            ...
            email:
              contactEmail ||
              companyDirectoryEmail(projectId, company.id, companyName),
            companyName,
            ...
            notes:
              ...
              [
                ...
                "Registered from Meeting attendee workflow.",
              ]
                .filter(Boolean)
                .join(" "),
            addedById: req.user!.userId,
...
router.patch(
  ...
  async (req, res) => {
    ...
    const { full_name, email, company_name, role, notes } =
      req.body as Partial<{
        full_name: string;
        email: string;
        company_name: string;
  ...
  async (req, res) => {
    ...
    try {
      ...
      await db
        .update(projectDirectoryTable)
        .set({
          bimlogStatus: result.kind === "existing" ? "active" : "invited",
      ...
      await db.insert(activityLogTable).values({
        ...
        details:
          result.kind === "existing"
            ? `${result.alreadyMember ? "Confirmed" : "Added"} ${fullName} (${result.user.email}) as a project member`
      ...
      try {
        if (result.kind === "invited") {
          ...
          notifEnabled(result.user.notificationPreferences, "team_member_added")
        ) {
          const lang = getUserLang(result.user.notificationPreferences);
          await sendEmail({
        ...
        console.error(
          "[project-directory] Membership notification email failed:",
            ...
            messages: [
              {
                ...
                content: [
                  ...
                  {
                    type: "text",
                    text: `Extract all contact/directory records from this PDF document. Return ONLY a JSON array, no markdown. If none found return []:
[{"fullName":"person name","email":"email or null","companyName":"company or null","role":"role or null","notes":"notes or null"}]`,
          ...
          const extractText =
            extractMsg.content[0]?.type === "text"
              ...
              : "[]";
          records = JSON.parse(
            extractText.replace(/```json\n?|```/g, "").trim(),
          ) as any[];
          console.log(
            "[directory-import] PDF direct extraction:",
            records.length,
            "records",
          );
        } catch (e) {
