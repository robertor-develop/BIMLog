...
import { createPdfDocument, drawBrandedHeader, drawFooter, REPORT_THEMES, reportFileName, type ReportTheme } from "../lib/pdf-kit";
...
function pdfHeader(doc: PDFKit.PDFDocument, project: { name: string; code: string }, title: string, theme: ReportTheme) {
  doc.y = drawBrandedHeader(doc, {
    margin: 50, companyName: "BIMLog", title,
    projectName: project.name, projectCode: project.code,
    reportDate: new Date(), theme,
  }) + 12;
}

function pdfFooter(doc: PDFKit.PDFDocument, project: { name: string }) {
  drawFooter(doc, { margin: 50, y: doc.page.height - 30, projectName: project.name, timestamp: new Date().toLocaleDateString("en-US") });
}
...
router.get("/projects/:projectId/reports/meeting-minutes/pdf", async (req, res) => {
  ...
  try {
    ...
    const doc = createPdfDocument({ size: "LETTER", margin: 50 });
    res.setHeader("Content-Type", "application/pdf");
    const title = "Meeting Minutes Report";
    res.setHeader("Content-Disposition", `attachment; filename="${reportFileName(title)}"`);
    doc.pipe(res);
