...
async function accessible(id: number, user: NonNullable<Express.Request["user"]>, reader: Pick<typeof db, "select"> = db) {
  ...
  if (row.projectId && !await projectAuthorized(row.projectId, { ...user, isSuperAdmin: false }, actor.companyId, reader)) return null;
  return row;
}
...
router.post("/feedback/:id/reopen", authMiddleware, async (req, res) => {
  ...
  const updated = await db.transaction(async tx => {
    await tx.execute(sql`select pg_advisory_xact_lock(hashtextextended(${`reopen:${id}`},0))`); const fresh=await accessible(id,user);if(!fresh)return undefined;
...
router.post("/feedback/:id/comments", authMiddleware, async (req, res) => {
  ...
  const event = await db.transaction(async tx=>{await tx.execute(sql`select pg_advisory_xact_lock(hashtextextended(${`comment:${id}`},0))`);if(!await accessible(id,user))return null;const [row]=await tx.insert(feedbackAuditEventsTable).values({ feedbackId: id, actorUserId: user.userId, eventType: "commented", reason: comment }).returning();return row;}); if(!event)return res.status(403).json({code:"FEEDBACK_COMMENT_DENIED",error:"Comment access is denied"});
