...
const transcriptionDto = (row: typeof feedbackTranscriptionJobsTable.$inferSelect) => ({ id: row.id, assetId: row.assetId, state: row.state, result: row.result, errorCode: row.errorCode, reviewState: row.reviewState, createdAt: row.createdAt, completedAt: row.completedAt });
...
router.post("/feedback/:id/assets", authMiddleware, upload, async (req, res) => {
  ...
  try {
    ...
    const results = await db.transaction(async tx => {
      ...
      if(captureConsent){const [consumed]=await tx.select({id:feedbackAssetsTable.id}).from(feedbackAssetsTable).where(and(eq(feedbackAssetsTable.uploadedById,user.userId),sql`${feedbackAssetsTable.provenance}->>'consentId'=${captureConsent.id}`)).limit(1);if(consumed)throw Object.assign(new Error("Capture consent was already consumed by another evidence upload"),{status:409,code:"FEEDBACK_CAPTURE_CONSENT_CONSUMED"});}
...
router.get("/feedback/mine", authMiddleware, async (req, res) => {
  ...
  const relays=ids.length?await db.select({id:feedbackRelayJobsTable.id,feedbackId:feedbackRelayJobsTable.feedbackId,lineageId:feedbackRelayJobsTable.lineageId,state:feedbackRelayJobsTable.state,version:feedbackRelayJobsTable.version,createdAt:feedbackRelayJobsTable.createdAt,updatedAt:feedbackRelayJobsTable.updatedAt}).from(feedbackRelayJobsTable).where(and(inArray(feedbackRelayJobsTable.feedbackId,ids),eq(feedbackRelayJobsTable.companyId,actor.companyId))).orderBy(feedbackRelayJobsTable.feedbackId,feedbackRelayJobsTable.lineageId,desc(feedbackRelayJobsTable.version),desc(feedbackRelayJobsTable.updatedAt),desc(feedbackRelayJobsTable.id)):[];
  const currentByLineage=new Map<string,(typeof relays)[number]>();for(const relay of relays){const key=`${relay.feedbackId}:${relay.lineageId}`;if(!currentByLineage.has(key))currentByLineage.set(key,relay);}const relayByFeedback=new Map<number,(typeof relays)[number]>();for(const relay of currentByLineage.values()){const current=relayByFeedback.get(relay.feedbackId);if(!current||relay.updatedAt>current.updatedAt||(relay.updatedAt.getTime()===current.updatedAt.getTime()&&(relay.version>current.version||(relay.version===current.version&&relay.lineageId<current.lineageId))))relayByFeedback.set(relay.feedbackId,relay);}
  const relayIds=[...relayByFeedback.values()].map(relay=>relay.id);const custody=relayIds.length?await db.select({jobId:feedbackRelayCustodyEventsTable.jobId,sequence:feedbackRelayCustodyEventsTable.sequence,state:feedbackRelayCustodyEventsTable.toState,occurredAt:feedbackRelayCustodyEventsTable.occurredAt}).from(feedbackRelayCustodyEventsTable).where(inArray(feedbackRelayCustodyEventsTable.jobId,relayIds)).orderBy(feedbackRelayCustodyEventsTable.jobId,feedbackRelayCustodyEventsTable.sequence,feedbackRelayCustodyEventsTable.occurredAt):[];
