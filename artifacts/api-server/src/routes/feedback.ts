...
router.get("/feedback/admin/export.csv", authMiddleware, isSuperAdminMiddleware, async (req, res) => {
  ...
  res.setHeader("Content-Type", "text/csv; charset=utf-8"); res.setHeader("Content-Disposition", "attachment; filename=feedback-review-v1.60.35.09-F.csv");
...
router.get("/feedback/admin/follow-up.csv", authMiddleware, isSuperAdminMiddleware, async (req, res) => {
  ...
  const headers = ["Feedback ID", "Status", "Priority", "Type", "Module", "Project", "Submitter", "Submitter email", "Owner user ID", "Target release", "Decision reason", "Customer visible", "Created", "Updated", "Resolved", "Last event", "Last event at"];
...
router.post("/feedback/admin/:id/events",authMiddleware,isSuperAdminMiddleware,async(req,res)=>{
  ...
  const outcome=await db.transaction(async tx=>{await tx.execute(sql`select pg_advisory_xact_lock(hashtextextended(${`feedback-response:${id}:${idempotencyKey}`},0))`);const [feedback]=await tx.select({id:feedbackItemsTable.id,stableId:feedbackItemsTable.stableId,userId:feedbackItemsTable.userId,projectId:feedbackItemsTable.projectId,customerVisible:feedbackItemsTable.customerVisible,email:usersTable.email,preferences:usersTable.notificationPreferences}).from(feedbackItemsTable).innerJoin(usersTable,eq(feedbackItemsTable.userId,usersTable.id)).where(eq(feedbackItemsTable.id,id)).limit(1);if(!feedback)return {status:404,code:"FEEDBACK_NOT_FOUND",error:"Feedback not found"} as const;const [prior]=await tx.select().from(feedbackAuditEventsTable).where(and(eq(feedbackAuditEventsTable.feedbackId,id),sql`${feedbackAuditEventsTable.afterState}->>'idempotencyKey'=${idempotencyKey}`)).limit(1);if(prior){const state=prior.afterState as Record<string,unknown>;return state.requestHash===requestHash?{status:200,event:prior,replayed:true,email:null} as const:{status:409,code:"FEEDBACK_RESPONSE_IDEMPOTENCY_CONFLICT",error:"This idempotency key belongs to another response"} as const;}
    if(visibility==="customer"&&!feedback.customerVisible)return {status:409,code:"FEEDBACK_CUSTOMER_VISIBILITY_DISABLED",error:"Customer-visible responses require customer visibility"} as const;const [event]=await tx.insert(feedbackAuditEventsTable).values({feedbackId:id,actorUserId:user.userId,eventType,reason:message,afterState:{responseType,visibility,idempotencyKey,requestHash}}).returning();if(visibility==="internal")return {status:201,event,replayed:false,email:null} as const;
    const [notification]=await tx.insert(notificationsTable).values({userId:feedback.userId,projectId:feedback.projectId,type:`feedback_${responseType}`,title:`Feedback ${feedback.stableId}: ${responseType}`,message,actionUrl:"/feedback"}).returning({id:notificationsTable.id});await tx.insert(feedbackAuditEventsTable).values({feedbackId:id,actorUserId:user.userId,eventType:"customer_notification_delivery",afterState:{sourceEventId:event.id,notificationId:notification.id,channel:"in_app",state:"created",idempotencyKey:`notify:${event.id}`}});
    const emailEnabled=feedbackEmailCopyEnabled(feedback.preferences);await tx.insert(feedbackAuditEventsTable).values({feedbackId:id,actorUserId:user.userId,eventType:"customer_notification_delivery",afterState:{sourceEventId:event.id,channel:"email_copy",state:emailEnabled?"queued":"disabled",idempotencyKey:`email:${event.id}`,provider:emailEnabled?"approved-sendgrid":null}});return {status:201,event,replayed:false,email:emailEnabled?{to:feedback.email,stableId:feedback.stableId,responseType,message}:null} as const;});
  if("code" in outcome)return res.status(outcome.status).json({code:outcome.code,error:outcome.error});if(outcome.email)await sendEmail({to:outcome.email.to,subject:`BIMLog feedback ${outcome.email.stableId}: ${outcome.email.responseType}`,html:feedbackEmailCopyHtml(outcome.email.stableId,outcome.email.responseType,outcome.email.message,`${publicBaseUrl()}/feedback`),triggerType:"feedback_customer_copy"});return res.status(outcome.status).json({event:{id:outcome.event.id,type:outcome.event.eventType,message:outcome.event.reason,createdAt:outcome.event.createdAt},replayed:outcome.replayed,emailCopy:outcome.email?"queued":"disabled"});
});
