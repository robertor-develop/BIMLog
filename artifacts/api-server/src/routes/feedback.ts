...
async function loadFeedbackFollowUpRows(): Promise<FeedbackFollowUpRecord[]> {
  const result = await db.execute(sql`SELECT f.id, f.stable_id, f.status, f.priority, f.feedback_type, f.module, f.target_release, f.disposition_reason, f.owner_user_id, f.customer_visible, f.created_at, f.updated_at, f.resolved_at, u.full_name submitter_name, u.email submitter_email, p.code project_code, p.name project_name,
    ...
    COALESCE(pkg.after_state->>'state','not-generated') package_state, COALESCE(alert.after_state->>'state','pending') reviewer_alert_state, COALESCE(tg.after_state->>'state','not-requested') telegram_delivery_state,
    ...
    LEFT JOIN LATERAL (SELECT after_state FROM feedback_audit_events WHERE feedback_id=f.id AND event_type='package_snapshot_created' AND after_state->>'visibility'='internal' ORDER BY id DESC LIMIT 1) pkg ON true
    ...
    LEFT JOIN LATERAL (SELECT after_state FROM feedback_audit_events WHERE feedback_id=f.id AND event_type='feedback_telegram_delivery' ORDER BY id DESC LIMIT 1) tg ON true
  ...
  return result.rows as FeedbackFollowUpRecord[];
}
