...
import { notificationChannelsTable, notificationsTable, projectMembersTable, projectsTable, usersTable } from "@workspace/db/schema";
...
async function loadFeedbackFollowUpRows(): Promise<FeedbackFollowUpRecord[]> {
  ...
  const result = await db.execute(sql`SELECT f.id, f.stable_id, f.status, f.priority, f.feedback_type, f.module, f.target_release, f.disposition_reason, f.owner_user_id, f.customer_visible, f.created_at, f.updated_at, f.resolved_at, u.full_name submitter_name, u.email submitter_email, p.code project_code, p.name project_name,
    ...
    COALESCE(pkg.after_state->>'state','not-generated') package_state, pkg.id package_snapshot_event_id, COALESCE(alert.after_state->>'state','pending') reviewer_alert_state,
    ...
    ARRAY(SELECT DISTINCT nc.user_id FROM notification_channels nc JOIN users ru ON ru.id=nc.user_id WHERE nc.adapter_id=${adapterId} AND nc.provider='telegram' AND nc.status='connected' AND ru.is_super_admin=true ORDER BY nc.user_id) telegram_recipient_ids,
    ...
    LEFT JOIN LATERAL (SELECT after_state FROM feedback_audit_events WHERE feedback_id=f.id AND event_type='submission_notification_outbox_settled' ORDER BY id DESC LIMIT 1) alert ON true
...
router.get("/feedback/admin", authMiddleware, isSuperAdminMiddleware, async (_req, res) => {
  ...
  const telegramConfig=getTelegramProductConfig();const telegramRecipients=telegramConfig.configured?await db.select({userId:notificationChannelsTable.userId}).from(notificationChannelsTable).innerJoin(usersTable,eq(notificationChannelsTable.userId,usersTable.id)).where(and(eq(notificationChannelsTable.adapterId,telegramConfig.adapterId),eq(notificationChannelsTable.provider,"telegram"),eq(notificationChannelsTable.status,"connected"),eq(usersTable.isSuperAdmin,true))):[];const recipientIds=telegramRecipients.map(row=>row.userId);
  ...
  const alertEvents=feedbackIds.length?await db.select({feedbackId:feedbackAuditEventsTable.feedbackId,afterState:feedbackAuditEventsTable.afterState}).from(feedbackAuditEventsTable).where(and(inArray(feedbackAuditEventsTable.feedbackId,feedbackIds),eq(feedbackAuditEventsTable.eventType,"submission_notification_outbox_settled"))).orderBy(desc(feedbackAuditEventsTable.id)):[];
  const alertByFeedback=new Map<number,string>();for(const event of alertEvents)if(!alertByFeedback.has(event.feedbackId))alertByFeedback.set(event.feedbackId,String((event.afterState as Record<string,unknown>)?.state||"unknown"));
  ...
  return res.json({ feedback: rows.map(row => { const counts = evidence.get(row.id) || { total: 0, clean: 0, quarantined: 0, rejected: 0 },snapshot=snapshotByFeedback.get(row.id),telegram=telegramByFeedback.get(row.id)!; return { ...row, evidence: counts, packageState: counts.total === 0 ? "metadata-only" : counts.clean === counts.total ? "ready" : counts.rejected > 0 ? "rejected-evidence" : "awaiting-scan",packageSnapshot:packageSnapshotDto(snapshot?.state),reviewerAlertState:alertByFeedback.get(row.id)||"pending",telegramDeliveryState:telegram.overallState,telegramDelivery:telegram,reviewUrl:`/admin?tab=feedback&feedback=${encodeURIComponent(row.stableId)}` }; }) });
});
...
router.get("/feedback/admin/export.csv", authMiddleware, isSuperAdminMiddleware, async (req, res) => {
  ...
  res.setHeader("Content-Type", "text/csv; charset=utf-8"); res.setHeader("Content-Disposition", "attachment; filename=feedback-review-v1.60.35.10-F.csv");
...
router.get("/feedback/admin/follow-up.csv", authMiddleware, isSuperAdminMiddleware, async (req, res) => {
  ...
  res.setHeader("Content-Type", "text/csv; charset=utf-8"); res.setHeader("Content-Disposition", "attachment; filename=feedback-follow-up-v1.60.35.10-F.csv"); return res.send(buildFeedbackFollowUpCsv(rows,publicBaseUrl()));
});
