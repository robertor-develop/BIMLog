...
CREATE TABLE IF NOT EXISTS folder_wizard_imports(
 id text PRIMARY KEY,company_id integer NOT NULL REFERENCES companies(id),project_id integer NOT NULL REFERENCES projects(id),version integer NOT NULL,source_sha256 text NOT NULL,source_text text NOT NULL,imported_by_id integer NOT NULL REFERENCES users(id),imported_at timestamptz NOT NULL DEFAULT now(),
 CONSTRAINT folder_wizard_imports_scope_uq UNIQUE(id,company_id,project_id),CONSTRAINT folder_wizard_imports_version_uq UNIQUE(company_id,project_id,version),CONSTRAINT folder_wizard_imports_version_chk CHECK(version>0),CONSTRAINT folder_wizard_imports_sha_chk CHECK(source_sha256~'^[a-f0-9]{64}$'));
CREATE TABLE IF NOT EXISTS folder_wizard_current_imports(
