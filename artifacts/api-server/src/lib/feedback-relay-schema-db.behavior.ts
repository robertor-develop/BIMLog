...
try {
  ...
  const receiptRace = await Promise.allSettled(["r1","r2"].map(requestId => pool.query(`SELECT feedback_relay_record_receipt($1,2,'lease-a',1,'1',$2,'nonce',now(),'receiver-key',$3,'signature',now(),now(),$4)`,[jobId,requestId,canonical,`receipt-${requestId}`]))); assert.equal(receiptRace.filter(v=>v.status==='fulfilled').length,1);
  ...
  await assert.rejects(() => pool.query(`SELECT feedback_relay_mark_temporary_deleted($1,4,'lease-a',1,$2,now(),now(),now())`,[jobId,temporaryId]), /active hold/);
