...
if (proofOutput) { if (fs.existsSync(proofOutput)) throw new Error("Feedback PDF proof output already exists"); fs.mkdirSync(proofOutput, { recursive: false }); fs.writeFileSync(`${proofOutput}/FB-TEST-feedback.pdf`, built.pdf, { flag: "wx" }); }
...
assert.match(route, /feedback\/admin\/follow-up\.csv/); assert.match(route, /LEFT JOIN LATERAL.*feedback_audit_events/s); assert.match(route, /feedback-follow-up-v1\.60\.35\.09-F\.csv/);
...
assert.match(packageWorker, /package_snapshot_created/); assert.match(packageWorker, /sourceEventId/); assert.match(packageWorker, /manifestStoragePath/); assert.match(packageWorker, /pdfStoragePath/); assert.match(packageWorker, /pg_try_advisory_lock/); assert.match(packageWorker, /storage\.delete\(manifestPath\)/);
assert.match(route, /package-snapshot\.pdf/); assert.match(route, /package-snapshot\.json/); assert.match(route, /FEEDBACK_PACKAGE_SNAPSHOT_INTEGRITY_FAILED/);
console.log(JSON.stringify({ status: "PASS", tests: ["canonical-manifest", "human-pdf", "embedded-image", "zip-evidence", "customer-redaction", "secure-link", "fail-closed-integrity", "atomic-two-role-capture-bundle", "customer-authority-recheck", "internal-secure-download", "automatic-pdf-json-snapshot", "snapshot-advisory-lock", "snapshot-compensation", "snapshot-download-integrity"] }));
