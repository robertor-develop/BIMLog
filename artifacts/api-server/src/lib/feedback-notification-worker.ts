  ...
  for (const candidate of candidates) {
    const outcome = await db.transaction(async tx => {
      ...
      if (!reviewers.length) {
        ...
        if (!prior) {
          await tx.insert(feedbackAuditEventsTable).values({ feedbackId: feedback.id, actorUserId: feedback.userId, eventType: "submission_reviewer_escalation_required", afterState: { state: "no-active-reviewer", release: FEEDBACK_RELEASE } });
          await tx.insert(feedbackAuditEventsTable).values({ feedbackId: feedback.id, actorUserId: feedback.userId, eventType: "submission_notification_outbox_settled", afterState: { state: "blocked", reasonCode: "no-active-reviewer", release: FEEDBACK_RELEASE } });
        }
      ...
      const reviewerNotifications = await tx.insert(notificationsTable).values(reviewers.map(reviewer => ({ userId: reviewer.id, projectId: feedback.projectId, type: "feedback_review_requested", title: `New feedback ${feedback.stableId}`, message: `${feedback.feedbackType} feedback requires review.`, actionUrl: `/admin/feedback?feedback=${encodeURIComponent(feedback.stableId)}` }))).returning({ id: notificationsTable.id });
      await tx.insert(feedbackAuditEventsTable).values({ feedbackId: feedback.id, actorUserId: feedback.userId, eventType: "internal_reviewer_notifications_created", afterState: { sourceEventId: acknowledgment.id, notificationIds: reviewerNotifications.map(item => item.id), reviewerCount: reviewerNotifications.length, state: "created", reconciled: true, release: FEEDBACK_RELEASE } });
      await tx.insert(feedbackAuditEventsTable).values({ feedbackId: feedback.id, actorUserId: feedback.userId, eventType: "submission_notification_outbox_settled", afterState: { sourceEventId: acknowledgment.id, state: "delivered", reviewerCount: reviewerNotifications.length, reconciled: true, release: FEEDBACK_RELEASE } });
