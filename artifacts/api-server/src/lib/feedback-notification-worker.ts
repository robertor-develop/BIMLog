...
export async function reconcileFeedbackNotificationsOnce(limit = 100): Promise<{ inspected: number; delivered: number; awaitingReviewer: number }> {
  ...
  for (const candidate of candidates) {
    const outcome = await db.transaction(async tx => {
      ...
      if (!customerDelivery) { const [notification] = await tx.insert(notificationsTable).values({ userId: feedback.userId, projectId: feedback.projectId, type: "feedback_acknowledgment", title: `Feedback ${feedback.stableId} received`, message: "Your feedback was received and is ready for review.", actionUrl: "/feedback" }).returning({ id: notificationsTable.id }); await tx.insert(feedbackAuditEventsTable).values({ feedbackId: feedback.id, actorUserId: feedback.userId, eventType: "customer_notification_delivery", afterState: { sourceEventId: acknowledgment.id, notificationId: notification.id, channel: "in_app", state: "created", idempotencyKey: `ack:${feedback.id}` } }); }
