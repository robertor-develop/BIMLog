...
export async function getContractWorkspace(input: { actorUserId: number; projectId: unknown; contractId?: unknown }) {
  ...
  if (contractId && contracts[0]) {
    ...
    const historyRows = await pool.query(`SELECT actor_user_id,payment_application_id,payment_version_id,event_type,before_state,after_state,reason_code,evidence,occurred_at FROM financial_contract_history WHERE contract_id=$1 ORDER BY occurred_at,id`, [contractId]);
    detail = { ...contracts[0], lines: lines.rows.map((r: any) => ({ contractSovLineId: r.id, stableLineId: r.stable_line_id, budgetSnapshotLineId: r.budget_snapshot_line_id, projectCostNodeId: r.project_cost_node_id, projectCode: r.project_code, projectName: r.project_name, description: r.description, amount: exactPositiveAmount(String(r.amount)), contractItem: r.contract_item_snapshot ?? {}, budgetAmount: exactDelta(String(r.budget_amount)), schedule: r.schedule_item_placement_id == null ? null : { placementId: Number(r.schedule_item_placement_id), sourceType: r.schedule_source_type, sourceId: Number(r.schedule_source_id) } })), amendments: amendments.rows.map((r: any) => ({ id: r.id, bimlogId: r.bimlog_id, legalNumber: r.legal_number, versionId: r.version_id, version: Number(r.version), status: r.status, title: r.title, currency: r.currency, amountDelta: exactDelta(String(r.amount_delta)), contentFingerprint: r.content_fingerprint, revision: Number(r.revision), approvedAt: r.approved_at ? iso(r.approved_at) : null, executedAt: r.executed_at ? iso(r.executed_at) : null })), history: historyRows.rows.map((r: any) => ({ actorUserId: Number(r.actor_user_id), paymentApplicationId: r.payment_application_id, paymentVersionId: r.payment_version_id, eventType: r.event_type, beforeState: r.before_state, afterState: r.after_state, reasonCode: r.reason_code, evidence: r.evidence, occurredAt: iso(r.occurred_at) })) };
  }
