...
assert.match(page, /Saved APU version \(optional\)/);
assert.match(page, /immutable version\(s\) available/);
assert.match(page, /Selecting one binds the Contract Item to that exact version/);
