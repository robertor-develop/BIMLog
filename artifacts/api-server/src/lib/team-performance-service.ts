...
export async function getTeamPerformance(input: { actorUserId: unknown; projectId: unknown; from?: unknown; to?: unknown }, client: Queryable = pool) {
  ...
  const assignments = (await client.query(`SELECT r.user_id "userId",r.task_id "taskId",r.role,r.employment_type "employmentType",
      ...
      t.status,t.progress_percent "progress",w.id "workItemId",w.name "workItem",w.workflow_template "workflow"
  ...
  const deliverables = (await client.query(`SELECT COALESCE(r.user_id,t.assignee_user_id) "userId",COUNT(DISTINCT d.id)::int count
    ...
    GROUP BY COALESCE(r.user_id,t.assignee_user_id)`, [projectId])).rows;
  const packages = (await client.query(`SELECT p.responsible_user_id "userId",p.status,p.package_type "packageType",p.due_date "dueDate",p.updated_at "updatedAt"
    FROM job_activation_work_packages p WHERE p.project_id=$1 AND p.responsible_user_id IS NOT NULL AND p.status<>'cancelled'`, [projectId])).rows;

  const assignmentRows = [...assignments, ...directTasks];
  const deliverablesByMember = new Map(deliverables.map(row => [Number(row.userId), Number(row.count)]));
  ...
  const people = members.map(member => {
    ...
    const evidencePoints = uniqueTasks.length + memberTime.length + memberPackages.length + (deliverablesByMember.get(userId) ?? 0);
    const lastActivity = [
    ...
    return {
      ...
      delivery: { deliverables: deliverablesByMember.get(userId) ?? 0, approvedPackages, returnedPackages, overduePackages, qualityRate: ratio(approvedPackages, reviewed) },
  ...
  return {
    ...
    methodology: {
      ...
      limitations: "Observed categories are evidence labels, not inferred skill ratings. No AI ranking, personality score, or fabricated history is used.",
    },
