...
Deterministic 20-process fairness/crash-boundary evidence, PID-reuse simulation across a foreign live PID, authenticated generation-fenced backup manifests with concurrent mutation refusal, bounded asynchronous backup I/O, encrypted temporary-store quotas/TTL, persisted transfer leases, poison/dead-letter handling, operator-facing projection moves, bilingual customer status, mounted receiver HTTP endpoints, and deployed receiver availability remain pending and must not be inferred from the protocol or custody-core fixtures. Real receiver URLs, production certificates/keys, provider storage, deployment, and customer data remain external gates.
