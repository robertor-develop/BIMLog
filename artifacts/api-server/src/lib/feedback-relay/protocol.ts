...
export type RelayKey = { id: string; secret: Buffer; status: "active" | "grace" | "revoked"; notBefore: Date; notAfter: Date };
export type RelayKeyRing = { activeKeyId: string; keys: readonly RelayKey[] };
export type RequestContract = { version: "1"; method: string; path: string; audience: string; keyId: string; timestamp: string; nonce: string; feedbackId: string; objectId: string; byteCount: number; sha256: string; bodySha256: string };
export type ReceiptContract = { version: "1"; feedbackId: string; objectId: string; byteCount: number; sha256: string; destinationId: string; receivedAt: string; requestNonce: string; requestId: string; receiverKeyId: string };
export type ReadbackContract = { version: "1"; feedbackId: string; objectId: string; byteCount: number; sha256: string; destinationId: string; verifiedAt: string; receiptSha256: string; receiverKeyId: string };
export type Signed<T> = { payload: T; signature: string };
export interface NonceAuthority { consume(input: { audience: string; keyId: string; nonce: string; timestamp: string }): Promise<boolean>; }
...
export function canonicalRequest(p:RequestContract){return canonical("bimlog-feedback-relay-request-v1",[["version",p.version],["method",token(p.method.toUpperCase(),"method")],["path",token(p.path,"path")],["audience",token(p.audience,"audience")],["keyId",token(p.keyId,"keyId")],["timestamp",iso(p.timestamp,"timestamp")],["nonce",token(p.nonce,"nonce")],["feedbackId",token(p.feedbackId,"feedbackId")],["objectId",token(p.objectId,"objectId")],["byteCount",bytes(p.byteCount)],["sha256",digest(p.sha256,"sha256")],["bodySha256",digest(p.bodySha256,"bodySha256")]]);}
export function canonicalReceipt(p:ReceiptContract){return canonical("bimlog-feedback-relay-receipt-v1",[["version",p.version],["feedbackId",token(p.feedbackId,"feedbackId")],["objectId",token(p.objectId,"objectId")],["byteCount",bytes(p.byteCount)],["sha256",digest(p.sha256,"sha256")],["destinationId",token(p.destinationId,"destinationId")],["receivedAt",iso(p.receivedAt,"receivedAt")],["requestNonce",token(p.requestNonce,"requestNonce")],["requestId",token(p.requestId,"requestId")],["receiverKeyId",token(p.receiverKeyId,"receiverKeyId")]]);}
export function canonicalReadback(p:ReadbackContract){return canonical("bimlog-feedback-relay-readback-v1",[["version",p.version],["feedbackId",token(p.feedbackId,"feedbackId")],["objectId",token(p.objectId,"objectId")],["byteCount",bytes(p.byteCount)],["sha256",digest(p.sha256,"sha256")],["destinationId",token(p.destinationId,"destinationId")],["verifiedAt",iso(p.verifiedAt,"verifiedAt")],["receiptSha256",digest(p.receiptSha256,"receiptSha256")],["receiverKeyId",token(p.receiverKeyId,"receiverKeyId")]]);}
...
function keyAt(ring:RelayKeyRing,keyId:string,at:Date,signing:boolean){
  ...
  if(key.status==="revoked")throw new RelayProtocolError("FEEDBACK_RELAY_KEY_REVOKED","Relay key is revoked",{keyId});
...
export function verifyReceipt(signed:Signed<ReceiptContract>,ring:RelayKeyRing,at=new Date()){const key=keyAt(ring,signed.payload.receiverKeyId,at,false);if(!equalSignature(signed.signature,mac(canonicalReceipt(signed.payload),key.secret)))throw new RelayProtocolError("FEEDBACK_RELAY_SIGNATURE_INVALID","Receipt signature is invalid",{keyId:key.id});return signed.payload;}
