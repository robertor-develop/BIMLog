...
export type DurableReadbackVerification={version:"1";requestId:string;companyId:string;projectId:string;keyId:string;signedAt:string;canonicalSha256:string;signature:string;verifiedAt:string};
...
export function verifyReadbackDurably(signed:Signed<ReadbackContract>,ring:RelayKeyRing,verifiedAt=new Date()):DurableReadbackVerification{const signedAt=new Date(iso(signed.payload.verifiedAt,"verifiedAt"));if(signedAt>verifiedAt)throw new RelayProtocolError("FEEDBACK_RELAY_TIMESTAMP_SKEW","Readback timestamp is in the future");const key=keyAt(ring,signed.payload.receiverKeyId,signedAt,false,true);const value=canonicalReadback(signed.payload);if(!equalSignature(signed.signature,mac(value,key.secret)))throw new RelayProtocolError("FEEDBACK_RELAY_SIGNATURE_INVALID","Readback signature is invalid",{keyId:key.id});return{version:"1",requestId:signed.payload.requestId,companyId:signed.payload.companyId,projectId:signed.payload.projectId,keyId:key.id,signedAt:signed.payload.verifiedAt,canonicalSha256:sha256(value),signature:signed.signature,verifiedAt:verifiedAt.toISOString()};}
export function verifyReadback(signed:Signed<ReadbackContract>,ring:RelayKeyRing,at=new Date()){verifyReadbackDurably(signed,ring,at);return signed.payload;}
