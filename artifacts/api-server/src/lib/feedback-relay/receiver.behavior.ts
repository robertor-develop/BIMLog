...
const byteInventory=(inventoryRoot:string)=>{const rows:{relativePath:string;byteCount:number;sha256:string}[]=[];const walk=(dir:string)=>{for(const entry of fs.readdirSync(dir,{withFileTypes:true})){const target=path.join(dir,entry.name);if(entry.isDirectory())walk(target);else{const bytes=fs.readFileSync(target);rows.push({relativePath:path.relative(inventoryRoot,target).replaceAll("\\","/"),byteCount:bytes.length,sha256:sha256(bytes)});}}};walk(inventoryRoot);return rows.sort((a,b)=>a.relativePath.localeCompare(b.relativePath));};
try {
  ...
  await check(
    ...
    async () => {
      ...
      purgeAuthority.set(command);clockNow=new Date(now.getTime()+120_000);await code("FEEDBACK_RECEIVER_PURGE_COMMAND_STALE",()=>service.consumePurgeCommand("request-one",command,purgeAuthority,now));assert.deepEqual(await service.readDeletionReceipt(signedFirst),deleted);clockNow=new Date(deleted.absenceVerifiedAt);
      const beforeHistoricalReads=byteInventory(root);assert.deepEqual(await service.readDeletionReceipt(signedFirst),deleted);assert.deepEqual(await service.readDeletionReceipt(signedFirst),deleted);await code("FEEDBACK_RECEIVER_DELETION_RECEIPT_IDENTITY_MISMATCH",()=>service.readDeletionReceipt(signRequest({...firstRequest,objectId:"divergent-object"},senderKeys,now)));assert.deepEqual(byteInventory(root),beforeHistoricalReads);
    },
  );
