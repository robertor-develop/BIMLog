...
async function snapshotOne(candidate: Candidate, visibility: "customer" | "internal") {
  ...
  if (Number(prior?.sourceEventId) === candidate.source_event_id) return false;
  ...
  try {
    ...
    const inserted = await db.transaction(async tx => {
      const latest = await tx.execute(sql`SELECT max(id)::integer source_event_id FROM feedback_audit_events WHERE feedback_id=${candidate.id} AND event_type NOT IN ('package_snapshot_created','admin_package_exported','admin_package_snapshot_exported','admin_exported','admin_follow_up_exported','admin_asset_exported')`);
      ...
      const [winner] = await tx.select({ id: feedbackAuditEventsTable.id }).from(feedbackAuditEventsTable)
        .where(and(eq(feedbackAuditEventsTable.feedbackId, candidate.id), eq(feedbackAuditEventsTable.eventType, SNAPSHOT_EVENT), sql`${feedbackAuditEventsTable.afterState}->>'visibility'=${visibility}`, sql`${feedbackAuditEventsTable.afterState}->>'sourceEventId'=${String(candidate.source_event_id)}`)).limit(1);
    ...
    if (!inserted) { await storage.delete(manifestPath); await storage.delete(pdfPath); }
    return inserted;
  } catch (error) {
...
export async function reconcileFeedbackPackageSnapshotsOnce(limit = DEFAULT_BATCH_SIZE) {
  const result = await db.execute(sql`SELECT f.id,f.user_id,f.project_id,source.source_event_id
    ...
    JOIN LATERAL (SELECT max(e.id)::integer source_event_id FROM feedback_audit_events e WHERE e.feedback_id=f.id AND e.event_type NOT IN ('package_snapshot_created','admin_package_exported','admin_package_snapshot_exported','admin_exported','admin_follow_up_exported','admin_asset_exported')) source ON source.source_event_id IS NOT NULL
    WHERE NOT EXISTS (SELECT 1 FROM feedback_audit_events s WHERE s.feedback_id=f.id AND s.event_type='package_snapshot_created' AND (s.after_state->>'sourceEventId')::integer=source.source_event_id AND s.after_state->>'visibility'='customer')
       OR NOT EXISTS (SELECT 1 FROM feedback_audit_events s WHERE s.feedback_id=f.id AND s.event_type='package_snapshot_created' AND (s.after_state->>'sourceEventId')::integer=source.source_event_id AND s.after_state->>'visibility'='internal')
