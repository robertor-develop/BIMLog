...
assert.match(browser, /Line notes/);
assert.match(browser, /line\.notes\.trim\(\)\.length < 1/);
check("initial Intake and APU estimate draft", "one or more exact lines with required notes remain bound to the pinned structure");
