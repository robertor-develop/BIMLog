...
check(9, "preference restricts but never grants", () => { const f={...get("notifications.deterministic"),capabilityStatus:"available" as const}; assert.equal(resolveEntitlement(f,{...active,userPreference:{enabled:false,version:1}}).code,"ENT_USER_DISABLED"); const denied=resolveEntitlement(f,{...active,platform:{status:"suspended",version:2},userPreference:{enabled:true,version:2}}); return code(denied,"ENT_TEMP_SUSPENDED"); });
