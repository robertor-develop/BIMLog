...
CREATE TABLE IF NOT EXISTS generic_cost_value_forecast_versions(
  id text PRIMARY KEY,
  project_id integer NOT NULL REFERENCES projects(id),
  plan_version_id text NOT NULL REFERENCES generic_cost_value_plan_versions(id),
  performance_version_id text NOT NULL REFERENCES generic_cost_value_performance_versions(id),
  version integer NOT NULL,
  content jsonb NOT NULL,
  evaluation jsonb NOT NULL,
  content_fingerprint text NOT NULL,
  supersedes_id text REFERENCES generic_cost_value_forecast_versions(id),
  created_by_id integer NOT NULL REFERENCES users(id),
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT generic_cost_value_forecast_version_positive_chk CHECK(version > 0)
);
CREATE UNIQUE INDEX IF NOT EXISTS generic_cost_value_forecast_project_version_uidx ON generic_cost_value_forecast_versions(project_id,version);
CREATE INDEX IF NOT EXISTS generic_cost_value_forecast_project_latest_idx ON generic_cost_value_forecast_versions(project_id,version DESC);
`);
    await client.query(`
...
DO $generic_apu_triggers$ BEGIN
  ...
  END IF;
  IF NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgname='generic_cost_value_forecast_versions_immutable' AND tgrelid='generic_cost_value_forecast_versions'::regclass) THEN
    CREATE TRIGGER generic_cost_value_forecast_versions_immutable BEFORE UPDATE OR DELETE ON generic_cost_value_forecast_versions FOR EACH ROW EXECUTE FUNCTION reject_generic_apu_history_mutation();
  END IF;
