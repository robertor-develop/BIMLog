...
for (const phrase of ["Project Controls Dashboard & Forecast", "Panel de Control y Pronóstico del Proyecto", "Export CSV", "Exportar CSV", "Phase / scope item", "Fase / partida", "Team member scope", "Alcance del miembro", "No AI invents missing data", "La IA no inventa datos faltantes"]) assert.match(ui, new RegExp(phrase));
assert.match(ui, /PrintPdfButton/);
assert.match(ui, /downloadGovernedCurrentViewPdf/);
assert.match(ui, /surface:\s*"job-operations"/);
assert.match(ui, /rows:\s*rows\.map/);
assert.doesNotMatch(ui, /window\.print|window\.open/);
