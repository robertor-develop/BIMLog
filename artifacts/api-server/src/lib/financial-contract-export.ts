...
export async function buildContractCurrentViewPdf(data: ContractCurrentViewExport): Promise<Buffer> {
  ...
  const header = () => {
    doc.fontSize(14).font("Helvetica-Bold").fillColor("#123f68").text(safe(data.project.companyName, "BIMLog"), 40, 36, { width: pageWidth * 0.42, lineBreak: false, ellipsis: true });
    doc.fontSize(14).font("Helvetica-Bold").fillColor("#123f68").text("Contracts & Commitments — Current View", 40 + pageWidth * 0.42, 36, { width: pageWidth * 0.58, align: "right", lineBreak: false, ellipsis: true });
