...
CREATE TABLE IF NOT EXISTS financial_contract_payment_versions(id text PRIMARY KEY,payment_application_id text NOT NULL,contract_version_id text NOT NULL,supersedes_id text,version integer NOT NULL,revision integer NOT NULL DEFAULT 1,status text NOT NULL,period_start date NOT NULL,period_end date NOT NULL,currency text NOT NULL,gross_amount numeric(30,6) NOT NULL,retainage_amount numeric(30,6) NOT NULL,net_amount numeric(30,6) NOT NULL,content_fingerprint text NOT NULL,prepared_by_id integer NOT NULL,submitted_by_id integer,reviewed_by_id integer,approved_by_id integer,submitted_at timestamptz,reviewed_at timestamptz,approved_at timestamptz,outcome_reason text,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),CONSTRAINT fin_contract_payment_status_chk CHECK(status IN('draft','submitted','approved','returned','rejected','withdrawn','voided')),CONSTRAINT fin_contract_payment_currency_chk CHECK(currency~'^[A-Z]{3}$'),CONSTRAINT fin_contract_payment_amounts_chk CHECK(gross_amount>=0 AND retainage_amount>=0 AND net_amount>=0 AND net_amount=gross_amount-retainage_amount),CONSTRAINT fin_contract_payment_period_chk CHECK(period_end>=period_start),CONSTRAINT fin_contract_payment_versions_app_version_key UNIQUE(payment_application_id,version),CONSTRAINT fin_contract_payment_versions_app_id_fkey FOREIGN KEY(payment_application_id) REFERENCES financial_contract_payment_applications(id),CONSTRAINT fin_contract_payment_versions_contract_ver_fkey FOREIGN KEY(contract_version_id) REFERENCES financial_contract_versions(id),CONSTRAINT fin_contract_payment_versions_supersedes_fkey FOREIGN KEY(supersedes_id) REFERENCES financial_contract_payment_versions(id),CONSTRAINT fin_contract_payment_versions_prepared_by_fkey FOREIGN KEY(prepared_by_id) REFERENCES users(id),CONSTRAINT fin_contract_payment_versions_submitted_by_fkey FOREIGN KEY(submitted_by_id) REFERENCES users(id),CONSTRAINT fin_contract_payment_versions_reviewed_by_fkey FOREIGN KEY(reviewed_by_id) REFERENCES users(id),CONSTRAINT fin_contract_payment_versions_approved_by_fkey FOREIGN KEY(approved_by_id) REFERENCES users(id));
...
ALTER TABLE financial_contract_payment_versions ADD COLUMN IF NOT EXISTS reviewed_by_id integer;
ALTER TABLE financial_contract_payment_versions ADD COLUMN IF NOT EXISTS reviewed_at timestamptz;
ALTER TABLE financial_contract_payment_versions ADD COLUMN IF NOT EXISTS supersedes_id text;
`);
    await client.query(`
DO $$ BEGIN
 ...
 IF NOT EXISTS(SELECT 1 FROM pg_constraint WHERE conname='fin_contract_payment_versions_reviewed_by_fkey') THEN ALTER TABLE financial_contract_payment_versions ADD CONSTRAINT fin_contract_payment_versions_reviewed_by_fkey FOREIGN KEY(reviewed_by_id) REFERENCES users(id) NOT VALID; ALTER TABLE financial_contract_payment_versions VALIDATE CONSTRAINT fin_contract_payment_versions_reviewed_by_fkey; END IF;
 IF NOT EXISTS(SELECT 1 FROM pg_constraint WHERE conname='fin_contract_payment_versions_supersedes_fkey') THEN ALTER TABLE financial_contract_payment_versions ADD CONSTRAINT fin_contract_payment_versions_supersedes_fkey FOREIGN KEY(supersedes_id) REFERENCES financial_contract_payment_versions(id) NOT VALID; ALTER TABLE financial_contract_payment_versions VALIDATE CONSTRAINT fin_contract_payment_versions_supersedes_fkey; END IF;
END $$;
`);
    await client.query(`
DO $$ BEGIN
...
CREATE OR REPLACE FUNCTION guard_financial_payment_version_mutation() RETURNS trigger LANGUAGE plpgsql AS $$ DECLARE governed_contract text; exceeded boolean; BEGIN
 ...
 IF OLD.status='returned' THEN RAISE EXCEPTION 'returned payment application version is immutable; create a successor'; END IF;
 ...
 IF NEW.reviewed_by_id IS NOT NULL AND NEW.reviewed_by_id=NEW.prepared_by_id THEN RAISE EXCEPTION 'payment application maker and reviewer must be separate'; END IF;
 IF NEW.status='approved' AND OLD.status<>'approved' THEN
  IF NEW.reviewed_by_id IS NULL THEN RAISE EXCEPTION 'payment application requires attributed review before approval'; END IF;
  IF NEW.approved_by_id=NEW.prepared_by_id OR NEW.approved_by_id=NEW.reviewed_by_id THEN RAISE EXCEPTION 'payment application maker reviewer and approver must be separate'; END IF;
