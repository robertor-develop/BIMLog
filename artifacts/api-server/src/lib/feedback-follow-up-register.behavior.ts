...
const workbook = XLSX.read(bytes, { type: "buffer" }), sheet = workbook.Sheets["Feedback follow-up"]; assert.equal(sheet.Z2.l?.Target, "https://bimlog.test/admin?tab=feedback&feedback=FB-ED3259007843"); assert.equal(sheet.D2.v, "Review scanner quarantine"); assert.equal(sheet.P2.v, "1");
console.log("feedback follow-up register: 9/9 passed");
