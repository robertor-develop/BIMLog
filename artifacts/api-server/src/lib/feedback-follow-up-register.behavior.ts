...
const row = { stable_id: "FB-ED3259007843", status: "new", priority: "urgent", feedback_type: "bug", module: "RFI", project_code: "P1", project_name: "Tower", submitter_name: "Customer", submitter_email: "=unsafe@example.test", owner_user_id: null, target_release: "v1.60.35.12-F", disposition_reason: null, customer_visible: true, evidence_total: 2, evidence_clean: 1, evidence_quarantined: 1, evidence_rejected: 0, package_state: "awaiting-scan", reviewer_alert_state: "pending", telegram_delivery_state: "partial", telegram_delivery_outcomes:[{recipientUserId:7,artifactKind:"docx",state:"sent"},{recipientUserId:7,artifactKind:"xlsx",state:"missing"}], created_at: new Date("2026-08-21T00:00:00Z"), updated_at: new Date("2026-08-21T01:00:00Z"), resolved_at: null, last_event_type: "created", last_event_at: new Date("2026-08-21T00:00:01Z") };
assert.equal(feedbackReviewUrl("https://bimlog.test", row.stable_id), "https://bimlog.test/admin/feedback?feedback=FB-ED3259007843"); assert.equal(feedbackNextAction(row), "Review scanner quarantine");
assert.equal(feedbackNextAction({...row,evidence_quarantined:0,reviewer_alert_state:"delivered"}),"Review Telegram delivery");
const csv = buildFeedbackFollowUpCsv([row], "https://bimlog.test"); assert.match(csv, /Evidence quarantined/); assert.match(csv, /admin\/feedback\?feedback=FB-ED3259007843/); assert.doesNotMatch(csv, /,"=unsafe/);
