...
function assertNoLinks(target: string) {
  ...
  for (const segment of absolute.slice(parsed.root.length).split(path.sep).filter(Boolean)) {
    ...
    const stat = fs.lstatSync(cursor); if (stat.isSymbolicLink() || (stat.mode & 0o170000) === 0o120000) throw new Error("STORAGE_REPARSE_COMPONENT_DENIED");
  }
}
...
export class LocalDiskStorageAdapter implements StorageAdapter {
  ...
  constructor(root: string, options: Options = {}) {
    ...
    this.root = path.resolve(root); this.backendId = options.backendId ?? "local-test"; this.backendType = options.backendType ?? "local-test"; this.capabilities = options.capabilities ?? PROVEN; this.faultAt = options.faultAt;
    if (!/^[a-z0-9][a-z0-9._-]{2,63}$/i.test(this.backendId) || this.capabilities.some(value => !PROVEN.includes(value))) throw new Error("STORAGE_BINDING_INVALID");
...
export function createStorageFromEnvironment(environment: NodeJS.ProcessEnv = process.env): StorageAdapter {
  ...
  if (backend === "durable-filesystem") {
    const backendId = environment.BIMLOG_FEEDBACK_STORAGE_BACKEND_ID, authorityPath = environment.BIMLOG_FEEDBACK_STORAGE_AUTHORITY_MANIFEST; if (!backendId) throw new Error("FEEDBACK_STORAGE_BACKEND_ID_REQUIRED");
    ...
    let capabilities: readonly Capability[] = PROVEN;
    if (production) { let manifest: Record<string, unknown>; try { manifest = JSON.parse(fs.readFileSync(path.resolve(authorityPath!), "utf8")); } catch { throw new Error("FEEDBACK_STORAGE_AUTHORITY_INVALID"); } const declared = Array.isArray(manifest.capabilities) ? manifest.capabilities : []; if (manifest.schemaVersion !== 1 || manifest.backendId !== backendId || path.resolve(String(manifest.dataRoot ?? "")) !== path.resolve(root) || manifest.backupRequired !== true || declared.some(value => !PROVEN.includes(value as Capability))) throw new Error("FEEDBACK_STORAGE_AUTHORITY_INVALID"); capabilities = declared as Capability[]; }
    return new LocalDiskStorageAdapter(path.resolve(root), { backendId, backendType: "durable-filesystem", capabilities });
  }
