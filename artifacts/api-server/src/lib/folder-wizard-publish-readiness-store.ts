...
export function createFolderWizardPublishReadinessStore(database?: Pool) {
  const connect = async () => (database ?? ((await import("@workspace/db")).pool as unknown as Pool)).connect();
  return {
    async read(scope: { projectId: number; actorUserId: number }, providerIdentity: { siteUrl: string; libraryId: string } | null = null) {
      ...
      try {
        ...
        const mapping = (await client.query(`SELECT m.state,m.site_id,m.library_id,c.state AS credential_state
        ...
        return evaluateFolderWizardPublishReadiness({
