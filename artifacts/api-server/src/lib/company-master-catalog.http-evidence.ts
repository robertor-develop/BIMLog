...
try {
  ...
  assert.equal(cap.body.canManage,true); assert.equal(cap.body.isSuperAdmin,false);
  ...
  console.log("Company master catalog local HTTP: migration, grant, four-kind API, tenant isolation, optimistic version, client identity, revoke PASS");
} finally {
