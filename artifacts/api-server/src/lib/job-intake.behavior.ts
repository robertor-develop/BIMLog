...
assert.match(service, /SELECT \* FROM job_intakes WHERE project_id=\$1 FOR UPDATE[\s\S]*JOB_INTAKE_STALE/);
assert.match(service, /jobIntakeCoreFingerprint\(normalizeJobIntakeData\(row\.data\)\)/);
assert.match(service, /if \(row\.status === "activated" && row\.activated_contract_id\)/);
...
assert.equal(completion.readinessSummary.work.plannedHours, completion.totals.plannedHours);
assert.equal(completion.readinessSummary.work.assignedHours, completion.totals.assignedHours);
assert.equal(completion.readinessSummary.work.assignmentCoveragePercent, 100);
assert.equal(typeof completion.readinessSummary.optionalItemsRemaining, "number");
assert.equal(completion.readinessSummary.financial.applicable, true);
