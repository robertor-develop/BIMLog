...
const { idempotencyKey: _createOnly, ...revisionInput } = base;
const revision = normalizePaymentRevision(revisionInput);
assert.match(revision.contentFingerprint, /^[a-f0-9]{64}$/);
assert.equal(revision.applicationNumber, base.applicationNumber);
