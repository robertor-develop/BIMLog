...
const results = [];
const run = (name, exe, args, options = {}) => { const r = spawnSync(exe, args, { cwd: root, encoding: "utf8", ...options }); process.stdout.write(r.stdout || ""); process.stderr.write(r.stderr || ""); results.push({ name, exitCode: r.status }); if (r.status !== 0) process.exit(r.status || 1); };
...
run("document-connections-regression", "cmd.exe", ["/d","/s","/c","pnpm","--filter","@workspace/api-server","exec","tsx","./src/lib/job-document-connections.behavior.ts"], {
  env: { ...process.env, PROD_DATABASE_URL: "postgresql://static-test:static-test@127.0.0.1:1/static-test" },
});
