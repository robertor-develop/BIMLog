...
const focused = [
  "post-p17-build26-rfi-identity-scope.behavior.mjs",
  "post-p17-build27-rfi-lifecycle-history.behavior.mjs",
  "post-p17-build28-rfi-attachment-integrity.behavior.mjs",
  "post-p17-build29-rfi-workflow-ui.behavior.mjs",
];
...
run("rfi-import-regression", "cmd.exe", ["/d", "/s", "/c", "pnpm", "--filter", "@workspace/api-server", "run", "test:rfi-import"]);
...
const output = path.join(root, "evidence", "post-p17-builds26-30-acceptance");
fs.mkdirSync(output, { recursive: true });
fs.writeFileSync(path.join(output, "results.json"), JSON.stringify({
  status: "PASS",
  version: "v1.05.N17-P17",
  results,
  browserEvidence: "evidence/post-p17-build29-rfi-chrome/acceptance.json",
...
console.log("POST-P17 Builds 26-30 consolidated acceptance: PASS");
