...
export function reconciliationPlan(development, production) {
  const plan = [];
  const devConstraints = new Map(development.constraints.map((row) => [`${row.table_name}|${normalizedConstraint(row.definition)}`, row]));
  for (const row of production.constraints) {
    const match = devConstraints.get(`${row.table_name}|${normalizedConstraint(row.definition)}`);
    if (match && match.name !== row.name) plan.push({ kind: "constraint", tableName: row.table_name, from: match.name, to: row.name });
  }
