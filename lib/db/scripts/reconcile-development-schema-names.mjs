...
export function reconciliationPlan(development, production) {
  ...
  const developmentConstraintNames = new Set(development.constraints.map((row) => row.name));
  const productionConstraintNames = new Set(production.constraints.map((row) => row.name));
  const devIndexes = new Map(development.indexes
    .filter((row) => !developmentConstraintNames.has(row.name))
    .map((row) => [`${row.table_name}|${normalizedIndex(row.definition)}`, row]));
  for (const row of production.indexes.filter((item) => !productionConstraintNames.has(item.name))) {
