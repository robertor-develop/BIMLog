...
export function reconciliationPlan(development, production) {
  ...
  for (const row of production.constraints) {
    const matches = devConstraints.get(`${row.table_name}|${normalizedConstraint(row.definition)}`) ?? [];
    const canonical = matches.find((match) => match.name === row.name) ?? matches[0];
    if (canonical && canonical.name !== row.name) plan.push({ kind: "constraint", tableName: row.table_name, from: canonical.name, to: row.name });
    for (const duplicate of matches) {
      if (duplicate !== canonical) plan.push({ kind: "drop_redundant_constraint", tableName: row.table_name, name: duplicate.name });
    }
  }
...
export async function main() {
  ...
  try {
    ...
    const plan = reconciliationPlan(developmentInventory, productionInventory);
