...
const databaseToolErrorPatterns = [
  ...
  /(?:^|\n)\s*(?:typeerror|rangeerror|syntaxerror)\s*:/im,
...
export function main() {
  ...
  // Replit's publisher compares object names as well as structure. Normalize
  // already-equivalent development objects before drizzle attempts its push;
  // otherwise drizzle can try to create a production-named constraint while
  // the equivalent development-named constraint still exists.
  const prePushReconcileNames = spawnSync(
    process.execPath,
    [fileURLToPath(new URL("./reconcile-development-schema-names.mjs", import.meta.url)), "--duplicates-only"],
    { env: process.env, stdio: "inherit" },
  );
  if (prePushReconcileNames.status !== 0) return prePushReconcileNames.status ?? 1;
  ...
  const reconcileNames = spawnSync(
    process.execPath,
    [fileURLToPath(new URL("./reconcile-development-schema-names.mjs", import.meta.url))],
    { env: process.env, stdio: "inherit" },
  );
  if (reconcileNames.status !== 0) return reconcileNames.status ?? 1;
