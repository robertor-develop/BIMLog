...
export const requiredConstraints = [
  ...
  {
    name: "fc_import_confirmed_version_uk",
    tableName: "financial_contract_import_sessions",
    type: "u",
...
export function evaluateParity(expected, actualTables, actualIndexes, actualConstraints) {
  ...
  for (const required of requiredConstraints) {
    ...
    if (
      ...
      constraintProblems.push(`unexpected definition for constraint ${required.name}`);
    }
  }
...
async function main() {
  ...
  try {
    ...
    const { missingTables, missingIndexes, constraintProblems } = evaluateParity(
      expected,
      actualTables,
      actualIndexes,
      actualConstraints,
    ...
    if (missingTables.length || missingIndexes.length || constraintProblems.length) {
