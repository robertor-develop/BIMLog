...
const preservationQueries = {
  tables: `SELECT tablename AS name FROM pg_catalog.pg_tables WHERE schemaname='public'`,
  constraints: `SELECT t.relname AS table_name,c.conname AS name,
...
async function readPreservationCatalog(client) {
  await client.query("BEGIN READ ONLY");
  try {
    const result = {};
    for (const [kind, query] of Object.entries(preservationQueries)) {
      result[kind] = (await client.query(query)).rows;
    }
...
export function evaluateProductionPreservation(development, production) {
  const problems = [];
  for (const kind of Object.keys(preservationQueries)) {
    const singular = { tables: "table", constraints: "constraint", indexes: "index", columns: "column" }[kind];
    const identity = (row) => kind === "columns"
      ? `${row.table_name}.${row.column_name}`
      : kind === "tables" ? row.name
      : `${row.table_name}.${row.name}`;
    const actual = new Map(development[kind].map((row) => [identity(row), row]));
    for (const expected of production[kind]) {
      const name = identity(expected);
      if (!actual.has(name)) {
        problems.push(`missing production ${singular} ${name} in development`);
      } else if (JSON.stringify(actual.get(name)) !== JSON.stringify(expected)) {
        problems.push(`changed production ${singular} ${name} in development`);
      }
    }
    const expected = new Set(production[kind].map(identity));
    for (const row of development[kind]) {
      const name = identity(row);
      if (!expected.has(name)) problems.push(`development-only ${singular} ${name}`);
    }
  }
  return problems;
}
...
async function main() {
  ...
  try {
    ...
    if (
      ...
      console.error("Do not Publish until the schemas coincide.");
      process.exitCode = 1;
      return;
    }
    ...
    try {
      const developmentCatalog = await readPreservationCatalog(client);
      const productionCatalog = await readPreservationCatalog(productionClient);
      const preservationProblems = evaluateProductionPreservation(
        developmentCatalog,
        productionCatalog,
      );
      if (preservationProblems.length) {
        console.error("Replit production-object preservation FAILED.");
        for (const problem of preservationProblems) console.error(problem);
        console.error("Do not Publish until the schemas coincide.");
        process.exitCode = 1;
        return;
      }
      console.log(
        `Replit exact schema correspondence: passed (${productionCatalog.tables.length} tables, ${productionCatalog.constraints.length} constraints, ${productionCatalog.indexes.length} indexes, ${productionCatalog.columns.length} columns).`,
      );
    } finally {
