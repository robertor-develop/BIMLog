...
export function evaluateProductionPreservation(development, production) {
  ...
  for (const kind of Object.keys(preservationQueries)) {
    const singular = { constraints: "constraint", indexes: "index", columns: "column" }[kind];
  ...
  try {
    ...
    try {
      ...
      if (preservationProblems.length) {
        ...
        console.error("Do not approve the Publish migration preview.");
        process.exitCode = 1;
        return;
      }
      console.log(
        `Replit production-object preservation: passed (${productionCatalog.constraints.length} constraints, ${productionCatalog.indexes.length} indexes, ${productionCatalog.columns.length} columns).`,
      );
    } finally {
