...
export const feedbackRelayNoncesTable = pgTable(
  ...
  (table) => ({
    authorityIdx: uniqueIndex("feedback_relay_nonces_authority_idx").on(
      ...
      table.requestHash,
      table.nonce,
    ).where(sql`${table.authorityVersion}='current'`),
    requestIdx: uniqueIndex("feedback_relay_nonces_request_idx").on(
      table.receiverId,
      table.direction,
      table.requestId,
    ).where(sql`${table.authorityVersion}='current'`),
    legacyAuthorityIdx: uniqueIndex("feedback_relay_nonces_legacy_authority_idx").on(table.audience, table.keyId, table.nonce).where(sql`${table.authorityVersion}='legacy'`),
    legacyRequestIdx: uniqueIndex("feedback_relay_nonces_legacy_request_idx").on(table.requestId).where(sql`${table.authorityVersion}='legacy'`),
    ...
    authorityVersionCheck: check("feedback_relay_nonces_authority_version_chk", sql`${table.authorityVersion} IN ('legacy','current')`),
