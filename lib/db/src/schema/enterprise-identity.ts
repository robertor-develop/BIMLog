...
export const enterpriseContactsTable = pgTable(
  ...
  (t) => [
    ...
    foreignKey({
      ...
      name: "enterprise_contacts_company_fk",
    }),
    foreignKey({
      columns: [t.linkedUserId],
      foreignColumns: [usersTable.id],
      name: "enterprise_contacts_user_fk",
    }),
    foreignKey({
      columns: [t.createdById],
      foreignColumns: [usersTable.id],
      name: "enterprise_contacts_creator_fk",
    }),
    check(
      "enterprise_contacts_state_chk",
      sql`${t.state} IN ('active','inactive','retired')`,
    ),
    check(
      "enterprise_contacts_lifecycle_chk",
...
export const projectCompanyRelationshipsTable = pgTable(
  ...
  (t) => [
    ...
    unique("project_company_relationship_id_project_company_uq").on(
      t.id,
      t.projectId,
      t.companyId,
    ),
    unique("project_company_relationship_project_company_role_uq").on(
      t.projectId,
      t.companyId,
      t.relationshipType,
    ),
    foreignKey({
      ...
      name: "project_company_relationship_project_fk",
    }),
    foreignKey({
      columns: [t.companyId],
      foreignColumns: [companiesTable.id],
      name: "project_company_relationship_company_fk",
    }),
    foreignKey({
      columns: [t.createdById],
      foreignColumns: [usersTable.id],
      name: "project_company_relationship_creator_fk",
    }),
    check(
      "project_company_relationship_type_chk",
      sql`${t.relationshipType} IN ('client','owner','general_contractor','service_provider','trade_contractor','consultant','vendor','partner','authority','other')`,
    ),
    check(
      "project_company_relationship_state_chk",
      sql`${t.state} IN ('active','inactive','retired')`,
    ),
    check(
      "project_company_relationship_lifecycle_chk",
      sql`(${t.state} = 'retired') = (${t.retiredAt} IS NOT NULL)`,
    ),
  ],
);
...
export const projectContactRelationshipsTable = pgTable(
  "project_contact_relationships",
  {
    ...
    projectId: integer("project_id").notNull(),
    companyId: integer("company_id").notNull(),
    projectCompanyRelationshipId: integer(
      "project_company_relationship_id",
    ).notNull(),
  ...
  (t) => [
    unique("project_contact_relationship_project_contact_role_uq").on(
      t.projectId,
      t.contactId,
      t.contactRole,
    ),
    ...
    foreignKey({
      ...
      foreignColumns: [
        projectCompanyRelationshipsTable.id,
        projectCompanyRelationshipsTable.projectId,
        projectCompanyRelationshipsTable.companyId,
      ],
      name: "project_contact_relationship_project_company_fk",
    }),
    foreignKey({
      columns: [t.contactId, t.companyId],
      foreignColumns: [
        enterpriseContactsTable.id,
        enterpriseContactsTable.companyId,
      ],
      name: "project_contact_relationship_contact_company_fk",
    }),
    foreignKey({
      columns: [t.createdById],
      foreignColumns: [usersTable.id],
      name: "project_contact_relationship_creator_fk",
    }),
    check(
      "project_contact_relationship_state_chk",
      sql`${t.state} IN ('active','inactive','retired')`,
    ),
  ],
);
...
export const enterpriseTradesTable = pgTable(
  ...
  (t) => [
    foreignKey({
      columns: [t.createdById],
      foreignColumns: [usersTable.id],
      name: "enterprise_trades_creator_fk",
    }),
    check(
      "enterprise_trades_code_chk",
      sql`${t.code} ~ '^[A-Z0-9][A-Z0-9._-]{0,63}$'`,
    ),
    check(
      "enterprise_trades_state_chk",
      sql`${t.state} IN ('active','inactive','retired')`,
    ),
  ],
);

function enterpriseClassificationColumns() {
  return {
...
export const enterpriseServicesTable = pgTable(
  "enterprise_services",
  () => enterpriseClassificationColumns(),
  (t) => [
    foreignKey({
      columns: [t.createdById],
      foreignColumns: [usersTable.id],
      name: "enterprise_services_creator_fk",
    }),
    foreignKey({
      columns: [t.updatedById],
      foreignColumns: [usersTable.id],
      name: "enterprise_services_updater_fk",
    }),
    check(
      "enterprise_services_code_chk",
      sql`${t.code} ~ '^[A-Z0-9][A-Z0-9._-]{0,63}$'`,
    ),
    check(
      "enterprise_services_state_chk",
      sql`${t.state} IN ('active','inactive','retired')`,
    ),
    check("enterprise_services_version_chk", sql`${t.version} > 0`),
    check(
      "enterprise_services_lifecycle_chk",
      sql`(${t.state} = 'retired') = (${t.retiredAt} IS NOT NULL)`,
    ),
  ],
);
export const enterprisePhasesTable = pgTable(
  "enterprise_phases",
  () => enterpriseClassificationColumns(),
  (t) => [
    foreignKey({
      columns: [t.createdById],
      foreignColumns: [usersTable.id],
      name: "enterprise_phases_creator_fk",
    }),
    foreignKey({
      columns: [t.updatedById],
      foreignColumns: [usersTable.id],
      name: "enterprise_phases_updater_fk",
    }),
    check(
      "enterprise_phases_code_chk",
      sql`${t.code} ~ '^[A-Z0-9][A-Z0-9._-]{0,63}$'`,
    ),
    check(
      "enterprise_phases_state_chk",
      sql`${t.state} IN ('active','inactive','retired')`,
    ),
    check("enterprise_phases_version_chk", sql`${t.version} > 0`),
    check(
      "enterprise_phases_lifecycle_chk",
      sql`(${t.state} = 'retired') = (${t.retiredAt} IS NOT NULL)`,
    ),
  ],
);
...
export const companyTradeRelationshipsTable = pgTable(
  ...
  (t) => [
    unique("company_trade_relationship_project_company_trade_uq").on(
      t.projectId,
      t.companyId,
      t.tradeId,
    ),
    foreignKey({
      ...
      name: "company_trade_relationship_project_company_fk",
    }),
    foreignKey({
      columns: [t.tradeId],
      foreignColumns: [enterpriseTradesTable.id],
      name: "company_trade_relationship_trade_fk",
    }),
    foreignKey({
      columns: [t.createdById],
      foreignColumns: [usersTable.id],
      name: "company_trade_relationship_creator_fk",
    }),
    check(
      "company_trade_relationship_state_chk",
      sql`${t.state} IN ('active','inactive','retired')`,
    ),
  ],
);
...
export const contractPartyRelationshipsTable = pgTable(
  ...
  (t) => [
    unique("contract_party_relationship_contract_company_role_uq").on(
      t.contractId,
      t.projectCompanyRelationshipId,
      t.partyRole,
    ),
    foreignKey({
      columns: [t.contractId, t.projectId],
      foreignColumns: [
        financialContractsTable.id,
        financialContractsTable.projectId,
      ],
      name: "contract_party_relationship_contract_project_fk",
    }),
    foreignKey({
      columns: [t.projectCompanyRelationshipId, t.projectId],
      foreignColumns: [
        projectCompanyRelationshipsTable.id,
        projectCompanyRelationshipsTable.projectId,
      ],
      name: "contract_party_relationship_project_company_fk",
    }),
    foreignKey({
      columns: [t.createdById],
      foreignColumns: [usersTable.id],
      name: "contract_party_relationship_creator_fk",
    }),
    check(
      "contract_party_relationship_role_chk",
      sql`${t.partyRole} IN ('client','owner','contractor','subcontractor','consultant','vendor','guarantor','other')`,
    ),
    check(
      "contract_party_relationship_state_chk",
      sql`${t.state} IN ('active','inactive','retired')`,
    ),
  ],
);
