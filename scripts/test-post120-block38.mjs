...
assert.equal(inventory.currentAuthority.heading, "Current open-loop authority — Coordination Knowledge Library Block 6 — 2026-09-21");
assert.equal(inventory.currentAuthority.uncheckedItems.length, 3, "current authority preserves publication, the next knowledge block, and Ruben's deferred 2025 field evidence");
...
assert.equal(currentItems.filter(item => item.workClass === "PROVIDER_EVIDENCE").length, 1, "only the active Build 255 publication remains current provider evidence");
...
assert.ok(currentItems.some(item => item.statement.includes("Build 255") && item.statement.includes("publish")), "the active publication boundary remains explicit");
assert.ok(currentItems.some(item => item.statement.includes("Builds 256–260")), "the next Coordination Knowledge block remains explicit");
